...
## 🛑 MANDATORY CODE GENERATION WORKFLOW (STOP IF NOT COMPLETE)

**🚨 WHEN GENERATING CODE FOR A NEW OPERATION, YOU MUST FOLLOW THIS WORKFLOW:**
...
- [ ] **IF ANY NO → STOP → Complete missing step → Then proceed**
...
- [ ] **🔴 MANDATORY WORKFLOW STEP 5:** See Section 3 (Domain Rules) for complete details
...
- [ ] **🔴 MANDATORY WORKFLOW STEP 4:** See Section 7 (DTO Rules) for complete details
...
- [ ] **🔴 MANDATORY WORKFLOW STEP 6:** See Section 5 (System Abstractions Rules) for complete details
...
- [ ] **🔴 MANDATORY WORKFLOW STEP 9:** See Section 2 (Azure Functions Rules) for complete details
...
**STEP 8: Update ALL appsettings files (🔴 BLOCKING)**
- [ ] **MANDATORY:** Read AppConfigs.cs → List ALL new properties added
- [ ] **MANDATORY:** For EACH new property in AppConfigs.cs:
  - [ ] Update `appsettings.dev.json` → Add property with value/placeholder
  - [ ] Update `appsettings.qa.json` → Add property with value/placeholder
  - [ ] Update `appsettings.stg.json` → Add property with value/placeholder
  - [ ] Update `appsettings.prod.json` → Add property with value/placeholder
  - [ ] Update `appsettings.dr.json` → Add property with value/placeholder
- [ ] **MANDATORY:** Verify ALL appsettings files have ALL properties from AppConfigs.cs
- [ ] **🔴 CRITICAL - BLOCKING:** Verify ALL appsettings VALUES against Boomi JSON files (source of truth):
  - [ ] **BLOCKING:** For EACH property in appsettings that has a known value (not TODO placeholder):
    ...
    - Read Boomi component JSON file (component_*.json) → Locate `DefinedProcessProperties/definedProcessProperty[]/defaultValue/_` field
    - Compare appsettings value with Boomi component JSON defaultValue EXACTLY (case-sensitive, character-by-character)
    - If values don't match → **VIOLATION** → Fix appsettings value to match Boomi JSON → Re-verify
  - [ ] **BLOCKING:** For error email configuration properties (File_Name prefix, HasAttachment):
    ...
    - **🔴 CRITICAL - HasAttachment Value Verification:**
      - **MANDATORY:** Read Phase 1 Section 2 (Process Properties Analysis) → Find `process.DPP_HasAttachment` property
      - **MANDATORY:** Check the "Source" column for `process.DPP_HasAttachment`:
        - If Source is "Defined parameter: PP_<Name>.Has_Attachment" → Use component JSON `defaultValue/_` field
        - If Source is "Static value" or "Concatenation" → **DO NOT use component JSON defaultValue** → Use the actual value from Phase 1 Section 2
      - **MANDATORY:** Verify the actual value used in the process (from Phase 1 Section 2) matches appsettings value
      - **⚠️ WARNING:** Component JSON `defaultValue/_` may NOT be the actual value used if the process writes a static value or transforms the value
      - **BLOCKING:** If Phase 1 Section 2 shows a different source (static/concatenation) than defined parameter → Use Phase 1 value, NOT component JSON defaultValue
...
**🛑 IF ANY CHECKPOINT FAILS → STOP ALL CODE GENERATION → Complete missing step → Then proceed**

---

## 📋 MASTER CHECKLIST - FILES TO UPDATE FOR NEW OPERATION

**🚨 WHEN ADDING A NEW OPERATION, YOU MUST UPDATE/CREATE THESE FILES:**
...
**🛑 VERIFY ALL FILES UPDATED BEFORE DECLARING CODE GENERATION COMPLETE**

---

## 🔴 **ALL RULES IN THIS DOCUMENT ARE CRITICAL AND MANDATORY** 🔴

---

## 1. Folder Structure Rules

**🚨 BEFORE CREATING ANY CODE:** Verify EVERY folder location against this structure.

### 1.1 Complete Structure (Standard Process Layer)

```
<ProjectName>/
├── ConfigModels/                      # AppConfigs.cs, nested configs
├── Constants/                         # ErrorConstants.cs, InfoConstants.cs
├── Domains/                           # Business objects with IDomain<T>
│   ├── <EntityName>.cs               # Single domain entity (if only ONE domain exists) - **🔴 CRITICAL: NO subfolder if only one domain**
│   └── <BusinessDomain>/              # Business domain subfolder (ONLY if MULTIPLE domains exist) (e.g., Leave, Maintenance, Properties, etc.)
│       └── <EntityName>.cs           # Domain entity with business logic (e.g., Leave.cs, WorkOrder.cs, Lead.cs) - **🔴 CRITICAL: Use entity name, NOT operation name**
├── DTOs/                              # ALL request/response DTOs
│   ├── <OperationName>/              # Per-operation folder (<Operation>Request, Get<Entity>, etc.) - **❌ NEVER include SOR name**
│   │   ├── <Feature>ReqDTO.cs       # Request DTO
│   │   └── <Feature>ResDTO.cs       # Response DTO
├── Enums/                             # Enums with [StringValue]
├── Extensions/                        # Project-specific extensions (optional)
├── EmailTemplate/                     # Optional - Embedded HTML templates
│   └── <TemplateName>.html           # Email templates as embedded resources
├── Functions/                         # Azure Functions (HTTP entry points)
│   └── <Feature>Functions/           # 🔴 MANDATORY: Subfolder with plural name
│       └── <Feature>Function.cs      # Function file inside subfolder
├── Helper/                            # ResponseDTOHelper (mandatory)
├── ProcessAbstractions/               # Process→Process Layer calls (optional)
│   └── <ModuleName>/
│       └── <Feature>Proc.cs
│   └── Interfaces/ (optional)
│       └── I<Feature>Proc.cs
...
├── SystemAbstractions/                # Process→System Layer calls
│   └── <ModuleName>Mgmt/
│       └── <Feature>MgmtSys.cs
│   └── Interfaces/ (optional)
│       └── I<Feature>Mgmt.cs
...
├── appsettings.prod.json
├── appsettings.dr.json
└── <ProjectName>.csproj, <ProjectName>.sln
```
...
| **ProcessAbstractions/** | Process→Process calls (optional, not all projects) | Inter-process communication |
| **SystemAbstractions/** | Process→System Layer calls (mandatory) | External communication |
...
**🔴 CRITICAL: Domain Folder Structure Rules**

- **✅ CORRECT: Single Domain (No Subfolder):** If only ONE domain exists for a business domain → `Domains/<EntityName>.cs`
  - ✅ **CORRECT:** `Domains/<Entity>.cs` with class `<Entity>` (if only one domain exists)
  - ✅ **CORRECT:** `Domains/<Entity>.cs` with class `<Entity>` (if only one domain exists)
- **✅ CORRECT: Multiple Domains (Subfolder Required):** If MULTIPLE domains exist for a business domain → `Domains/<BusinessDomain>/<EntityName>.cs`
  - ✅ **CORRECT:** `Domains/<BusinessDomain>/<Entity>.cs` and `Domains/<BusinessDomain>/<Entity>Attachment.cs` (multiple related domains)
  - ✅ **CORRECT:** `Domains/<BusinessDomain>/<Entity>.cs`, `Domains/<BusinessDomain>/<Entity>Details.cs`, etc. (multiple related domains)
- **✅ CORRECT Domain Class Name:** Use the business entity name, NOT the operation name
  - ✅ **CORRECT:** Class `<Entity>` (for `<Operation>` operation)
  - ❌ **WRONG:** Class `<Operation>Request` (operation name, not entity name)
  - ❌ **WRONG:** Class `<Operation>` (operation name, not entity name)
  - ❌ **WRONG:** Class `<Adjective><Operation>` (e.g., `LateLogin`, `CreateLeave`, `GetEmployee`)
- **Rule:** Domain represents the business entity, not the operation. Operations are in Functions/DTOs, entities are in Domains.
- **🔴 CRITICAL - Domain Naming Rules:**
  - **✅ CORRECT:** Generic business entity names (e.g., `Leave`, `Attendance`, `Employee`, `Record`)
  - **❌ WRONG:** Operation-specific names (e.g., `LateLogin`, `CreateLeave`, `GetEmployeeDetails`)
  - **❌ WRONG:** Names with adjectives/qualifiers from operation (e.g., `LateLogin` should be `Attendance`, `CreateLeave` should be `Leave`)
  ...
  - **🔴 CRITICAL:** When creating a domain, ask: "What is the generic business entity this represents?" NOT "What operation is this for?"
- **🔴 CRITICAL:** Only create a subfolder if MULTIPLE domains exist in that business domain. If only one domain exists, put it directly in `Domains/` without a subfolder.
...
**🔴 CRITICAL: DTO Folder Naming Rules**

- **✅ CORRECT: Operation-Based Naming:** DTO folder names MUST be based on the operation, NEVER include SOR name
  - ✅ **CORRECT:** `DTOs/<Operation>Request/` (e.g., `DTOs/CreateEntityRequest/`, `DTOs/GetEntity/`)
  - ✅ **CORRECT:** `DTOs/Get<Entity>/` (e.g., `DTOs/GetEmployee/`, `DTOs/GetLeave/`)
- **❌ WRONG: SOR-Based Naming:** DTO folder names MUST NOT include SOR (System of Record) name
  - ❌ **WRONG:** `DTOs/<Operation><SOR>/` (e.g., `DTOs/CreateEntityD365/`, `DTOs/GetEntityOracle/`)
  - ❌ **WRONG:** `DTOs/<Operation><SOR>Request/` (e.g., `DTOs/CreateEntitySAPRequest/`)
...
**🔴 CRITICAL: Function Folder Structure**

**WRONG:**
```
Functions/
└── <Feature>Function.cs  // ❌ Missing subfolder
```

**CORRECT:**
```
Functions/
└── <Entity>Functions/  // ✅ Plural subfolder matching generic domain name
    └── <Operation>Function.cs  // ✅ Function class is operation-specific
```

**Why:** Process Layer Functions MUST be in subfolders with plural names.

**🔴 CRITICAL - Function Folder Naming Rules:**
- **✅ CORRECT:** Function folder name MUST match the generic domain name (e.g., `<Entity>Functions` for `<Entity>` domain)
- **❌ WRONG:** Operation-specific folder names (e.g., `<Operation>Functions` should be `<Entity>Functions`)
...
- **Examples:**
  - Domain: `<Entity>` → Folder: `<Entity>Functions` (not `<Operation>Functions`)
  - **Real-world example:** Domain: `Attendance` → Folder: `AttendanceFunctions` (not `LateLoginFunctions`)
...
**🛑 BEFORE creating any code, verify folder structure:**
...
**🛑 IF ANY CHECKPOINT FAILS → STOP → Fix folder structure → Then proceed**

---

## 2. WHEN PROCESS LAYER IS REQUIRED (CRITICAL DECISION)

**🔴 CRITICAL RULE - Process Layer vs System Layer Entry Point:**

**Process Layer IS Required When:**
- ✅ **Multiple SORs:** Process orchestrates multiple System Layer Functions (cross-SOR orchestration)
- ✅ **Business Logic Orchestration:** Complex business logic, multi-step workflows, cross-SOR decisions
- ✅ **Error Handling Orchestration:** Multiple error handling System Layer calls (email operations in error handling are not implemented per "Email Operations in Error Handling/Observability" rule - System-Layer-Rules.md, Section 11)
- ✅ **Looping/Iteration:** Business logic that requires looping/iteration over multiple API calls
- ✅ **Request/Response Transformation with Business Logic:** Complex transformations that require business rules, validations, or multi-step processing

**System Layer CAN Be Entry Point When:**
- ✅ **Single SOR:** Only one System Layer operation (no cross-SOR orchestration)
- ✅ **No Orchestration:** No multiple System Layer calls, no business logic decisions across SORs
- ✅ **Simple Request/Response Transformation:** System Layer handles client request/response transformation using System Layer rules (DTOs, Handlers, mapping)
- ✅ **Direct Client Access:** External clients call System Layer Function directly (no Process Layer in between)

**Decision Rule:**
- **No orchestration needed** → System Layer can be entry point (clients call System Layer directly)
- **Orchestration needed** → Process Layer required (clients call Process Layer, which orchestrates System Layer)
...
**🔴 CRITICAL: Simple vs Complex Transformation**

**Simple Request/Response Transformation (System Layer CAN handle):**
- ✅ **Header extraction:** `req.Headers.TryGetValue("header-name")` - Reading HTTP headers and storing in DTO
- ✅ **Query parameter extraction:** `req.Query.TryGetValue("param-name")` - Reading query parameters and storing in DTO
- ✅ **Simple conditional logic:** If empty/null, use default value (e.g., if dateTime is empty, use current date)
- ✅ **Date/time formatting:** Convert to ISO 8601, apply format mask, parse date strings
- ✅ **Field mapping:** Direct field-to-field mapping (no business rules, just data transformation)
- ✅ **Data type conversion:** String to int, date parsing, boolean conversion, etc.
- ✅ **Array/collection operations:** Filter, map, transform elements (simple operations, not business logic)
- ✅ **String operations:** Trim, uppercase, lowercase, substring, concatenation
- ✅ **Simple validation:** Null/empty checks, required field validation (NOT complex business rules)

**Complex Request/Response Transformation (Process Layer REQUIRED):**
- ❌ **Multi-step business rules:** Multiple conditions with business logic (if status=X and role=Y then Z, else W)
- ❌ **Cross-SOR data enrichment:** Fetch data from multiple SORs to enrich request before calling target SOR
- ❌ **Business validation rules:** Complex validation logic beyond simple null/empty checks (e.g., validate business rules, check permissions, validate against business policies)
- ❌ **Decision-based transformations:** Transform based on business decisions (if status=X then transform to Y format, else transform to Z format)
- ❌ **Aggregation logic:** Sum, count, group by with business rules (not just simple math)
- ❌ **Workflow orchestration:** Multi-step processing with dependencies between steps
- ❌ **Business rule evaluation:** Evaluate business conditions and apply different transformations based on results
...
- **Boomi process:** Entry point (WSS/HTTP) → Extract companyCode from header → Transform dateTime (if empty, use current date) → Single SOR operation (D365) → Response
- **Analysis:** 
  - ✅ Single SOR (D365 only)
  - ✅ Simple transformations:
    - Header extraction: `req.Headers.TryGetValue("company-code")` → Simple data extraction
    - DateTime transformation: If empty, use current date → Simple conditional (not complex business logic)
  - ✅ No orchestration (no multiple System Layer calls)
...
- **Boomi process:** Entry point (WSS/HTTP) → Single SOR operation → Response
...
- **Boomi process:** Entry point (WSS/HTTP) → Validate business rules → Decision (if status=X) → SOR-1 operation → If SOR-1 success, call SOR-2 → Aggregate results → Response
- **Analysis:**
  - ❌ Multiple SORs (SOR-1 and SOR-2)
  - ❌ Complex business logic (decision-based routing, aggregation)
  - ❌ Orchestration required (multiple System Layer calls with dependencies)
- **Decision:** ❌ Process Layer required
...
Is there orchestration? (Multiple SORs, multiple System Layer calls)
  ├─ YES → Process Layer Required
  └─ NO → Continue
    ↓
Are transformations needed?
  ├─ NO → System Layer Entry Point
  └─ YES → Continue
    ↓
Are transformations SIMPLE? (Header extraction, simple conditionals, date formatting)
  ├─ YES → System Layer Entry Point
  └─ NO → Continue
    ↓
Are transformations COMPLEX? (Multi-step business rules, cross-SOR enrichment, decision-based)
  ├─ YES → Process Layer Required
  └─ NO → System Layer Entry Point (default - when in doubt, System Layer can handle it)
```

## 2. Azure Functions Rules

**🚨 CRITICAL:** Functions are thin HTTP orchestrators.

### Azure Functions - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE ANY FUNCTION UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I created Domain class?
2. ✅ **STOP AND VERIFY:** Have I created Service class?
3. ✅ **STOP AND VERIFY:** Do I know the exact folder location (Functions/<Feature>Functions/ subfolder)? (See Section 1.5 for folder structure rules)
4. ✅ **STOP AND VERIFY:** Have I read ALL sections in "AZURE FUNCTIONS RULES"?
5. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document (Section 13: Orchestration Diagram) for ALL System Layer calls (success path AND error path)?
6. ✅ **STOP AND VERIFY:** Have I verified orchestration requirements (single call vs. multiple calls, error handling orchestration)?
7. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document for HTTP headers and request parameters required by the Boomi process?
8. ✅ **STOP AND VERIFY:** Have I verified which fields the System Layer DTO requires (check System Layer DTO validation methods)?
9. ✅ **STOP AND VERIFY:** Have I identified all sources of data (request body, headers, query parameters, config) needed to populate System Layer DTO fields?
10. ✅ **🔴 CRITICAL - STOP AND VERIFY:** Do I understand that I MUST pass DTO (not Domain) to Service? Domain is for internal purposes only, DTO is the contract between layers (see Section 8.1: Services Rules - Pre-Creation Validation)?
11. ✅ **🔴 CRITICAL - STOP AND VERIFY:** Do I understand that Function names MUST NOT contain "API" keyword? "API" suffix/keyword is ONLY for System Layer Functions. Process Layer Functions MUST NOT contain "API", "Api", or "api" in Function attribute, class name, or file name.

**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete missing step → Then create Function**

**🔴 MANDATORY WORKFLOW STEP 9: Functions**

**🚨 BEFORE creating Functions (BLOCKING - CANNOT PROCEED WITHOUT COMPLETING):**
...
- [ ] **BLOCKING:** Did I read "🛑 PRE-CREATION VALIDATION" in Section 2?
- [ ] **BLOCKING:** Did I check "COMMON MISTAKES" #19 (`var` keyword), #22 (Verbose Header Reading)?

**If you write code without reading Section 2 first → DELETE code → Read Section 2 → Retry**

**Implementation:**
- [ ] Create Function classes
- [ ] **🔴 CRITICAL:** Verify Function attribute, class name, and file name do NOT contain "API" keyword (NO "API", "Api", or "api")
...
**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete missing step → Then proceed**

### Azure Functions - Folder Structure & Naming

**🔴 CRITICAL: Function Folder Structure**

**🔴 CROSS-REFERENCE:** See Section 1.5 (Functions Folder Structure Rules) for complete folder structure details.
...
- **🔴 CRITICAL:** Function folder name represents the business entity (matches domain), while Function class/attribute represent the operation.

**WRONG:**
```
Functions/
└── EntityFunction.cs  // ❌ Missing subfolder
```

**CORRECT:**
```
Functions/
└── EntityFunctions/  // ✅ Plural subfolder matching generic domain name
    └── CreateEntityFunction.cs  // ✅ Function class is operation-specific
    └── GetEntityFunction.cs  // ✅ Multiple operations can be in same folder
```

**Example with Generic Domain:**
```
Functions/
└── AttendanceFunctions/  // ✅ Matches generic domain name (Attendance)
    └── LateLoginFunction.cs  // ✅ Operation-specific class
    └── EarlyLoginFunction.cs  // ✅ Another operation for same entity
```
...
- **Examples:**
  - Domain: `<Entity>` → Folder: `<Entity>Functions` (not `<Operation>Functions`)
  - **Real-world example:** Domain: `Attendance` → Folder: `AttendanceFunctions` (not `LateLoginFunctions`)
- **🔴 CRITICAL: ❌ NEVER EVER use "API" keyword in Process Layer Function names** - This is a MANDATORY rule with NO EXCEPTIONS
  - ❌ **WRONG:** `[Function("CreateEntityAPI")]` - Contains "API" keyword
  - ❌ **WRONG:** `CreateEntityAPIFunction` - Class name contains "API" keyword
  - ❌ **WRONG:** `CreateEntityApiFunction` - "Api" (case-insensitive) is also PROHIBITED
  - ✅ **CORRECT:** `[Function("CreateEntity")]` - No "API" keyword
  - ✅ **CORRECT:** `CreateEntityFunction` - No "API" keyword
...
- **❌ NEVER include SOR (System of Record) name in Function names** - Function names MUST be based on the operation it performs, NOT the target SOR
  - ❌ **WRONG:** `<Operation><SOR>Function` (e.g., `<Operation>D365Function`, `<Operation>OracleFunction`, `<Operation>SAPFunction`)
  - ✅ **CORRECT:** `<Operation>Function` (e.g., `<Operation>Function`)
...
|-------|-----------|-------------------|---------|
| **Process Layer** | `<Operation>Function` | `[Function("<Operation>")]` | `CreateEntityFunction` → `[Function("CreateEntity")]` |
| **System Layer** | `<Operation>API` | `[Function("<Operation>")]` | `CreateEntityAPI` → `[Function("CreateEntity")]` |

### Azure Functions - Function Reuse & Versioning

**🔴 CRITICAL:** Before creating any new Function, you MUST:

1. ✅ **Check if Function already exists** in the target Process Layer repository
...
2. ✅ **If Function exists:**
   - **Analyze changes:** Are the changes non-breaking (backward compatible)?
     - ✅ **Non-breaking changes** → Update existing Function
     ...
     - ❌ **Breaking changes** → Create new versioned Function
...
3. ✅ **If Function does NOT exist:** Create new Function following naming conventions

**Function Versioning Pattern:**
- Original: `<Operation>Function` → `[Function("<Operation>")]`
- Version 2: `<Operation>V2Function` → `[Function("<Operation>V2")]` (if breaking changes)
- Version 3: `<Operation>V3Function` → `[Function("<Operation>V3")]` (if further breaking changes)

**Breaking Change Examples:**
```csharp
// ❌ BREAKING: Removed required field
// Original: public string <Entity>Id { get; set; }  // Required
// New: Field removed → BREAKING

// ❌ BREAKING: Changed field type
// Original: public DateTime <Entity>Date { get; set; }
// New: public string <Entity>Date { get; set; } → BREAKING

// ✅ NON-BREAKING: Added optional field
// Original: public string <Entity>Name { get; set; }
// New: public string <Entity>Name { get; set; }
//      public string? <Entity>Email { get; set; }  // Optional → NON-BREAKING
```
...
└─ YES → Are changes breaking?
    ├─ NO → Update existing Function
    └─ YES → Create versioned Function (V2, V3, etc.)
```

**🔴 CRITICAL: Same Operation, Different SOR (System of Record)**
...
**Example:**
- Existing: `<Operation>Function` → calls `I<Entity>Mgmt` → `sys-<sor1>-mgmt` (`<SOR1>`)
- New: `<Operation>Function` → calls `I<SOR2>Mgmt` → `sys-<sor2>-mgmt` (`<SOR2>`)
...
- **🔴 CRITICAL:** If both Functions need to exist in the same repository, they must be differentiated by route or be in different repositories. The Function class name MUST remain `<Operation>Function` (no SOR name in Function name).
...
- [ ] **If same operation, different SOR → CREATE NEW Function (documented in Phase 2)**
- [ ] Decided: Update existing OR Create new OR Create versioned
- [ ] Documented decision in Phase 2 documentation

**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete repository analysis → Then proceed**

### Azure Functions - Orchestration Verification

**🔴 CRITICAL: FUNCTION ORCHESTRATION VERIFICATION (MANDATORY)**

**🚨 BEFORE creating a Function, you MUST verify orchestration requirements:**

1. ✅ **Check Phase 1 document (Section 13: Process and System Layer Orchestration Diagram)** to identify:
...
2. ✅ **Check Phase 1 document (Section 7: Execution Order)** to identify orchestration requirements:
...
3. ✅ **Determine orchestration location:**
   - **Single System Layer call** → Service handles it (Function just calls Service)
   - **Multiple System Layer calls (business logic)** → Service orchestrates (Function calls Service, Service orchestrates multiple System Abstractions - see Section 8.4: Services Rules - Handling Multiple Failures)
   - **Error handling System Layer calls (cross-cutting concerns)** → Function ONLY orchestrates (Services MUST NOT orchestrate cross-cutting concerns - see Section 8.2: Service Separation Rule)

4. ✅ **Verify error handling orchestration:**
   - If Phase 1 shows System Layer call on error → Function MUST orchestrate that System Layer call in error path (using `else` block, NOT try-catch)
   - If Phase 1 shows multiple error handling steps → Function MUST orchestrate all error handling System Layer calls
   - **Pattern:** Function `else` block → Call error handling Service (e.g., `NotificationService`) → Throw exception
   - **🔴 CRITICAL:** Services MUST NOT orchestrate error handling - Error notifications are cross-cutting concerns and violate Service Separation Rule (Section 8.2)
...
**🔴 CRITICAL: NEVER skip error handling orchestration:**
- ❌ **WRONG:** Function only handles success path, ignores error handling System Layer calls
- ✅ **CORRECT:** Function orchestrates both success and error paths, including all error handling System Layer calls identified in Phase 1

**🔴 CRITICAL: API Ecosystem Principle - HTTP Status Code Must Match Response Body (MANDATORY)**

**🚨 MANDATORY RULE:** HTTP status code MUST accurately reflect the response body state. This is a fundamental API ecosystem principle.

**Principle:**
- ✅ **If HTTP status code is success (2xx)** → Response body MUST contain success data
- ✅ **If HTTP status code is error (4xx/5xx)** → Response body MUST contain error data
- ❌ **NEVER return HTTP 200 with error body** - This violates RESTful API principles

**Rule:**
- ✅ **CORRECT:** Error path throws exception (e.g., `PassThroughHttpException`) → Results in HTTP error status code (4xx/5xx) with error body
- ❌ **WRONG:** Error path returns `BaseResponseDTO` directly → Results in HTTP 200 with error body
...
{
    // Error path: <BusinessOperation> returns non-20x → send email → throw exception
    ...
    // 🔴 CRITICAL: API Ecosystem Principle - HTTP Status Code Must Match Response Body
    // Principle: If HTTP status code is success → body should be success
    //            If HTTP status code is error → body should be error
...
**Example Violations:**
- ❌ **WRONG:** `return new BaseResponseDTO("Error", errorMessage, errorResDto);` (returns HTTP 200 with error body)
- ✅ **CORRECT:** `throw new PassThroughHttpException(errorResponse, (HttpStatusCode)response.StatusCode);` (returns HTTP error status with error body)

**🔴 CRITICAL: Error Message Source Documentation (MANDATORY)**
- **🚨 MANDATORY RULE:** When error path contains multiple operations (e.g., business operation + error notification operation), Function MUST clearly document which error message is used in the response
...
- **Implementation Pattern:**
  ...
  {
      // Error path: <BusinessOperation> returns non-20x → send email → return error response
      BaseResponseDTO errorFromBusinessOperation = response.ExtractBaseResponse();
      ...
      // Return error response (matches Boomi <error-map-shape> → <return-shape> return with error message)
...
- **Example Violations:**
  - ❌ **WRONG:** No comment explaining which error message is used
  - ❌ **WRONG:** Using error message from email operation in response (should use business operation error message)
  - ❌ **WRONG:** Ambiguous variable names (e.g., `errorMessage` without clarifying source)
  - ✅ **CORRECT:** Clear comments documenting error message comes from business operation, email operation failures logged separately
...
- **🔴 CRITICAL:** Functions MUST pass DTOs to Services (NEVER pass Domain objects)

**KEY RULES:**
- ✅ Functions inject Services (can inject directly: `<Entity>Service` OR via interface: `I<Entity>Service`)
- ✅ Functions pass DTOs to Services (not Domain objects, not individual parameters)
- ❌ Functions MUST NOT inject System/Process Abstractions directly
- ❌ Functions MUST NOT call System/Process Layers directly
- ❌ Functions MUST NOT pass Domain objects to Services
- ✅ **🔴 CRITICAL:** Functions MUST use constants (InfoConstants.cs, ErrorConstants.cs) for business logic string literals, NEVER hardcode strings
  - **🔴 CROSS-REFERENCE:** Use constants instead of hardcoded strings (ErrorConstants.cs, InfoConstants.cs)
...
public class Create<Entity>Function {
    ...
    private readonly <Entity>Service _service;  // ✅ Direct injection allowed
...
public class Create<Entity>Function {
    private readonly ILogger<Create<Entity>Function> _logger; 
    private readonly I<Entity>Service _service;  // ✅ Interface injection also allowed

    public Create<Entity>Function(ILogger<Create<Entity>Function> l, I<Entity>Service s) {
        _logger = l; 
        _service = s; 
    }
    [Function("CreateEntity")]  // ✅ NO "API" keyword - Process Layer Functions MUST NOT contain "API"
    public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "entity/create")] HttpRequest req) {
        ...
        // ✅ Headers collection is need-based - only if headers need propagation to System Layer/SOR
        // dto.Headers = req.Headers.ToDictionary(h => h.Key.ToLower(), h => h.Value.FirstOrDefault() ?? string.Empty);
        dto.Validate(); 
        // ✅ Domain is created and populated for internal purposes (business logic, validation, etc.)
        <Entity>Domain domain = new <Entity>Domain(); 
        dto.Populate(domain);
        // ✅ Pass DTO to Service (not domain) - DTO is used for data passing between layers
...
public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req) {
    ...
    // ❌ NOTE: This example shows passing domain to service - THIS IS WRONG per rulebook rules
    // ✅ CORRECT: Should be: HttpResponseSnapshot r1 = await _service1.GetEntity(dto);
    ...
    // ❌ NOTE: This example shows passing domain + extra parameters - THIS IS WRONG per rulebook rules
    // ✅ CORRECT: Should populate id and param in dto, then: HttpResponseSnapshot r3 = await _service3.Create<Entity>(dto);
...
```csharp
// ❌ NOTE: This example shows [Function("BatchCreateAPI")] which violates the "NO API keyword" rule
// ✅ CORRECT: Should be [Function("BatchCreate")] - NO "API" keyword
[Function("BatchCreate")]  // ✅ CORRECTED: Removed "API" keyword
public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req) {
    // ❌ NOTE: Original example had "BatchCreateAPI" in logging - violates "NO API keyword" rule
    // ✅ CORRECT: Should be "BatchCreate" - NO "API" keyword
    ...
    foreach (ItemReqDTO item in dto.Items) {
        // ❌ NOTE: try-catch is PROHIBITED per rulebook - exceptions should propagate to middleware
        // ❌ NOTE: var keyword is PROHIBITED - should use explicit types
        try { 
            // Domain is for internal purposes only (if needed)
            <Entity>Domain domain = new <Entity>Domain(); item.Populate(domain);
            // ✅ CORRECT: Pass DTO to Service (not Domain)
...
**🔴 CRITICAL NOTE:** The example below uses `var` keyword which is PROHIBITED. It is preserved from the original rulebook for reference but should use explicit types.
...
public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "entity/{entityId}")] HttpRequest req, string entityId) {
    ...
    // ❌ NOTE: var keyword is PROHIBITED - should be: <Operation>ReqDTO? dto = await req.ReadBodyAsync<<Operation>ReqDTO>();
    <Operation>ReqDTO? dto = await req.ReadBodyAsync<<Operation>ReqDTO>();
...
public class <Operation>Function {
    ...
    public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req) {
        ...
        // ✅ Headers collection is need-based - only if headers are used in business logic (e.g., `<header-name>` for DB lookups)
...
|-----------|---------|-----------|-------|
| **Function Attribute** | `[Function("<Operation>")]` | ✅ Always | **🔴 CRITICAL: NO "API" keyword** - NEVER use "API", "Api", or "api" in Function attribute |
| **Class Name** | `<Operation>Function` | ✅ Always | **🔴 CRITICAL: NO "API" keyword** - NEVER use "API", "Api", or "api" in class name |
| **File Name** | `<Operation>Function.cs` | ✅ Always | **🔴 CRITICAL: NO "API" keyword** - NEVER use "API", "Api", or "api" in file name |
| **Authorization** | `AuthorizationLevel.Anonymous` | ✅ Always | NEVER use Function or Admin level |
| **HTTP Method** | `"post"` | ✅ Always | POST is standard for all APIs |
| **Return Type** | `Task<BaseResponseDTO>` | ✅ Always | Always async, always BaseResponseDTO |
| **Logging** | `_logger.Info("HTTP Request received...")` | ✅ Always | Use Info() for function start |
| **Body Reading** | `await req.ReadBodyAsync<DTO>()` | ✅ Always | Framework extension method |
| **Null Check** | `throw new NoRequestBodyException(errorDetails:..., stepName:...)` | ✅ Always | MUST include errorDetails and stepName |
| **Headers** | `req.Headers.ToDictionary(...)` | ⚠️ **Need-based only** | Only if headers need propagation to System Layer/SOR (see 8.7) |
| **Validation** | `dto.Validate()` | ✅ Always | AFTER headers assignment (if headers collected) |

**🔴 CRITICAL: Boomi Request Source Mapping (MANDATORY)**
...
**Process Layer → System Layer Data Flow:**
- **If Process Layer receives data in request body:** Data is stored in Process Layer DTO → System Abstraction passes DTO properties to System Layer Function → System Layer Function MUST accept POST and receive data in request body via DTO
- **If Process Layer receives data in query parameters:** Data is stored in Process Layer DTO → System Abstraction passes DTO properties to System Layer Function → System Layer Function can accept POST with request body (prefer POST for consistency)
- **If Process Layer receives data in headers:** Data is stored in Process Layer DTO.Headers → System Abstraction passes headers to System Layer Function → System Layer Function extracts from headers

**How to Determine:**
1. Check Phase 1 Section 1 (Operations Inventory): Look for `Input Type` field
   - `Input Type: singlejson` → Request body (JSON) → Process Layer POST, System Layer POST
   - `Input Type: query` → Query parameters → Process Layer GET, System Layer GET or POST
   - `Input Type: header` → Headers → Process Layer POST, extract headers, System Layer extract headers
...
**Example Violations:**
- ❌ **WRONG:** Boomi receives JSON body (`Input Type: singlejson`) but Process Layer Function uses GET and extracts from query params
- ❌ **WRONG:** Process Layer receives data in request body but System Layer Function uses GET with query params
- ✅ **CORRECT:** Boomi receives JSON body → Process Layer POST with `req.ReadBodyAsync<DTO>()` → System Abstraction passes DTO to System Layer → System Layer POST with `req.ReadBodyAsync<DTO>()`
| **Domain Population** | `dto.Populate(_domain)` | ✅ Always for standard Process Layer | NOT for Shared Services |
| **Response Mapping** | `ResponseDTOHelper` | ✅ Always | NEVER manual JSON parsing |
| **Error Handling** | PassThroughHttpException, BusinessCaseFailureException | ✅ Always | Framework exceptions only |
| **Route Parameters** | Match parameter name (case-insensitive) | ✅ If using route | Parameter name doesn't need exact case match; can use Tuple pattern for route parameters |

**🔴 CRITICAL: Route Validation**
- ❌ **WRONG:** `Route = "api/<resource>/<operation>"` - **PROHIBITED**
- ✅ **CORRECT:** `Route = "<resource>/<operation>"` - Azure Functions automatically adds `/api` prefix
- **🔴 CRITICAL:** Azure Functions runtime automatically prefixes all routes with `/api`, so NEVER include `"api"` in Route attribute
- **🔴 CRITICAL:** This rule has been missed - DO NOT SKIP THIS CHECKPOINT
- **🔴 CRITICAL: DO NOT use Route attribute when Function name is sufficient** - When you don't need nested routes (e.g., `"<resource>/<operation>"`), DO NOT use Route attribute. Azure Functions automatically uses Function name as route when Route attribute is omitted. Only use Route when you need nested routes like `"<resource>/<operation>"` or route parameters like `"<resource>/{id}"`
  ...
  - **When NOT to use Route:** When the route matches the Function name exactly (e.g., `[Function("CreateEntity")]` → route is automatically `CreateEntity`)

### Azure Functions - Headers Handling

**🔴 CRITICAL: Headers collection is NEED-BASED ONLY**

**When to collect headers:**
- ✅ Headers need to be propagated to System Layer
- ✅ System Layer has validations on these headers
- ✅ System Layer needs to pass these headers to SOR (System of Record)
- ✅ Headers are used in business logic (e.g., `<header-name>` for database lookups)
- ✅ **Headers are defined in Boomi contract/process JSON files** - Only add headers that exist in the Boomi process definition

**When NOT to collect headers:**
- ❌ Headers are not used anywhere downstream
- ❌ Only technical headers (TestRunId, RequestId) are needed (these are automatically added by System Abstractions)
- ❌ No business logic depends on request headers
- ❌ **Headers are NOT defined in Boomi contract/process JSON files** - Do NOT add headers that don't exist in the Boomi process definition
- ❌ **Do NOT add header constants (e.g., `HeaderName<Field>`) or helper methods (e.g., `Get<Field>ValueFromHeader()`) unless the header is actually in the Boomi contract and required**

**🔴 CRITICAL: Header Reading Pattern (MANDATORY)**

**🚨 When reading headers to populate DTO properties, you MUST use the clean, idiomatic C# pattern:**

**🔴 CRITICAL: Determine WHERE to assign header value BEFORE reading:**
- **STOP:** Check Phase 1 Section 17 (Field Mapping Analysis) or Section 23 (Complete Request Payload Documentation)
- **If property is part of business entity data** (sent to System Layer/SOR as part of entity payload) → ✅ Assign to nested request object (e.g., `dto.<EntityRequest>.<PropertyName>`)
- **If property is NOT part of business entity** → Assign to root-level DTO property or use directly

**✅ CORRECT Pattern (Header value is part of entity data):**
```csharp
using Microsoft.Extensions.Primitives; // Add at top of file

// Header value is part of entity data → Assign to nested request object
...
**✅ CORRECT Pattern (Header value is NOT part of entity data):**
```csharp
using Microsoft.Extensions.Primitives; // Add at top of file

// Header value is NOT part of entity data → Assign to root-level DTO property
...
**❌ WRONG Patterns:**
```csharp
// ❌ WRONG: Verbose if-block pattern
...
dto.PropertyName = req.Headers.TryGetValue("header-name", out var headerValue)  // ❌ var is prohibited
    ? headerValue.FirstOrDefault() ?? string.Empty
    : string.Empty;

// ❌ WRONG: Full namespace in method call
if (req.Headers.TryGetValue("header-name", out Microsoft.Extensions.Primitives.StringValues headerValue))
{
    dto.PropertyName = headerValue.FirstOrDefault() ?? string.Empty;
}
...
**Key Points:**
1. ✅ **ALWAYS add `using Microsoft.Extensions.Primitives;`** at the top of the file to use `StringValues` directly
2. ✅ **ALWAYS use ternary operator pattern** for single-line header reading
3. ✅ **ALWAYS use explicit type `StringValues`** (NOT `var`) - matches rulebook requirement
4. ✅ **ALWAYS provide default value** (`string.Empty`) if header is missing
5. ✅ **ALWAYS use `FirstOrDefault() ?? string.Empty`** to safely extract the first header value
...
```csharp
// ✅ ONLY if headers need to be propagated to System Layer/SOR
dto.Headers = req.Headers.ToDictionary(h => h.Key.ToLower(), h => h.Value.FirstOrDefault() ?? string.Empty);
```
...
```csharp
// ✅ ONLY if headers are used in business logic (e.g., `<header-name>` for DB lookups)
...
```csharp
// ✅ CORRECT - If headers are not needed downstream
<Operation>ReqDTO? dto = await req.ReadBodyAsync<<Operation>ReqDTO>();
if (dto == null) throw new NoRequestBodyException(...);
// ❌ NO headers assignment needed
dto.Validate();
<Entity>Domain domain = new <Entity>Domain();
dto.Populate(domain);
```
...
**🔴 CRITICAL: NO TRY-CATCH BLOCKS:**
- ❌ **PROHIBITED:** NEVER use `try-catch` blocks in Functions
- ✅ **CORRECT:** Handle errors in `else` blocks, then throw exceptions
- ✅ **CORRECT:** Let ExceptionHandlerMiddleware catch and normalize all exceptions
...
**🔴 CRITICAL: Error Handling Orchestration Checkpoint**

**🚨 MANDATORY CHECKPOINT:** Error handling orchestration is implemented (Phase 1 Section 13 requirement)
...
- [ ] **STOP:** If Phase 1 Section 13 shows System Layer call on error (e.g., `<ErrorHandlingOperation>`) → Did I implement it in the error path?
- [ ] **STOP:** If error handling includes email notification → Did I verify email template requirements (see Section 2.7: Azure Functions - Error Handling & Orchestration)?
- [ ] **STOP:** If error handling includes email notification → Did I check subprocess JSON for email template structure?
- [ ] **STOP:** If error handling includes email notification → Did I use embedded HTML template (NOT hardcoded string)?
- [ ] **BLOCKING:** If Phase 1 Section 9 shows email subprocess uses attachment properties (e.g., `process.DPP_<PayloadProperty>`, `process.DPP_<FileNameProperty>`) → Did I extract request payload and construct attachment filename in error path?
- [ ] **BLOCKING:** If Phase 1 Section 9 shows email subprocess uses `process.DPP_<PropertyName>` → Did I populate ALL required properties from correct sources (request payload, headers, config, etc.)?
...
- ✅ **CORRECT:** Error path orchestrates error handling System Layer calls (e.g., `HttpResponseMessage errorResponse = await _service.<ErrorHandlingMethod>(...);` before throwing exception) and logs response
- ❌ **WRONG:** Fire-and-forget async calls: `await _service.<ErrorHandlingMethod>(...);` without storing response - **PROHIBITED**
- ✅ **CORRECT:** Error path extracts request payload for attachment: `string attachmentContent = JsonSerializer.Serialize(<RequestDTO>);` (ONLY in error path, NOT before service call)
- ✅ **CORRECT:** Error path constructs attachment filename: `string attachmentFileName = $"{_options.<FileNameProperty>}{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ss.fffZ}.txt";` (ONLY in error path, NOT before service call)
- ✅ **CORRECT:** Error path determines `<DecisionProperty>` via code logic: `bool <decisionProperty> = !string.IsNullOrWhiteSpace(<dataField1>) && !string.IsNullOrWhiteSpace(<dataField2>);`
- ✅ **CORRECT:** Email notification uses embedded HTML template with placeholders populated from correct sources
- ❌ **WRONG:** Function only handles success path, ignores error handling System Layer calls from Phase 1 Section 13
- ❌ **WRONG:** Error path passes `attachmentFileName: null, attachmentContent: null` when Phase 1 Section 9 shows attachment properties are used
- ❌ **WRONG:** Error path doesn't extract request payload (`process.DPP_<PayloadProperty>`) for attachment when Phase 1 Section 9 shows it's required
- ❌ **WRONG:** Request payload serialized before service call (e.g., `string requestPayload = JsonSerializer.Serialize(dto);` before `await _service.Method(dto);`) - **PROHIBITED** (must be in error path only)
- ❌ **WRONG:** Execution ID extracted before service call (e.g., `string executionId = RequestHeaders.RequestId.Item2.Value?.ToString();` before `await _service.Method(dto);`) - **PROHIBITED** (must be in error path only)
- ❌ **WRONG:** Error path doesn't construct attachment filename (`process.DPP_<FileNameProperty>`) when Phase 1 Section 9 shows it's required
- ❌ **WRONG:** Email body is hardcoded string (e.g., `$"Error: {message}"`) instead of embedded HTML template
- ❌ **WRONG:** Email body doesn't match Boomi template structure from subprocess JSON
- **🔴 CRITICAL:** Phase 1 Section 13 is the source of truth for error handling orchestration requirements
- **🔴 CRITICAL:** Phase 1 Section 9 (Subprocess Analysis) is the source of truth for ALL properties used in error handling subprocesses
- **🔴 CRITICAL:** If Phase 1 shows `<ErrorHandlingOperation>` in error path → Function MUST call `<ErrorHandlingService>` in error path
- **🔴 CRITICAL:** If Phase 1 Section 9 shows email subprocess uses attachment properties → Function MUST extract request payload and construct filename in error path (NOT before service call)
- **🔴 CRITICAL:** Request payload serialization MUST be in error path ONLY (when attachment is needed) - NEVER serialize before service call
- **🔴 CRITICAL:** Execution ID extraction MUST be in error path ONLY (when needed for error notifications) - NEVER extract before service call
- **🔴 CRITICAL:** Error handling System Layer calls are Process Layer orchestration responsibility, NOT middleware responsibility
- **🔴 CRITICAL:** Email templates MUST be embedded resources matching Boomi subprocess template exactly
- **🔴 CRITICAL:** Email template HTML MUST be extracted EXACTLY from Boomi subprocess JSON file (subprocess_*.json) - locate in msgTxt section of message shapes
- **🔴 CRITICAL:** Email template placeholders MUST match Boomi subprocess placeholders EXACTLY (verify placeholder numbering and order from msgParameters section in subprocess JSON)
- **🔴 CRITICAL:** Placeholder population parameter order MUST match Boomi subprocess msgParameters key order (verify parameter order in string.Format matches msgParameters)
- **🔴 CRITICAL:** ALL properties from Phase 1 Section 9 (Properties Read table) MUST be populated correctly in error path

### Azure Functions - Logging Message Patterns (MANDATORY)

**🔴 CRITICAL: Logging Messages MUST Use Literal Strings**

**🚨 MANDATORY RULE:** All logging messages (`_logger.Info()`, `_logger.Error()`, `_logger.Warning()`) MUST use literal strings, NEVER constants.

**Rule:**
- ✅ **CORRECT:** `_logger.Error($"Operation failed: {errorMessage}");` (literal string)
- ✅ **CORRECT:** `_logger.Info("HTTP Request received for CreateLeave.");` (literal string)
- ✅ **CORRECT:** `_logger.Error($"Failed to send error notification email. Status: {emailResponse.StatusCode}. Error: {emailErrorMessage}");` (literal string)
- ❌ **WRONG:** `_logger.Error($"{InfoConstants.OPERATION_FAILED}: {errorMessage}");` (constant in logging)
- ❌ **WRONG:** `_logger.Info(InfoConstants.HTTP_REQUEST_RECEIVED);` (constant in logging)
...
**What Goes Where:**
- ✅ **Constants (InfoConstants.cs):** Business logic strings (process names, success messages in BaseResponseDTO, email subjects, default values)
- ✅ **Literal Strings (Logging):** ALL logging messages (`_logger.Info()`, `_logger.Error()`, `_logger.Warning()`)

**Examples:**
```csharp
// ✅ CORRECT: Literal strings in all logging
...
- If any logging message uses a constant → **VIOLATION** → Replace with literal string
...
**🔴 CRITICAL: Always Store Async Service/System Abstraction Call Responses:**
- ❌ **NEVER:** `await _service.Method(...);` or `await _systemAbstraction.Method(...);` without storing response
- ✅ **ALWAYS:** Store response in variable: `HttpResponseMessage response = await _service.Method(...);`
- ✅ **ALWAYS:** Log the response after storing: `_logger.Info($"Service call completed. Status: {response.StatusCode}");`
...
**🔴 CRITICAL: Logging Check**
- [ ] **BLOCKING:** Run `grep -n "LogInformation" <file>` → If found → **VIOLATION** → Must use `_logger.Info()` → Fix → Retry
- [ ] **BLOCKING:** Run `grep -n "LogError" <file>` → If found → **VIOLATION** → Must use `_logger.Error()` → Fix → Retry
...
```csharp
// ❌ WRONG - Reusing same domain instance AND passing Domain to Service
<Entity>Domain domain = new <Entity>Domain();
foreach (<Entity>ItemDTO item in dto.Items) { 
    item.Populate(domain); // ❌ Overwrites previous item data!
    await _service.Create<Entity>(domain); // ❌ WRONG: Passing Domain instead of DTO
}

// ✅ CORRECT - Pass DTO to Service (Domain is for internal purposes only)
foreach (<Entity>ItemDTO item in dto.Items) { 
...
**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 CRITICAL: MANDATORY SYSTEMATIC VALIDATION WORKFLOW**

**🔴 CRITICAL CHANGE: NO GREP-BASED VALIDATION - SYSTEMATIC FILE-BY-FILE VALIDATION ONLY**
...
**🚨 CRITICAL:** The validation checklist below is NOT optional. You MUST systematically review the ENTIRE file against ALL applicable rules. Missing even ONE check is a VIOLATION. This is a BLOCKING checklist - you CANNOT claim validation complete until ALL steps pass for the file being validated.

**🔴 MANDATORY VALIDATION APPROACH:**
- ✅ **CORRECT:** Read the entire file from top to bottom, checking each line against applicable rules
- ✅ **CORRECT:** Document violations as you find them (file name, line number, rule violated)
- ✅ **CORRECT:** Fix all violations, then re-read the entire file to verify fixes
- ❌ **WRONG:** Using grep commands to search for patterns - This can miss violations
- ❌ **WRONG:** Skipping sections of the file - You MUST review the ENTIRE file
- ❌ **WRONG:** Claiming validation complete without systematically checking every rule

**🔴 CRITICAL: Email Template Validation (MANDATORY - DO NOT SKIP)**
- **🚨 BEFORE starting validation:** Check if Function has error handling with email notifications
- **🚨 IF email notifications exist:** Step 21 (Email template validation) is MANDATORY and BLOCKING
- **🚨 This rule has been missed repeatedly** - Email templates MUST be embedded resources, NOT hardcoded strings
- **🚨 If you skip Step 21 when email notifications exist → VIOLATION**

**🚨 AFTER creating Function, you MUST verify (BLOCKING - CANNOT SAVE FILE UNTIL ALL PASS):**

**🔴 CRITICAL: Domain Population Must Be Inline**
- ✅ **CORRECT:** `dto.Populate(domain);` called directly in Function method (INLINE)
- ❌ **WRONG:** Creating a separate method like `PopulateDomain(dto, domain)` and calling it from Function
- **Why:** Domain population must be visible and direct in the Function code for clarity and maintainability

**🔴 MANDATORY VIOLATION CHECKS (RUN BEFORE SAVING FILE - SYSTEMATIC LINE-BY-LINE VALIDATION):**

**🚨 CRITICAL: You MUST systematically review the ENTIRE file line by line against ALL applicable rules. Missing even ONE check is a VIOLATION.**
...
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for `try` or `catch` keywords
- [ ] **BLOCKING:** For EACH occurrence found: Is it in a comment? → ALLOWED | Is it in code? → **VIOLATION** → Document line number
- [ ] **BLOCKING:** If ANY violations found → STOP → Remove ALL try-catch blocks → Fix → Re-read file → Verify zero violations
- [ ] **BLOCKING:** Verify NO try-catch blocks exist - exceptions must propagate to middleware
- **🚨 CRITICAL:** You MUST read the entire file and document ALL try-catch occurrences. Do NOT skip this step.

**Step 2: API Keyword Prohibition Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for "API", "Api", or "api" keywords
- [ ] **BLOCKING:** For EACH occurrence found:
  - [ ] Is it in a comment explaining the rule? → **ALLOWED** (educational comments)
  - [ ] Is it in a wrong example marked with ❌? → **ALLOWED** (wrong examples for reference)
  - [ ] Is it in Function attribute (e.g., `[Function("...API")]`)? → **VIOLATION** → Remove "API" keyword → Fix → Re-read file
  - [ ] Is it in class name (e.g., `CreateEntityAPIFunction`)? → **VIOLATION** → Remove "API" keyword → Fix → Re-read file
  - [ ] Is it in file name? → **VIOLATION** → Rename file to remove "API" keyword → Fix → Re-read file
  - [ ] Is it in logging message? → **VIOLATION** → Remove "API" keyword from message → Fix → Re-read file
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify zero violations
- [ ] **BLOCKING:** Verify NO "API", "Api", or "api" keyword exists in Function attribute, class name, file name, or logging messages
- **🔴 CRITICAL:** "API" keyword is PROHIBITED in Process Layer Functions - This is a MANDATORY rule with NO EXCEPTIONS
- **🚨 CRITICAL:** You MUST read the entire file and document ALL API keyword occurrences. Do NOT skip this step.

**Step 3: Route Validation Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for `Route` attribute assignments
- [ ] **BLOCKING:** For EACH route found:
  - [ ] Does it contain `"api"` prefix? → **VIOLATION** → Remove "api" prefix → Fix → Re-read file
  - [ ] Does Route value match Function name exactly? → **VIOLATION** → Remove Route attribute (Function name is sufficient) → Fix → Re-read file
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify no "api" prefix and no redundant Route attributes
...
- **🚨 CRITICAL:** You MUST read the entire file and verify all routes. Do NOT skip this step.

**Step 4: Hardcoded String Literals Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for ALL string literals (quoted strings)
- [ ] **BLOCKING:** For EACH string literal found:
  - [ ] Is it a Function attribute name (e.g., `[Function("<Operation>")]`)? → **ALLOWED** (literal string) → Document as allowed
  - [ ] Is it a stepName in exception (e.g., `stepName: "ClassName.cs / Method"`)? → **ALLOWED** (literal string) → Document as allowed
  - [ ] Is it a logging message (e.g., `_logger.Info("message")`)? → **ALLOWED** (literal string) → Document as allowed
  - [ ] Is it a header name in `req.Headers.TryGetValue(...)`? → **MUST BE CONSTANT** → Document violation → Add to InfoConstants → Fix → Re-read file
  - [ ] Is it an error message in exception? → **MUST BE CONSTANT** → Document violation → Add to ErrorConstants → Fix → Re-read file
  - [ ] Is it a response message in BaseResponseDTO? → **MUST BE CONSTANT** → Document violation → Add to InfoConstants/ErrorConstants → Fix → Re-read file
  - [ ] Is it any other business logic string? → **MUST BE CONSTANT** → Document violation → Add to InfoConstants or ErrorConstants → Fix → Re-read file
- [ ] **BLOCKING:** For EACH violation: Add constant → Replace hardcoded string → Re-read file to verify fix
- [ ] **BLOCKING:** Re-read file → Verify only Function attributes, stepName, and logging messages remain as literal strings (all business logic strings replaced with constants)
- [ ] **BLOCKING:** Verify ALL business logic string literals use constants from InfoConstants.cs or ErrorConstants.cs
- **🚨 CRITICAL:** You MUST read the entire file, document ALL string literals, and verify business logic strings are replaced with constants. Do NOT skip this step.

**Step 5: Execution ID Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for execution ID usage
- [ ] **BLOCKING:** For EACH occurrence found:
  - [ ] Does it use `RequestHeaders.RequestId.Item2.Value?.ToString()`? → ✅ PASS
  - [ ] Does it use `Guid.NewGuid()`? → **VIOLATION** → Fix → Re-read file
  - [ ] Does it use hardcoded header name (e.g., `"x-execution-id"`)? → **VIOLATION** → Fix → Re-read file
  - [ ] Does it use any other method? → **VIOLATION** → Fix → Re-read file
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify correct method used
- **🚨 CRITICAL:** You MUST read the entire file and verify the correct method. Do NOT skip this step.

**Step 6: Header Name Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for header access patterns
- [ ] **BLOCKING:** For EACH header access found:
  - [ ] Does it use constant from InfoConstants? → ✅ PASS
  - [ ] Is it hardcoded (e.g., `"header-name"`)? → **VIOLATION** → Document violation → Add constant to InfoConstants → Replace hardcoded name → Re-read file
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify constants used
- **🚨 CRITICAL:** You MUST read the entire file and verify constants are used. Do NOT skip this step.

**Step 7: Logging Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for logging method calls
- [ ] **BLOCKING:** For EACH logging call found: Is it `LogInformation` or `LogError`? → **VIOLATION** → Must use `_logger.Info()` or `_logger.Error()` → Fix → Re-read file
- [ ] **BLOCKING:** Verify file has `using Core.Extensions;` (if using `_logger.Info()` extension method)
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify correct logging methods used
- **🚨 CRITICAL:** You MUST read the entire file and verify correct logging methods. Do NOT skip this step.

**Step 8: Type Declaration Check:**
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for `internal` keyword
- [ ] **BLOCKING:** For EACH occurrence found: Is it in class or method declaration? → **VIOLATION** → Must use `public` → Fix → Re-read file
- [ ] **BLOCKING:** If ANY violations found → STOP → Fix → Re-read file → Verify no `internal` keyword
- **🚨 CRITICAL:** You MUST read the entire file and verify no `internal` keyword. Do NOT skip this step.

**Step 9: Final Verification:**
- [ ] **BLOCKING:** Re-read the ENTIRE file from top to bottom → Verify ALL violations from previous steps are fixed
- [ ] **BLOCKING:** If ANY violation found → **STOP** → Fix → Re-read file → Re-verify
- [ ] **BLOCKING:** Only claim completion when ALL checks pass (entire file reviewed, zero violations)
- [ ] **BLOCKING:** Document final status: ✅ ALL PASS / ❌ FAILURES REMAIN
- **🚨 CRITICAL:** You MUST re-read the entire file and document final status. Do NOT claim completion without re-verification.

**Additional Validation Checkpoints:**

1. ✅ **STOP AND VERIFY:** Function is in `Functions/<Feature>Functions/<Feature>Function.cs` (plural subfolder)
2. ✅ **STOP AND VERIFY:** Uses `AuthorizationLevel.Anonymous`
3. ✅ **STOP AND VERIFY:** HTTP method is `"post"` (or appropriate method)
4. ✅ **STOP AND VERIFY:** Return type is `Task<BaseResponseDTO>`
5. ✅ **STOP AND VERIFY:** Reads body with `await req.ReadBodyAsync<DTO>()`
6. ✅ **STOP AND VERIFY:** Null check with `NoRequestBodyException`
7. ✅ **STOP AND VERIFY:** Headers assigned ONLY if needed: `req.Headers.ToDictionary(...)` (need-based)
8. ✅ **STOP AND VERIFY:** Calls `dto.Validate()`
9. ✅ **STOP AND VERIFY:** Creates Domain and calls `dto.Populate(domain)` for internal purposes
   - ✅ **CORRECT:** `<Entity>Domain domain = new <Entity>Domain(); dto.Populate(domain);` (domain for internal use)
   - **🔴 CRITICAL:** `dto.Populate(domain)` MUST be called INLINE (not in a separate method)
...
10. ✅ **STOP AND VERIFY:** Calls Service method with DTO (not System Abstraction directly, not domain)
    - ✅ **CORRECT:** `await _service.Method(dto);` (pass DTO to Service, not domain)
    - ❌ **WRONG:** `await _service.Method(domain, param1, param2, ...);` (passing domain + individual parameters)
    - ❌ **WRONG:** `await _service.Method(dto.Field1, dto.Field2, ...);` (passing individual DTO properties)
    - ❌ **WRONG:** `await _service.Method(dto, extraParam1, extraParam2, ...);` (passing DTO + extra parameters)
    - **🔴 CRITICAL: NEVER add extra parameters to Service/System Abstraction methods** - All data MUST be in the DTO. If data comes from headers/query parameters, populate the DTO property in the Function, then pass the DTO.
11. ✅ **STOP AND VERIFY:** Uses DTO properties directly after Validate() - NO null coalescing (`??`) operators
    - ✅ **CORRECT:** `dto.<FieldName>` (after Validate() passes)
    - ❌ **WRONG:** `dto.<FieldName> ?? string.Empty` (unnecessary and shows architectural misunderstanding)
12. ✅ **STOP AND VERIFY:** Reads HTTP headers if required by Boomi process (check Phase 1, Section 2: Process Properties Analysis)
    - **🔴 MANDATORY:** Follow "MANDATORY PROPERTY SOURCE VERIFICATION WORKFLOW" (see top of document) for EACH property
    - ✅ **CORRECT:** Read headers that Boomi process uses (e.g., `<header-name>` header → `process.DPP_<property-name>`)
    - ✅ **CORRECT:** Pass header values to System Abstraction if System Layer DTO requires them
    - ❌ **WRONG:** Ignoring headers that Boomi process reads and System Layer DTO requires
    - **🔴 CRITICAL:** Check "Source Type" in Phase 1 Section 2 - ONLY properties with `Source Type: trackparameter` and property name containing `inheader_` should be read from headers
    - **🔴 CRITICAL:** If property name contains `inheader_` → HTTP header (read in Function from `req.Headers`), design it to read it from headers, create it as a part of DTO and pass it to system layer in request DTO
    - **🔴 CRITICAL:** Properties with `Source Type: definedparameter` are configuration (AppConfigs), NOT headers - ❌ **NEVER read from headers**
13. ✅ **STOP AND VERIFY:** Reads query parameters if required by Boomi process (check Phase 1, Section 2: Process Properties Analysis)
    - ✅ **CORRECT:** Read parameters that Boomi process uses (e.g., `<parameter-name>` parameter → `dynamicdocument.<property-name>`)
    - ✅ **CORRECT:** Pass parameter values to System Abstraction if System Layer DTO requires them
    - ❌ **WRONG:** Ignoring parameters that Boomi process reads and System Layer DTO requires
    - **🔴 CRITICAL:** Check "Source Type" in Phase 1 Section 2 - ONLY properties with `Source Type: trackparameter` and property name containing `inquery_` should be read from query
    - **🔴 CRITICAL:** If property name contains `inquery_` → Query parameter (read in Function from `req.Query`), design it to read it from query, create it as a part of DTO and pass it to system layer in request DTO
14. ✅ **STOP AND VERIFY:** Configuration values (defined parameters) are in AppConfigs, NOT in headers/DTO (check Phase 1, Section 2: Process Properties Analysis)
    - ✅ **CORRECT:** Properties with `Source Type: definedparameter` → Add to AppConfigs class, add to appsettings.json, inject via IOptions<AppConfigs>
    - ❌ **WRONG:** Reading defined parameters from HTTP headers or adding to DTO
    - **🔴 CRITICAL:** Phase 1 Section 2 shows "Source Type: definedparameter" → This means configuration, NOT request data
15. ✅ **STOP AND VERIFY:** Verifies System Layer DTO requirements before mapping (check System Layer DTO validation methods)
    - ✅ **CORRECT:** Check `ValidateAPIRequestParameters()` or `ValidateDownStreamRequestParameters()` to see which fields are REQUIRED
    - ✅ **CORRECT:** Ensure ALL required System Layer DTO fields have a source (DTO property, HTTP header, query parameter, or config)
    - ❌ **WRONG:** Assuming a field can be populated from another field without verifying the correct source from Phase 1 document
16. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Business logic string literals use constants (InfoConstants.cs, ErrorConstants.cs)
    ...
    - ✅ **CORRECT:** `string executionId = RequestHeaders.RequestId.Item2.Value?.ToString() ?? string.Empty;` (rulebook requirement)
    ...
    - ❌ **WRONG:** `string processName = "<Operation> Process";` (hardcoded string literal in business logic - **PROHIBITED**)
    - ❌ **WRONG:** `req.Headers.TryGetValue("company-code", ...)` (hardcoded header name - **PROHIBITED**, must use constant)
17. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Uses `ResponseDTOHelper` for response mapping
    - ✅ **CORRECT:** `ResponseDTOHelper.Populate<Entity>Res(responseContent, resDto)` - mapping method in `Helper/ResponseDTOHelper.cs`
    ...
    - **🔴 CRITICAL:** ALL response mapping logic MUST be in `Helper/ResponseDTOHelper.cs` as `public static` methods
    - **🔴 CRITICAL:** Function class MUST NOT contain any mapping/transformation methods - Functions are thin orchestrators
18. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Async patterns follow rulebook requirements
   ...
   - ✅ **CORRECT:** `HttpResponseSnapshot response = await _service.Method(...);` (store return value with explicit type)
   - ✅ **CORRECT:** Check response status: `if (!response.IsSuccessStatusCode) { ... }`
   - ❌ **WRONG:** `_ = await _service.Method(...);` (discarding task result - **PROHIBITED**)
   - ❌ **WRONG:** `await _service.Method(...);` without storing return value (missing response status check)
   - **🔴 CRITICAL:** ALL async calls MUST store return values for consistency and proper error handling
   - **🔴 CRITICAL:** ALL async calls MUST check response status codes

19. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Configuration files validation completed
   ...
   - **🚨 CRITICAL:** `appsettings.json` validation is MANDATORY and MUST be completed as part of Process Layer validation

20. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** host.json validation completed
   ...
   - **🚨 CRITICAL:** `host.json` validation is MANDATORY and MUST be completed as part of Process Layer validation

21. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Email template validation (if Function has error handling with email notifications)
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Check if this Function has error handling that includes email notifications (check error path in Function code)
   - [ ] **BLOCKING:** If YES → Did I verify email templates are embedded HTML resources (NOT hardcoded strings)?
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for email body construction patterns (`emailBody`, `BuildErrorEmailBody`, HTML strings, etc.)
   - [ ] **BLOCKING:** For EACH email body construction found:
     - [ ] Is it calling a helper method that reads embedded template (e.g., `ResponseDTOHelper.ReadEmailTemplate(...)`)? → ✅ PASS
     - [ ] Is it a hardcoded HTML string (e.g., `$@"<!DOCTYPE html>..."` or `"<html>..."`)? → **VIOLATION** → Must use embedded template → Document line number → Fix → Re-read file
     - [ ] Is it a private method that builds email body with hardcoded HTML? → **VIOLATION** → Must use embedded template → Document line number → Fix → Re-read file
   ...
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Verify NO hardcoded email HTML strings in Function or Service code (e.g., `string.Format(InfoConstants.EMAIL_BODY_TEMPLATE, ...)`)
   - [ ] **BLOCKING:** Read InfoConstants.cs and ErrorConstants.cs files → Verify NO email HTML constants exist
   - ✅ **CORRECT:** Email notification uses embedded HTML template: `string template = ResponseDTOHelper.ReadEmailTemplate("<ProjectName>.EmailTemplate.<TemplateName>.html"); string emailBody = string.Format(template, processName, environment, executionId, errorMessage);`
   - ✅ **CORRECT:** Email template file exists: `EmailTemplate/<TemplateName>.html` with exact Boomi HTML structure extracted from subprocess JSON msgTxt section
   - ✅ **CORRECT:** Email template marked as embedded resource in .csproj: `<EmbeddedResource Include="EmailTemplate\<TemplateName>.html" />`
   - ❌ **WRONG:** Hardcoded email body: `string emailBody = $@"<!DOCTYPE html>...";` - **PROHIBITED**
   - ❌ **WRONG:** Private method with hardcoded HTML: `private string BuildErrorEmailBody(...) { return $@"<!DOCTYPE html>..."; }` - **PROHIBITED**
   - ❌ **WRONG:** Email HTML hardcoded in constants: `EMAIL_BODY_TEMPLATE = "Process: {0}<br>..."` in `InfoConstants.cs` - **PROHIBITED**
   - ❌ **WRONG:** Email template doesn't match Boomi structure from subprocess JSON - **PROHIBITED**
   - ❌ **WRONG:** Email template placeholders don't match Boomi subprocess msgParameters numbering - **PROHIBITED**
   - ❌ **WRONG:** Placeholder population parameter order doesn't match Boomi subprocess msgParameters key order - **PROHIBITED**
   - **🔴 CRITICAL:** Email templates MUST be embedded HTML resources matching Boomi subprocess template exactly (see Section 17.4: Email Template Helpers)
   - **🔴 CRITICAL:** Email template HTML MUST be extracted EXACTLY from Boomi subprocess JSON file (subprocess_*.json) - locate in msgTxt section of message shapes
   - **🔴 CRITICAL:** Email template placeholders MUST match Boomi subprocess placeholders EXACTLY (verify placeholder numbering and order from msgParameters section in subprocess JSON)
   - **🔴 CRITICAL:** Placeholder population parameter order MUST match Boomi subprocess msgParameters key order (verify parameter order in string.Format matches msgParameters)
   - **🔴 CRITICAL:** Each process has different email templates - verify template name and structure from Phase 1 Section 9 or subprocess JSON
   - **🔴 CRITICAL:** This rule has been missed repeatedly - DO NOT SKIP THIS CHECKPOINT if Function has error handling with email notifications
   - **🚨 CRITICAL:** You MUST read the entire file systematically and verify embedded templates are used. Do NOT skip this step.

22. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Function dependency injection validation
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for constructor parameters and field declarations
   - [ ] **BLOCKING:** For EACH constructor parameter or field declaration found:
     - [ ] Is it in a comment? → **ALLOWED**
     - [ ] Is it a System/Process Abstraction (e.g., `SendNotificationMgmtSys`, `ILeaveMgmt`, `EntityProc`)? → **VIOLATION** → Function MUST NOT inject System/Process Abstractions directly → Document line number
     - [ ] Is it a Service (e.g., `<Entity>Service`, `NotificationService`)? → ✅ PASS
   - [ ] **BLOCKING:** If ANY violations found → STOP → Create Service (e.g., `<Entity>Service`, `NotificationService`) → Inject Service into Function → Remove System/Process Abstraction injection → Re-read file → Verify zero violations
   ...
   - ✅ **CORRECT:** `private readonly <Entity>Service _<entity>Service;` (Function injects Service)
   - ✅ **CORRECT:** `private readonly NotificationService _notificationService;` (Function injects Service for cross-cutting concerns)
   - ✅ **CORRECT:** `private readonly I<Entity>Service _<entity>Service;` (Function injects Service via interface)
   - ❌ **WRONG:** `private readonly <Entity>MgmtSys _<entity>MgmtSys;` (Function injects System Abstraction directly - **PROHIBITED**)
   - ❌ **WRONG:** `private readonly I<Entity>Mgmt _<entity>Mgmt;` (Function injects System Abstraction interface directly - **PROHIBITED**)
   - **🔴 CRITICAL:** Functions MUST NOT inject System/Process Abstractions directly - This violates the mandatory call flow (Functions → Services → System/Process Abstractions)
   - **🔴 CRITICAL:** Cross-cutting concerns (e.g., error notifications) MUST be handled via Services (e.g., `NotificationService`), NOT by injecting System Abstractions directly into Functions
   - **🚨 CRITICAL:** You MUST read the entire file systematically and verify Functions only inject Services. Do NOT skip this step.

**🛑 IF ANY VIOLATION → STOP → Fix Function → Then proceed**

### Azure Functions - Common Mistakes

**❌ Using "API" Keyword in Function Names**
- **🔴 CRITICAL: NEVER EVER use "API" keyword** - This is a MANDATORY rule with NO EXCEPTIONS
...
**❌ Using `var` Keyword in Functions**
- **🔴 CRITICAL: NEVER EVER use `var` keyword** - This is a MANDATORY rule with NO EXCEPTIONS
...
**❌ Using `internal` Keyword in Functions**
- **🔴 CRITICAL: NEVER EVER use `internal` keyword** - This is a MANDATORY rule with NO EXCEPTIONS
...
**❌ Null Coalescing After Validation**
...
**❌ Verbose Header Reading Pattern**
...
**❌ Wrong Function Folder Structure**
...
**❌ Using Route Attribute When Function Name is Sufficient**
- **🔴 CRITICAL:** When you don't need nested routes (e.g., `"<resource>/<operation>"`), DO NOT use Route attribute
- **WRONG:** `[Function("CreateEntity")]` with `Route = "CreateEntity"` (redundant - Function name already defines the route)
- **CORRECT:** `[Function("CreateEntity")]` without Route attribute (Azure Functions automatically uses Function name as route)
- **When to use Route:** Only when you need nested routes like `"<resource>/<operation>"` or route parameters like `"<resource>/{id}"`
- **When NOT to use Route:** When the route matches the Function name exactly (e.g., `[Function("CreateEntity")]` → route is automatically `CreateEntity`)
...
**❌ Manual Response Mapping in Function**
...
**🔴 CRITICAL: Process Layer AppConfigs URL Rules**

**Process Layer AppConfigs URLs MUST ONLY contain:**
- ✅ **System Layer Function URLs** - URLs pointing to System Layer Azure Functions (e.g., `SystemLayerUrl`, `NotificationSystemLayerUrl`)
- ✅ **Process Layer Function URLs** - URLs pointing to other Process Layer Azure Functions (e.g., `AuthProcessLayerUrl`, `DataProcessLayerUrl`)

**Process Layer AppConfigs URLs MUST NEVER contain:**
- ❌ **SOR (System of Record) URLs** - Direct URLs to external systems/APIs (e.g., `https://api.example.com`, `https://smtp.example.com`)
- ❌ **Downstream API URLs** - Direct URLs to third-party services
- ❌ **SOR base URLs** - Base URLs for external systems
- ❌ **SOR resource paths** - Resource paths for external systems
- ❌ **Any SOR URL construction components** - Components used to build SOR URLs
...
- ❌ **WRONG:** `SystemApiUrl`, `NotificationSmtpUrl`, `SORUrl` (these suggest SOR URLs)
- ✅ **CORRECT:** `SystemSystemLayerUrl`, `NotificationSystemLayerUrl` (these indicate System Layer Functions)

**Example:**
- ✅ **CORRECT:** `Create<Entity>Url = "https://<system-layer-function-url>/api/<operation>"` (System Layer Function URL)
- ❌ **WRONG:** `SORBaseUrl = "https://<sor-base-url>"`, `SORResourcePath = "/api/services/..."` (SOR URL construction - belongs in System Layer AppConfigs)

**Configuration Injection:**
- Configuration values MUST be injected via `IOptions<AppConfigs>` in Function/Service
- ❌ **NEVER read from HTTP headers** - Defined parameters are configuration, not request data
- ❌ **NEVER add to DTO** - Defined parameters are configuration, not request data

---

## 3. Domain Rules (Business Objects)

**🚨 CRITICAL:** Domains are ALWAYS required and ALWAYS populated. They encapsulate business entities with behavior and are MANDATORY for Process Layer operations.

### 3.1 Domain - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE ANY DOMAIN UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I read ALL sections in "DOMAIN RULES"?
2. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document to identify which entity properties are needed?
3. ✅ **STOP AND VERIFY:** Have I verified which properties are in the Boomi contract (Phase 1 document, Boomi JSON profile files)?
4. ✅ **STOP AND VERIFY:** Have I verified which properties are used in System Abstraction when building request to SOR?
5. ✅ **STOP AND VERIFY:** Do I understand that Domain is for internal purposes only (created in Function, populated by DTO, but NEVER passed to Services or System Abstractions)?
6. ✅ **STOP AND VERIFY:** Do I understand that Domain MUST NOT inject System/Process Abstractions or have methods that call external systems?
7. ✅ **STOP AND VERIFY:** Do I understand that `ICacheable` is NEED-BASED ONLY (only implement if caching is actually used with `[CacheableService]` attribute)?

**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete missing step → Then create Domain**

**🔴 MANDATORY WORKFLOW STEP 5: Domains**

**🚨 BEFORE creating Domains (BLOCKING - CANNOT PROCEED WITHOUT COMPLETING):**
...
- [ ] **BLOCKING:** Did I copy template exactly before writing code?
- [ ] **BLOCKING:** Did I read "🛑 PRE-CREATION VALIDATION" in Section 3?
- [ ] **BLOCKING:** Did I check "COMMON MISTAKES" #25 (Domain Object Passing Between Layers)?

**If you write code without reading Section 3 first → DELETE code → Read Section 3 → Retry**
...
public class <Entity>Domain : IDomain<int> {
    ...
    // ✅ ONLY simple business validation methods (need-based only)
...
**🔴 CRITICAL:** This section applies **ONLY** if you are actually using caching with `[CacheableService]` attribute. If caching is not needed, do NOT implement `ICacheable`.
...
**🔴 CRITICAL:** `ICacheable` is **NEED-BASED** - Only implement if you are actually using caching with `[CacheableService]` attribute. If caching is not needed, do NOT implement `ICacheable`.

### 3.5 Domain - Child Domain Objects - NEED-BASED ONLY

**🔴 CRITICAL: Only Create If Actually Required**

- ✅ **ONLY create child domain objects** if they are explicitly required by:
...
- ❌ **NEVER create child domain objects** "just in case" or for future use
- ❌ **NEVER create child domain objects** if a simple property (e.g., `string Attachment`) is sufficient
- ✅ **Verify necessity** - Before creating a child domain object, verify it's actually needed
...
```csharp
// ✅ ONLY create if System Layer API or business logic actually requires this structure
...
```csharp
// ❌ WRONG - Creating unnecessary child class when simple property is sufficient
public class <Entity> {
    public List<<Entity>Attachment> Attachments { get; set; }  // ❌ Unnecessary if attachment is just a string
}

// ✅ CORRECT - Use simple property if that's all that's needed
public class <Entity> {
    public string Attachment { get; set; } = string.Empty;  // ✅ Sufficient if System Layer just needs a string
}
```
...
**🔴 CRITICAL: Domain Registration Rules**
...
| **ICacheable** | ✅ **ONLY** if caching is actually used (with `[CacheableService]` attribute) | ❌ Not used | ❌ Not used |
| **Constructor DI** | ❌ **NEVER** inject dependencies | ✅ ILogger (if needed) | ❌ No constructor |
| **Methods** | ✅ **ONLY** simple business validation (need-based: IsRecurring, IsExpired, HasItems) | Rare | None |
| **System Calls** | ❌ **NEVER** call System/Process Abstractions | ❌ Not used | ❌ Not used |
| **Registration** | ❌ **NOT** registered (POCOs) | `AddScoped<<Entity>Details>()` | ❌ Not registered |
| **Id Property** | ✅ Required (IDomain<T>) | ✅ Optional | ❌ Rarely |
...
**🚨 WHAT DOMAINS SHOULD DO:**

- ✅ Hold data properties **ONLY if actually needed** - Properties must be:
...
- ✅ Implement business logic validation methods **ONLY WHEN NEEDED** (e.g., `IsRecurring()`, `IsExpired()`, `HasItems()`)
- ✅ Implement `IDomain<T>` if needed
- ✅ Implement `ICacheable` **ONLY** if caching is actually used (with `[CacheableService]` attribute) - **NEED-BASED, NOT MANDATORY**
- ✅ Be populated by DTOs via `dto.Populate(domain)`

**🔴 CRITICAL: Domain Property Verification**

- ✅ **ALWAYS verify domain properties against Boomi JSON profile files** - Check Phase 1 document (Step 1a) to see which fields are in the Boomi request
- ✅ **ALWAYS verify domain properties are used** - Check if property is:
  - Populated from DTO in `Populate()` method
  - Used in System Abstraction when building request to SOR
- ❌ **NEVER add properties that don't exist in Boomi contract** - If property is not in Boomi JSON profile, it should not be in domain
- ❌ **NEVER add properties "just in case"** - Only add properties that are actually needed and used

**❌ WHAT DOMAINS SHOULD NEVER DO:**

- ❌ **NEVER** inject System Abstractions or Process Abstractions
- ❌ **NEVER** have methods that call System Layer or Process Layer APIs
- ❌ **NEVER** contain orchestration logic
- ❌ **NEVER** directly call external systems
- ❌ **NEVER** duplicate DTO validation (checking if required fields are present)
...
**⚠️ NEED-BASED ONLY:** Domain validation methods are **OPTIONAL** and should **ONLY** be created when there's a business logic need. Do NOT create methods that just check if required fields are present - that's DTO validation territory.
...
public class <Entity>Domain : IDomain<int>, ICacheable {
    ...
    // ✅ GOOD: Business logic checks (need-based only)
    ...
    // ❌ WRONG: Duplicating DTO validation
    // public bool IsValid() => !string.IsNullOrEmpty(Code) && Items.Count > 0;  // ❌ This is DTO validation, not business logic
...
- ✅ `IsRecurring()` - Checks if entity is recurring based on `Details.Code` property (business rule)
- ✅ `IsExpired()` - Checks if token/entity expired based on `ExpiresAt` date (business state)
- ✅ `HasItems()` - Checks if entity has items for business processing (business condition)
- ❌ `IsValid()` checking if required fields are present - **This is DTO validation, NOT domain validation**

**❌ WRONG PATTERN (DEPRECATED - DO NOT USE):**

```csharp
// ❌ NEVER DO THIS - Domains should NOT call System Abstractions
public class <Entity>Domain : IDomain<int> {
    private readonly ILogger<<Entity>Domain> _logger;
    private readonly <Entity>Sys _entitySys;  // ❌ WRONG: No System Abstraction injection
    ...
    public async Task<HttpResponseMessage> CreateEntity() {  // ❌ WRONG: No API calls in domains
        HttpResponseMessage r = await _entitySys.CreateEntity(this);  // ❌ WRONG
        return r;
    }
}
```
...
- ✅ Services call System Abstractions
- ✅ Azure Functions orchestrate via Services
- ✅ DTOs are passed from Function to Service, then from Service to System Abstraction (NEVER pass Domain objects between layers)

### 3.9 Domain - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Domain, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Implements `IDomain<T>` (if main domain)
2. ✅ **STOP AND VERIFY:** Implements `ICacheable` **ONLY** if caching is actually used (with `[CacheableService]` attribute) - **NEED-BASED, NOT MANDATORY**
3. ✅ **STOP AND VERIFY:** **ALL domain properties are actually needed** - Verify each property is:
...
4. ✅ **STOP AND VERIFY:** NO properties that don't exist in Boomi contract or aren't used anywhere - **NEVER add properties "just in case"**
5. ✅ **STOP AND VERIFY:** NO constructor injection of System/Process Abstractions
6. ✅ **STOP AND VERIFY:** NO methods that call external systems
7. ✅ **STOP AND VERIFY:** ONLY simple business validation methods (need-based: IsRecurring, IsExpired, etc.) - NOT input validation
8. ✅ **STOP AND VERIFY:** NO implementation-specific comments in Domain properties
   - ❌ **WRONG:** `// From header: company-code` (implementation detail - DTO concern)
   - ❌ **WRONG:** `// Transformed from dateTime via PopulateDate` (implementation detail - Service concern)
   - ✅ **CORRECT:** No comments about data sources, transformations, or mapping logic
...
9. ✅ **STOP AND VERIFY:** Located correctly based on number of domains:
   ...
   - **🔴 CRITICAL:** Domain class name MUST be entity name (e.g., `<Entity>`), NOT operation name (e.g., `<Operation>Request`)
   - **🔴 CRITICAL:** Domain name MUST be generic business entity, NOT operation-specific
     - ✅ **CORRECT:** `<Entity>` (for `<Operation>` operation) - Generic business entity name
     - ❌ **WRONG:** `<Operation>` (operation-specific), `<Adjective><Operation>` (operation-specific)
     - **Rule:** If operation is `<Operation>` → Domain should be `<Entity>` (generic, reusable)
     - **Real-world example:** If operation is `LateLogin` → Domain should be `Attendance` (generic, reusable)
     - **Rule:** Domain name should represent WHAT the entity is, not HOW it's used in operations
     - **🔴 CRITICAL:** Ask: "What is the generic business entity?" NOT "What operation is this for?"
   - **🔴 CRITICAL:** Only create subfolder if MULTIPLE domains exist in that business domain
9. ✅ **STOP AND VERIFY:** NOT registered in Program.cs (POCOs, instantiated directly)
10. ✅ **STOP AND VERIFY:** Child domain objects (e.g., `<Entity>Attachment`, `<Entity>Child`) are **ONLY** created if actually required by System Layer API contract or business logic - **NEED-BASED, NOT MANDATORY**

**🛑 IF ANY VIOLATION → STOP → Fix Domain → Then proceed**

### 3.10 Domain - Common Mistakes

**❌ Domain Object Passing Between Layers**
...
**❌ Unnecessary ICacheable Interface**
...
**❌ Creating Unnecessary Child Domain Objects**
...
**❌ Adding Implementation-Specific Comments in Domain Classes**

- **WRONG:** 
  ```csharp
  public class Attendance
  {
      public string CompanyCode { get; set; } = string.Empty; // From header: company-code  // ❌ Implementation detail
      public string RequestDateTime { get; set; } = string.Empty; // Transformed from dateTime via PopulateDate  // ❌ Implementation detail
  }
  ```
- **CORRECT:**
  ```csharp
  public class Attendance
  {
      public string CompanyCode { get; set; } = string.Empty;  // ✅ No implementation comments
      public string RequestDateTime { get; set; } = string.Empty;  // ✅ No implementation comments
  }
  ```
- **Why:** 
...
- **🔴 CRITICAL:** Domain classes must be implementation-agnostic. No comments about data sources, transformations, or mapping logic.

**❌ Using Operation-Specific Names for Domain Classes**

- **WRONG:** 
  ```csharp
  public class <Operation>  // ❌ Operation-specific name
  {
...
- **CORRECT:**
  ```csharp
  public class <Entity>  // ✅ Generic business entity name
  {
      public string Property1 { get; set; }
      public string Property2 { get; set; }
      public string Property3 { get; set; }
...
- **Why:** 
  ...
  - **Examples:**
    - Operation: `<Operation>` → Domain: `<Entity>` (not `<Operation>`)
    - **Real-world example:** Operation: `LateLogin` → Domain: `Attendance` (not `LateLogin`)
- **🔴 CRITICAL:** When creating a domain, ask: "What is the generic business entity this represents?" NOT "What operation is this for?"
...
- **Why:** 
  ...
  - **Examples:**
    - Operation: `LateLogin` → Domain: `Attendance` (not `LateLogin`)
    - Operation: `CreateLeave` → Domain: `Leave` (not `CreateLeave`)
    - Operation: `GetEmployeeDetails` → Domain: `Employee` (not `GetEmployeeDetails`)
- **🔴 CRITICAL:** When creating a domain, ask: "What is the generic business entity this represents?" NOT "What operation is this for?"

**❌ Domain Methods Calling External Systems**
...
**❌ Duplicating DTO Validation in Domain**
...
**🚨 CRITICAL:** Use caching interceptor for System Layer API calls to improve performance. Caching is **NEED-BASED ONLY** - only implement if you actually need to cache System Layer responses.

### 4.1 Caching - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT IMPLEMENT CACHING UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I verified that caching is actually needed for this System Layer call?
2. ✅ **STOP AND VERIFY:** Do I understand that caching is NEED-BASED ONLY (only implement if actually needed)?
3. ✅ **STOP AND VERIFY:** Have I read ALL sections in "CACHING RULES"?
4. ✅ **STOP AND VERIFY:** Do I understand that System Abstraction method MUST return `HttpResponseSnapshot` (not `HttpResponseMessage`) for caching to work?
5. ✅ **STOP AND VERIFY:** Do I understand that Domain MUST implement `ICacheable` interface with `GetCacheKey()` method (ONLY if caching is used)?
6. ✅ **STOP AND VERIFY:** Do I understand that method MUST be decorated with `[CacheableService(Namespace = nameof(MethodName))]` attribute (ONLY if caching is used)?
7. ✅ **STOP AND VERIFY:** Have I registered System Abstraction with `AddCachedService` (not `AddScoped`) in Program.cs?

**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete missing step → Then implement caching**

**🔴 CRITICAL:** `ICacheable` and `[CacheableService]` are **NEED-BASED** - Only implement if you are actually using caching. If caching is not needed, do NOT implement `ICacheable` or use `[CacheableService]` attribute.
...
public class <Entity>MgmtSys : I<Entity>Mgmt {
    ...
    public async Task<HttpResponseSnapshot> GetDetails(<Entity>Info entity, string type) {
        // ✅ CORRECT - Method returns HttpResponseSnapshot for caching
...
4. If cache hit → Returns cached `HttpResponseSnapshot`
5. If cache miss → Calls System Layer, caches response, returns result
...
1. ✅ Domain MUST implement `ICacheable` interface
2. ✅ MUST implement `GetCacheKey()` method
3. ✅ `GetCacheKey()` MUST return `IDictionary<PropertyInfo, object>`
4. ✅ Cache key MUST be unique for each distinct request
5. ✅ List properties MUST be sorted before adding to cache key (for consistency)
...
1. ✅ Attribute MUST be placed on System Abstraction method
2. ✅ `Namespace` parameter MUST use `nameof(MethodName)` (method name)
3. ✅ Method MUST return `HttpResponseSnapshot` (not `HttpResponseMessage`)
4. ✅ Method parameter MUST be Domain that implements `ICacheable`
5. ✅ System Abstraction MUST be registered with `AddCachedService` (not `AddScoped`)
...
**🔴 CRITICAL:** This pattern is ONLY for Central Data Layer (Shared Services). Standard Process Layer uses caching interceptor pattern (Section 4.2).
...
- `TTLsInMinutes`: Method-specific TTL configuration (method name → TTL in minutes)

### 4.7 Caching - DI Registration

**🔴 CRITICAL:** System Abstractions with caching MUST be registered with `AddCachedService` (not `AddScoped`).
...
1. ✅ Caching is **NEED-BASED ONLY** - Only implement if actually needed
2. ✅ System Abstraction method MUST return `HttpResponseSnapshot` for caching
3. ✅ Domain MUST implement `ICacheable` with `GetCacheKey()` method
4. ✅ Method MUST be decorated with `[CacheableService(Namespace = nameof(MethodName))]`
5. ✅ System Abstraction MUST be registered with `AddCachedService` (not `AddScoped`)
6. ✅ Redis Cache Library MUST be registered before System Abstractions
7. ❌ **NEVER** implement `ICacheable` "just in case" - only if caching is actually used
8. ❌ **NEVER** use `[CacheableService]` without `ICacheable` implementation
9. ❌ **NEVER** register cached service with `AddScoped` - use `AddCachedService`

### 4.9 Caching - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER implementing caching, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Domain implements `ICacheable` interface (ONLY if caching is used)
2. ✅ **STOP AND VERIFY:** `GetCacheKey()` method is implemented correctly
3. ✅ **STOP AND VERIFY:** List properties are sorted before adding to cache key (for consistency)
4. ✅ **STOP AND VERIFY:** System Abstraction method returns `HttpResponseSnapshot` (not `HttpResponseMessage`)
5. ✅ **STOP AND VERIFY:** Method is decorated with `[CacheableService(Namespace = nameof(MethodName))]`
6. ✅ **STOP AND VERIFY:** System Abstraction is registered with `AddCachedService` (not `AddScoped`)
7. ✅ **STOP AND VERIFY:** Redis Cache Library is registered in Program.cs
8. ✅ **STOP AND VERIFY:** Cache settings (TTL) are configured in appsettings.json (if needed)

**🛑 IF ANY VIOLATION → STOP → Fix caching implementation → Then proceed**
...
**❌ Using CacheableService Without ICacheable**
...
**❌ Missing Redis Cache Library Registration**
...
**❌ Unsorted List Properties in Cache Key**
...
**🚨 CRITICAL:** System Abstractions handle Process→System Layer communication via HTTP calls. They are MANDATORY for all Process Layer implementations that call System Layer Functions.

### 5.1 System Abstractions - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE System Abstraction UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I identified which System Layer Function to call?
2. ✅ **STOP AND VERIFY:** Do I have the System Layer Function URL in AppConfigs?
3. ✅ **STOP AND VERIFY:** Have I verified I'm NOT adding SOR base URLs or resource paths to Process Layer AppConfigs? (SOR URL construction belongs in System Layer AppConfigs)
4. ✅ **STOP AND VERIFY:** Have I read this section completely?
5. ✅ **STOP AND VERIFY:** Do I know System Abstractions make HTTP calls (NOT project references)?
6. ✅ **STOP AND VERIFY:** Do I know to use `SendProcessHTTPReqAsync()` extension method for HTTP calls?
7. ✅ **STOP AND VERIFY:** Have I checked date/datetime field formats from Boomi JSON profile files (System Layer output profile) to ensure dates are formatted correctly for SOR API?
8. ✅ **STOP AND VERIFY:** Do I understand the System Layer Function's response structure (BaseResponseDTO format, data property structure)?
9. ✅ **STOP AND VERIFY:** Do I know how to map System Layer response to Process Layer response DTO (for ResponseDTOHelper)?
10. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document (Section 2: Process Properties Analysis) for HTTP headers and request parameters that the Boomi process reads?
11. ✅ **STOP AND VERIFY:** Have I verified the System Layer DTO validation methods (`ValidateAPIRequestParameters()`, `ValidateDownStreamRequestParameters()`) to see which fields are REQUIRED?
12. ✅ **STOP AND VERIFY:** Have I verified that ALL required System Layer DTO fields can be populated from available sources (Process Layer DTO, HTTP headers passed from Function, query parameters passed from Function, or config)?
13. ✅ **STOP AND VERIFY:** Have I verified the correct source for each System Layer DTO field from Phase 1 document (NOT assuming one field can be populated from another field)?
14. ✅ **STOP AND VERIFY:** Have I read System Layer Function's request DTO file to verify property names and required fields?
15. ✅ **🔴 CRITICAL:** Do I understand that dynamic request property names MUST match System Layer Function's request DTO property names EXACTLY?
16. ✅ **🔴 CRITICAL:** Do I understand that all required fields from System Layer Function's request DTO validation MUST be included in dynamic request?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create System Abstraction**

**🔴 MANDATORY WORKFLOW STEP 6: System Abstractions**

**🚨 BEFORE creating System Abstractions (BLOCKING - CANNOT PROCEED WITHOUT COMPLETING):**
...
- [ ] **BLOCKING:** Did I copy template exactly before writing code?
- [ ] **BLOCKING:** Did I read "🛑 PRE-CREATION VALIDATION" in Section 5?
- [ ] **BLOCKING:** Did I verify orchestration requirements from Phase 1 document?

**If you write code without reading Section 5 first → DELETE code → Read Section 5 → Retry**
...
**🔴 CRITICAL: PROCESS ORCHESTRATION VERIFICATION (MANDATORY)**

**🚨 BEFORE implementing Process Layer code, you MUST verify orchestration requirements:**

1. ✅ **Check Phase 1 document (Section 13: Process and System Layer Orchestration Diagram)** to identify ALL System Layer calls:
...
3. ✅ **Check Phase 1 document (Section 9: Subprocess Analysis)** to identify subprocess calls:
   - Are there subprocesses that translate to System Layer calls?
   - Do subprocesses need to be orchestrated in Process Layer?

4. ✅ **Verify orchestration pattern:**
   - **Single System Layer call** → Service directly calls System Abstraction (no orchestration needed)
   - **Multiple System Layer calls (business logic)** → Service MUST orchestrate multiple System Abstraction calls (see Section 8.4: Services Rules - Handling Multiple Failures)
   - **Error handling System Layer calls (cross-cutting concerns)** → Function ONLY MUST orchestrate error handling System Layer calls (Services MUST NOT orchestrate cross-cutting concerns - see Section 8.2: Service Separation Rule)

5. ✅ **🔴 CRITICAL: NEVER miss error handling orchestration:**
   - If Phase 1 shows System Layer call on error → Function MUST call that System Layer System Abstraction in error path (using `else` block, NOT try-catch)
   - If Phase 1 shows multiple error handling steps → Function MUST orchestrate all error handling System Layer calls
   - **Source of truth:** Phase 1 document (Section 13: Orchestration Diagram) shows error handling System Layer calls
   - **🚨 MANDATORY:** Review Section 2.7 (Azure Functions - Error Handling & Orchestration) for detailed error handling checklist
   - **🔴 CRITICAL:** Error handling orchestration is Function responsibility ONLY - Services MUST NOT orchestrate cross-cutting concerns like error notifications (violates Service Separation Rule - Section 8.2)

6. ✅ **🔴 CRITICAL: Verify ALL properties used in error handling subprocesses (Phase 1 Section 9):**
   ...
   - ✅ **CORRECT:** Error path extracts request payload: `string attachmentContent = JsonSerializer.Serialize(<RequestDTO>);`
   - ✅ **CORRECT:** Error path constructs filename: `string attachmentFileName = $"{_options.<FileNameProperty>}{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ss.fffZ}.txt";`
   - ❌ **WRONG:** Error path passes `attachmentFileName: null, attachmentContent: null` when Phase 1 Section 9 shows these properties are required
   - ❌ **WRONG:** Error path doesn't extract request payload when Phase 1 Section 9 shows `process.DPP_<PayloadProperty>` is used
   - **🔴 CRITICAL:** Phase 1 Section 9 (Subprocess Analysis) is the source of truth for ALL properties used in error handling subprocesses
   - **🔴 CRITICAL:** ALL properties listed in Phase 1 Section 9 "Properties Read" table MUST be populated and passed correctly
   - **🔴 CRITICAL:** Never use try catch while generating code

**Example Orchestration Verification:**
```
Phase 1 Section 13 shows:
  - Success path: <SOR> API call (1 System Layer call) → ✅ Single call, no orchestration needed
  - Error path: <ErrorHandlingSOR> API call (1 System Layer call) → ✅ Error handling orchestration required

Current implementation:
  - Service only calls <SOR> → ❌ MISSING: <ErrorHandlingSOR> call on error
  - Fix: Add error handling System Layer orchestration in Function catch block or Service error handling
```

**🔴 CRITICAL: System Layer DTO Field Source Verification**

**🚨 BEFORE creating System Abstraction, you MUST verify field sources:**

1. ✅ **🔴 MANDATORY: Read System Layer DTO file FIRST** - You CANNOT proceed without this:
...
2. ✅ **Check Phase 1 document (Section 2: Process Properties Analysis)** to identify data sources:
   - **🔴 CRITICAL: Check "Source Type" field in Phase 1 Section 2 for EACH property:**
     - **`Source Type: trackparameter`** → Property comes from HTTP request (header, query parameter, or request body)
       - If property name contains `inheader_` → HTTP header (read in Function from `req.Headers`), design it to read it from headers, create it as a part of DTO and pass it to system layer in request DTO
       - If property name contains `inquery_` → Query parameter (read in Function from `req.Query`), design it to read it from query, create it as a part of DTO and pass it to system layer in request DTO
       - If property name contains `indocument_` → Request body field (in DTO)
     - **`Source Type: definedparameter`** → Property comes from Boomi defined parameter (configuration)
       - **🔴 CRITICAL FIRST CHECK: Is this used in a decision shape?**
         ...
         - **If used in decision shape** → ✅ **Code Logic (NOT AppConfigs)** - Determine via code logic based on actual data (e.g., `bool hasAttachment = !string.IsNullOrWhiteSpace(attachmentFileName) && !string.IsNullOrWhiteSpace(attachmentContent);`)
         ...
         - **If NOT used in decision shape** → Continue to next check
       - **🔴 CRITICAL: Determine which layer needs this configuration:**
         ...
         - **If used to construct SOR (external system) URLs** → ❌ **DO NOT add to Process Layer AppConfigs** - This belongs in System Layer AppConfigs
           - **Example:** `<ResourcePath>` used to build `<SOR>` API URL → System Layer constructs `<SOR>` URL internally → `<ResourcePath>` belongs in System Layer AppConfigs, NOT Process Layer
         ...
         - **If used for Process Layer configuration** → ✅ Add to Process Layer AppConfigs
       ...
       - **🔴 CRITICAL RULE - URL CONSTRUCTION SEPARATION:**
         - ✅ **Process Layer AppConfigs:** ONLY System Layer Function URLs (complete Azure Function endpoints)
         - ❌ **Process Layer AppConfigs:** NEVER SOR API URLs, SOR base URLs, SOR resource paths, or any SOR URL construction components
         - ✅ **System Layer AppConfigs:** SOR base URLs, SOR resource paths, SOR API construction details
       ...
       - ❌ **NEVER read from HTTP headers** - Defined parameters are configuration, not request data
       - ❌ **NEVER add to DTO** - Defined parameters are configuration, not request data
     - **`Source Type: processparameter`** → Property comes from process property (runtime, not config)
   - HTTP headers that Boomi process reads (e.g., `<header-name>` header → `process.DPP_<property-name>`)
   - Request parameters that Boomi process reads (e.g., `<parameter-name>` parameter → `dynamicdocument.<property-name>`)
   - How these headers/parameters are used in the Boomi process flow
   - **🔴 MANDATORY CHECKLIST for each property in Phase 1 Section 2:**
     1. Read the "Source Type" field
     2. If "definedparameter" → **STOP:** 
        - **FIRST:** Check Phase 1 Section 3 (Decision Shape Analysis) or subprocess JSON - Is this property used in a decision shape?
          - **If YES** → ✅ **Code Logic (NOT AppConfigs)** - Determine via code logic based on actual data
          - **If NO** → Continue to next step
        - **SECOND:** Check which layer needs it (Process Layer or System Layer)
          - If used for SOR URL construction → System Layer AppConfigs (NOT Process Layer)
          - If used for Process Layer configuration → Process Layer AppConfigs
     3. If "trackparameter" with "inheader_" → Read from req.Headers in Function, create it as a part of DTO and pass it to system layer in request DTO
     4. If "trackparameter" with "inquery_" → Read from req.Query in Function, create it as a part of DTO and pass it to system layer in request DTO
     5. If "trackparameter" with "indocument_" → Add to DTO request body structure

3. ✅ **Verify field mapping sources** - For each REQUIRED System Layer DTO field, verify the source:
   - **Request body fields** → Map from Process Layer DTO to System Layer DTO
   - **HTTP headers** → Read from `HttpRequest.Headers` in Function, create it as a part of DTO and pass it to system layer in request DTO
   - **Query parameters** → Read from `HttpRequest.Query` in Function, create it as a part of DTO and pass it to system layer in request DTO
   - **Config values** → Use from `AppConfigs` in System Abstraction

4. ✅ **🔴 CRITICAL: NEVER assume field mapping** - Always verify the correct source:
   - ❌ **WRONG:** Using `dto.<SomeField>` for `<AnotherField>` without verifying from Phase 1
   - ✅ **CORRECT:** Reading `<header-name>` header in Function, passing to System Abstraction, mapping to `<FieldName>` in System Layer DTO
...
Phase 1 Section 2 shows: Boomi process reads "<header-name>" header → process.DPP_<property-name>
Solution: Function reads req.Headers["<header-name>"], passes to System Abstraction, maps to <FieldName>
```

**🔴 CRITICAL:** Before implementing Process Layer response mapping, you MUST:
...
**🔴 MANDATORY ARCHITECTURE RULE - NEVER VIOLATE:**

- ✅ **Process Layer calls System Layer via HTTP** (Azure Function to Azure Function)
- ❌ **NEVER add project references to System Layer projects** in Process Layer .csproj
- ❌ **NEVER register System Layer services/handlers** in Process Layer Program.cs
- ❌ **NEVER inject System Layer services directly** in Process Layer
- ✅ **ONLY register System Abstractions** that make HTTP calls to System Layer Functions
- **🔴 CRITICAL: URL CONSTRUCTION SEPARATION:**
  - ✅ **Process Layer System Abstraction:** Calls System Layer Function URL (complete Azure Function endpoint from Process Layer AppConfigs)
  - ❌ **Process Layer System Abstraction:** NEVER constructs SOR API URLs, NEVER uses SOR base URLs or resource paths
  - ✅ **System Layer:** Constructs SOR API URLs internally using System Layer AppConfigs (base URL + resource path)
  - **Example:**
    - ✅ **Process Layer AppConfigs:** `<Operation>Url = "https://<system-layer-function-url>/api/<operation>"` (System Layer Function URL)
    - ❌ **Process Layer AppConfigs:** `<SOR>BaseUrl`, `<SOR>ResourcePath` (SOR URL construction - belongs in System Layer AppConfigs)
    - ✅ **System Layer AppConfigs:** `<SOR>BaseUrl = "https://<sor-base-url>"`, `<SOR>ResourcePath = "/api/services/..."` (SOR URL construction)

**❌ WRONG PATTERN (DO NOT USE):**

```csharp
// ❌ WRONG - Process Layer .csproj
<ProjectReference Include="..\SystemLayerProject\SystemLayerProject.csproj" />

// ❌ WRONG - Process Layer Program.cs
...
// ❌ WRONG - System Abstraction calling System Layer service directly
public class <Entity>MgmtSys : I<Entity>Mgmt {
    private readonly ISystemLayerService _systemLayerService;  // ❌ Direct service injection
    public async Task<HttpResponseSnapshot> CreateEntity(...) {
        return await _systemLayerService.CreateEntity(...);  // ❌ Direct call
    }
}
```
...
```csharp
// ✅ CORRECT - Process Layer .csproj (NO System Layer references)
<ItemGroup>
  <ProjectReference Include="..\Framework\Core\Core\Core.csproj" />
  <!-- ❌ NO System Layer project references -->
</ItemGroup>

// ✅ CORRECT - Process Layer Program.cs (ONLY System Abstractions)
services.AddScoped<I<Entity>Mgmt, <Entity>MgmtSys>();  // System Abstraction only

// ✅ CORRECT - System Abstraction making HTTP call to System Layer Function
public class <Entity>MgmtSys : I<Entity>Mgmt {
    private readonly AppConfigs _options;
    private readonly CustomHTTPClient _customHttpClient;  // ✅ HTTP client
    ...
    public async Task<HttpResponseMessage> CreateEntity(<Entity>Domain domain) {
        string url = _options.CreateEntityUrl;  // ✅ System Layer Function URL from config
        // ✅ Direct call - SendProcessHTTPReqAsync automatically adds TestRunId/RequestId headers
        ...
        HttpResponseMessage response = await _customHttpClient.SendProcessHTTPReqAsync(
            ...
            body: request, 
            reqHeaders: null);
        return response;  // ✅ HTTP call to System Layer Function
    }
}
```
...
**🔴 CRITICAL: System Abstraction Class Separation Rule (MANDATORY - NEVER VIOLATE)**

**🚨 System Abstraction classes MUST be separated by business domain/entity, NOT by SOR (System of Record):**

- ✅ **CORRECT:** Each business domain/entity has its own System Abstraction class
  - `<Entity1>MgmtSys` → Contains ONLY `<Entity1>`-related operations (e.g., `Create<Entity1>`, `Get<Entity1>Details`)
  - `<Entity2>MgmtSys` → Contains ONLY `<Entity2>`-related operations (e.g., `Create<Entity2>`, `Update<Entity2>`)
  - Both can be in the same SOR directory (`SystemAbstractions/<SOR>Mgmt/`) since they connect to the same SOR
- ❌ **WRONG:** Mixing different business domains/entities in the same System Abstraction class
...
**Rule:**
- ✅ **One System Abstraction class per business domain/entity** (e.g., `<Entity1>MgmtSys`, `<Entity2>MgmtSys`, `<Entity3>MgmtSys`)
- ✅ **Multiple System Abstraction classes can exist in the same SOR directory** (e.g., `SystemAbstractions/<SOR>Mgmt/<Entity1>MgmtSys.cs` and `SystemAbstractions/<SOR>Mgmt/<Entity2>MgmtSys.cs`)
- ✅ **System Abstraction class name reflects the business domain/entity** (e.g., `<Entity2>MgmtSys` for `<Entity2>` operations, NOT `<Entity1>MgmtSys`)
- ❌ **NEVER add operations from different business domains/entities to the same System Abstraction class**

**Examples:**
- ✅ **CORRECT:** `<Entity1>MgmtSys` contains `Create<Entity1>()`, `Get<Entity1>Details()` (both `<Entity1>` operations)
- ✅ **CORRECT:** `<Entity2>MgmtSys` contains `Create<Entity2>()` (`<Entity2>` operation)
- ✅ **CORRECT:** Both `<Entity1>MgmtSys.cs` and `<Entity2>MgmtSys.cs` in `SystemAbstractions/<SOR>Mgmt/` directory
- ❌ **WRONG:** `<Entity1>MgmtSys` containing `<Entity2>` operations (different business domain/entity)
- ❌ **WRONG:** `<SOR>MgmtSys` containing all `<SOR>` operations (mixing business domains)
...
public class <Entity>MgmtSys : I<Entity>Mgmt {
    private readonly AppConfigs _options;
    private readonly CustomHTTPClient _customHttpClient;  // ✅ HTTP client for System Layer calls
    private readonly ILogger<<Entity>MgmtSys> _logger;
    ...
    public async Task<HttpResponseMessage> CreateEntity(<Entity>Domain domain) {
        ...
        // ✅ Direct call to SendProcessHTTPReqAsync - no helper methods needed
        // Extension method automatically adds TestRunId/RequestId headers and handles JSON serialization
        string url = _options.CreateEntityUrl;  // ✅ System Layer Function URL from AppConfigs
...
- ✅ MUST inject `CustomHTTPClient` for HTTP calls
- ✅ MUST inject `IOptions<AppConfigs>` for System Layer Function URLs
- ✅ MUST use `SendProcessHTTPReqAsync()` extension method
- ✅ MUST use `await` (NEVER `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
- ✅ Builds dynamic request using `ExpandoObject`
- ✅ Calls System Layer Function URL (not SOR URL)
- ✅ Logs at start and end of method (simple logging)
- ✅ Returns `HttpResponseMessage` directly
- ✅ **🔴 CRITICAL:** One System Abstraction class per business domain/entity (e.g., `<Entity1>MgmtSys` for `<Entity1>`, `<Entity2>MgmtSys` for `<Entity2>`)
- ❌ **NEVER checks response status codes** (Service/Function responsibility)
- ❌ **NEVER extracts error details from response** (Service/Function responsibility)
- ❌ **NEVER logs based on response status** (Service/Function responsibility)
- ❌ **NEVER mix different business domains/entities in the same System Abstraction class**

**🔴 CRITICAL: Request Payload Alignment Rule (MANDATORY - NEVER VIOLATE)**

**🚨 System Abstraction dynamic request MUST align EXACTLY with System Layer Function's request DTO structure:**
- ✅ **MUST match property names** - Dynamic request property names MUST match System Layer Function's request DTO property names exactly
- ✅ **MUST include all required fields** - All required fields from System Layer Function's request DTO validation MUST be included
- ✅ **MUST verify field mapping** - Before creating dynamic request, MUST read System Layer Function's request DTO to verify property names and required fields
- ✅ **MUST verify from System Layer repository** - Source of truth is System Layer Function's request DTO file (e.g., `<Operation>ReqDTO.cs`)
- ✅ **MUST open and read System Layer DTO file** - Cannot validate without opening the actual System Layer DTO file
- ✅ **MUST compare every property** - For each property in System Layer DTO, verify it exists in dynamic request with exact same name
- ✅ **MUST compare every dynamic property** - For each property in dynamic request, verify it exists in System Layer DTO with exact same name
- ❌ **NEVER assume property names** - Property names in dynamic request MUST match System Layer DTO exactly
- ❌ **NEVER skip required fields** - All required fields from System Layer DTO validation MUST be populated
- ❌ **NEVER use different property names** - Cannot use Process Layer DTO property names if they differ from System Layer DTO property names
...
// ✅ CORRECT: Dynamic request matches System Layer DTO property names exactly
...
// ❌ WRONG: Using Process Layer DTO property names that don't match System Layer DTO
...
**🚨 CACHING INTERCEPTOR:** Use `[CacheableService]` attribute to enable caching for System Layer responses.
...
public class <Entity>MgmtSys : I<Entity>Mgmt {
    ...
    public async Task<HttpResponseSnapshot> GetDetails(<Entity>Info entity, string type) {
        ...
        // ✅ Direct call - use await, NEVER use .Result
        HttpResponseMessage r = await _customHttpClient.SendProcessHTTPReqAsync(
            method: HttpMethod.Post, 
            url: url, 
            contentType: "application/json", 
...
**🔴 CROSS-REFERENCE:** See Section 4 (Caching Rules) for complete caching pattern.

### 5.5 System Abstractions - HTTP Call Method (SendProcessHTTPReqAsync)

**🔴 MANDATORY:** Use `SendProcessHTTPReqAsync()` extension method for ALL Process→System Layer calls (System Abstractions) and ALL Process→Process Layer calls (Process Abstractions).

**Where it's used:**
- ✅ **System Abstractions** - Process→System Layer calls (Section 5)
- ✅ **Process Abstractions** - Process→Process Layer calls (Section 6)
...
**Key Features:**
- ✅ **Automatically adds technical headers:** TestRunId and RequestId are automatically added by the extension method
- ✅ **JSON serialization:** Automatically serializes body to JSON if provided
- ✅ **Content-Type handling:** Defaults to "application/json" but can be customized
- ✅ **Optional headers:** Can pass additional headers via `reqHeaders` parameter

**Usage Pattern:**

```csharp
// ✅ CORRECT - Direct call with await (technical headers added automatically, no helper methods needed)
// Used in: System Abstractions (Process→System) and Process Abstractions (Process→Process)
public async Task<HttpResponseMessage> CreateEntity(<Entity>Domain domain) {
    dynamic req = new ExpandoObject();
    req.PropertyA = domain.PropertyA;
    req.PropertyB = domain.PropertyB;

    // ✅ CORRECT - Use await, method must be async
    HttpResponseMessage response = await _customHttpClient.SendProcessHTTPReqAsync(
        method: HttpMethod.Post, 
        url: url, 
        contentType: "application/json", 
...
// ✅ CORRECT - With additional business headers
...
// ❌ WRONG - Using .Result causes deadlocks and thread blocking
public Task<HttpResponseMessage> CreateEntity(<Entity>Domain domain) {
    HttpResponseMessage response = _customHttpClient.SendProcessHTTPReqAsync(...).Result;  // ❌ PROHIBITED
    return Task.FromResult(response);
}

// ❌ WRONG - Blocking async call
public HttpResponseMessage CreateEntity(<Entity>Domain domain) {
    HttpResponseMessage response = _customHttpClient.SendProcessHTTPReqAsync(...).GetAwaiter().GetResult();  // ❌ PROHIBITED
    return response;
}

// ❌ WRONG - Don't manually add TestRunId/RequestId (extension method does this)
List<Tuple<string, string>> h = new List<Tuple<string, string>> 
{ 
    RequestHeaders.TestRunId.Simplify(),  // ❌ Redundant - extension method adds this
    RequestHeaders.RequestId.Simplify()   // ❌ Redundant - extension method adds this
};
```

**📘 NOTE:** `.Simplify()` is a Framework extension function (`Core/Extensions/TupleExtension.cs`) that converts `Tuple<string, AsyncLocal<StringBuilder>>` to `Tuple<string, string>` by extracting the string value from AsyncLocal. It's used internally by the extension method to prepare RequestHeaders.

### 5.6 System Abstractions - Date/DateTime Format Handling

**🔴 CRITICAL: Date/DateTime Format Requirements**

**MANDATORY CHECK:**
- ✅ **ALWAYS check Boomi JSON profile files** for date/datetime field formats before implementing System Abstraction
- ✅ **Check the System Layer output profile** (the profile that maps to SOR API) for `ProfileDateFormat` with `dateFormat` attribute
- ✅ **Document date format in Phase 1** if not already documented (e.g., `"yyyy-MM-dd"`, `"yyyy-MM-dd'T'HH:mm:ss"`, `"yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"`)
...
3. Look for `DataFormat` → `ProfileDateFormat` → `dateFormat` attribute
4. Example: `"dateFormat": "yyyy-MM-dd"` means dates must be formatted as `"2024-01-15"`

**System Abstraction Implementation:**
- ✅ **Format dates according to SOR API requirements** before sending to System Layer
- ✅ **Use the format specified in Boomi JSON profile** (found in `ProfileDateFormat.dateFormat`)
- ✅ **Document the format in Phase 1** if not already documented
...
dynamic dynamicReq = new ExpandoObject();
dynamicReq.StartDate = DateTime.Parse(dto.StartDate).ToString("yyyy-MM-dd");  // ✅ Format according to SOR requirement
dynamicReq.EndDate = DateTime.Parse(dto.EndDate).ToString("yyyy-MM-dd");      // ✅ Format according to SOR requirement
```
...
**🚨 CRITICAL:** System Abstractions MUST implement interfaces and be injected via interface in Services.

| Component | Pattern | Usage |
|-----------|---------|-------|
| **Interface** | `public interface I<Entity>Mgmt` | ✅ MANDATORY - All System Abstractions must have interfaces |
| **Implementation** | `public class <Entity>MgmtSys : I<Entity>Mgmt` | ✅ MANDATORY - Must implement interface |
| **Injection in Service** | `private readonly I<Entity>Mgmt _entityMgmt;` | ✅ MANDATORY - Inject via interface |
| **Registration** | `builder.Services.AddScoped<I<Entity>Mgmt, <Entity>MgmtSys>();` | ✅ MANDATORY - Register with interface |
...
| **SOR URL Construction** | ❌ **NEVER** - System Layer constructs SOR URLs internally | Process Layer does NOT construct SOR URLs |

**🔴 CRITICAL RULES:**

1. ✅ System Abstractions MUST implement interfaces
2. ✅ System Abstractions MUST be registered with interface in Program.cs
3. ✅ System Abstractions MUST use `SendProcessHTTPReqAsync()` extension method
4. ✅ System Abstractions MUST use `await` (NEVER `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
5. ✅ System Abstractions MUST call System Layer Function URL (not SOR URL)
6. ✅ **🔴 CRITICAL:** Dynamic request property names MUST match System Layer Function's request DTO property names EXACTLY (MUST verify from System Layer repository)
7. ✅ **🔴 CRITICAL:** All required fields from System Layer Function's request DTO validation MUST be included in dynamic request
8. ✅ **🔴 CRITICAL:** System Abstractions do NOT check response status codes (Service/Function responsibility)
9. ✅ **🔴 CRITICAL:** System Abstractions do NOT extract error details from response (Service/Function responsibility)
10. ✅ **🔴 CRITICAL:** System Abstractions do NOT log based on response status (Service/Function responsibility)
11. ✅ System Abstractions log at start and end of method (simple logging only)
12. ✅ System Abstractions return `HttpResponseMessage` directly (no status checking or error extraction)
6. ✅ Process Layer AppConfigs contains ONLY System Layer Function URLs (not SOR URLs)
7. ❌ **NEVER** add project references to System Layer projects
8. ❌ **NEVER** register System Layer services/handlers in Process Layer
9. ❌ **NEVER** inject System Layer services directly
10. ❌ **NEVER** construct SOR API URLs in Process Layer
11. ❌ **NEVER** use SOR base URLs or resource paths in Process Layer AppConfigs

### 5.8 System Abstractions - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating System Abstraction, you MUST verify (BLOCKING - CANNOT SAVE FILE UNTIL ALL PASS):**

**🔴 MANDATORY VIOLATION CHECKS (RUN BEFORE SAVING FILE):**
- [ ] **BLOCKING:** Run `grep -n "<SOR>SystemBaseUrl\|<SOR>BaseUrl\|SORBaseUrl\|ResourcePath" <file>` → If found → **VIOLATION** → System Abstraction must NOT construct SOR URLs → Fix → Retry
- [ ] **BLOCKING:** Run `grep -n "\\.Result\\|\\.Wait()\\|\\.GetAwaiter()" <file>` → If found → **VIOLATION** → Must use `await` → Fix → Retry
...
- [ ] **BLOCKING:** Read ALL property definitions (lines with `public` properties) → Document all property names
- [ ] **BLOCKING:** For EACH property in System Layer DTO: Check if it exists in dynamic request with EXACT same property name (case-sensitive) → If mismatch → **VIOLATION** → Fix → Retry
- [ ] **BLOCKING:** For EACH property in dynamic request: Check if it exists in System Layer DTO with EXACT same property name (case-sensitive) → If mismatch → **VIOLATION** → Fix → Retry
...
- [ ] **BLOCKING:** Identify ALL required fields (fields that throw validation errors if null/empty) → Document all required fields
- [ ] **BLOCKING:** For EACH required field: Verify it is included in dynamic request → If missing → **VIOLATION** → Fix → Retry
- [ ] **BLOCKING:** For EACH required field: Verify it is populated (not null/empty) or has a valid source → If not populated → **VIOLATION** → Fix → Retry

1. ✅ **STOP AND VERIFY:** System Abstraction calls System Layer Function URL from Process Layer AppConfigs (complete Azure Function endpoint)
2. ✅ **🔴 CRITICAL:** System Abstraction does NOT construct SOR API URLs, does NOT use SOR base URLs or resource paths
3. ✅ **🔴 CRITICAL:** Process Layer AppConfigs contains ONLY System Layer Function URLs, NOT SOR base URLs or resource paths
4. ✅ **STOP AND VERIFY:** System Abstraction uses `SendProcessHTTPReqAsync()` extension method (NOT direct HTTP calls)
5. ✅ **STOP AND VERIFY:** System Abstraction uses `await` (NOT `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
6. ✅ **STOP AND VERIFY:** System Abstraction implements interface and is registered with interface in Program.cs
7. ✅ **STOP AND VERIFY:** Date/datetime fields are formatted according to Boomi JSON profile requirements
8. ✅ **STOP AND VERIFY:** All required System Layer DTO fields are populated from correct sources (verified from Phase 1)
9. ✅ **🔴 CRITICAL:** Dynamic request property names match System Layer Function's request DTO property names EXACTLY (verified from System Layer repository)
10. ✅ **🔴 CRITICAL:** All required fields from System Layer Function's request DTO validation are included in dynamic request
11. ✅ **🔴 CRITICAL:** System Abstraction does NOT check response status codes (Service/Function responsibility)
12. ✅ **🔴 CRITICAL:** System Abstraction does NOT extract error details from response (Service/Function responsibility)
13. ✅ **🔴 CRITICAL:** System Abstraction does NOT log based on response status (Service/Function responsibility)
14. ✅ System Abstraction logs at start and end of method (simple logging only)
15. ✅ System Abstraction returns `HttpResponseMessage` directly (no status checking or error extraction)

**🛑 IF ANY VIOLATION → STOP → Fix System Abstraction → Then proceed**

### 5.9 System Abstractions - Common Mistakes

**❌ Adding SOR URL Construction Components to Process Layer AppConfigs**
...
**❌ Using .Result or Blocking Async Calls**
...
**❌ Manually Adding TestRunId/RequestId Headers**
...
**❌ Not Formatting Dates According to SOR Requirements**
...
**❌ Assuming Field Mapping Without Verification**
...
**❌ Request Payload Not Aligned with System Layer Function DTO**
...
**❌ Checking Response Status Codes or Logging Based on Response Status**
...
**🚨 CRITICAL:** Process Abstractions handle Process→Process Layer communication via HTTP calls. They are OPTIONAL - only create if you need to call other Process Layer Functions.

### 6.1 Process Abstractions - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE Process Abstraction UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I verified that I actually need to call another Process Layer Function?
2. ✅ **STOP AND VERIFY:** Do I understand that Process Abstractions are OPTIONAL (only create if needed)?
3. ✅ **STOP AND VERIFY:** Have I identified which Process Layer Function to call?
4. ✅ **STOP AND VERIFY:** Do I have the Process Layer Function URL in AppConfigs?
5. ✅ **STOP AND VERIFY:** Have I read this section completely?
6. ✅ **STOP AND VERIFY:** Do I know Process Abstractions make HTTP calls (NOT project references)?
7. ✅ **STOP AND VERIFY:** Do I know to use `SendProcessHTTPReqAsync()` extension method for HTTP calls?
8. ✅ **STOP AND VERIFY:** Do I understand the target Process Layer Function's request/response structure?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Process Abstraction**

**🔴 CRITICAL:** Process Abstractions are **OPTIONAL** - Only create if you actually need to call other Process Layer Functions. If you don't need Process→Process Layer communication, do NOT create Process Abstractions.
...
public class <Entity>Proc : I<Entity>Proc {
    ...
    public async Task<HttpResponseMessage> Process<Entity>(<Entity>ReqDTO request, string provider, int priority) {
        ...
        // ✅ Direct call - SendProcessHTTPReqAsync automatically adds TestRunId/RequestId headers
        // No helper methods needed - extension method handles everything
        // ✅ Use await for consistency and proper error handling
        return await _customHttpClient.SendProcessHTTPReqAsync(
            method: HttpMethod.Post, 
...
- ✅ MUST implement interface
- ✅ MUST inject `CustomHTTPClient` for HTTP calls
- ✅ MUST inject `IOptions<AppConfigs>` for Process Layer Function URLs
- ✅ MUST use `SendProcessHTTPReqAsync()` extension method
- ✅ MUST use `await` (NEVER `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
- ✅ Builds dynamic request using `ExpandoObject`
- ✅ Calls Process Layer Function URL from AppConfigs
...
```csharp
// Process→Process: Technical headers automatically added by extension method + business headers (if needed)
// ✅ Only add business headers if required by target Process Layer
...
// ✅ Direct call - extension method automatically adds TestRunId/RequestId
return await _customHttpClient.SendProcessHTTPReqAsync(
    method: HttpMethod.Post, 
    url: url, 
    contentType: "application/json", 
...
**Key Rules:**
- ✅ Technical headers (TestRunId, RequestId) are automatically added by `SendProcessHTTPReqAsync()` extension method
- ✅ Business headers are optional - only add if required by target Process Layer
- ✅ ❌ **NEVER manually add TestRunId/RequestId** - extension method handles this automatically

### 6.4 Process Abstractions - HTTP Call Method

**🔴 MANDATORY:** Use `SendProcessHTTPReqAsync()` extension method for ALL Process→Process Layer calls (Process Abstractions).

**🔴 CROSS-REFERENCE:** See Section 5.5 (System Abstractions - HTTP Call Method) for complete details on `SendProcessHTTPReqAsync()` extension method. The same extension method is used for both System Abstractions and Process Abstractions.

**Key Features:**
- ✅ **Automatically adds technical headers:** TestRunId and RequestId are automatically added
- ✅ **JSON serialization:** Automatically serializes body to JSON if provided
- ✅ **Content-Type handling:** Defaults to "application/json" but can be customized
- ✅ **Optional headers:** Can pass additional business headers via `reqHeaders` parameter

**Usage Pattern:**

```csharp
// ✅ CORRECT - Direct call with await (technical headers added automatically)
public async Task<HttpResponseMessage> Process<Entity>(<Entity>ReqDTO request, string provider, int priority) {
    dynamic req = new ExpandoObject();
    req.Code = request.Code;
    req.Config = new { Provider = provider, Priority = priority };
...
**🚨 CRITICAL:** Process Abstractions MUST implement interfaces and be injected via interface in Services.

| Component | Pattern | Usage |
|-----------|---------|-------|
| **Interface** | `public interface I<Entity>Proc` | ✅ MANDATORY - All Process Abstractions must have interfaces |
| **Implementation** | `public class <Entity>Proc : I<Entity>Proc` | ✅ MANDATORY - Must implement interface |
| **Injection in Service** | `private readonly I<Entity>Proc _entityProc;` | ✅ MANDATORY - Inject via interface |
| **Registration** | `builder.Services.AddScoped<I<Entity>Proc, <Entity>Proc>();` | ✅ MANDATORY - Register with interface |
| **Method** | `SendProcessHTTPReqAsync()` (extension method) | Process→Process calls - automatically adds TestRunId/RequestId headers |
| **Async Pattern** | **MUST use `await`** - NEVER use `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()` | Prevents deadlocks and thread blocking |
...
| **Optional** | ✅ **OPTIONAL** - Only create if calling other Process Layers | Not all projects need Process Abstractions |

**🔴 CRITICAL RULES:**

1. ✅ Process Abstractions MUST implement interfaces
2. ✅ Process Abstractions MUST be registered with interface in Program.cs
3. ✅ Process Abstractions MUST use `SendProcessHTTPReqAsync()` extension method
4. ✅ Process Abstractions MUST use `await` (NEVER `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
5. ✅ Process Abstractions are OPTIONAL - only create if needed
6. ❌ **NEVER** add project references to other Process Layer projects
7. ❌ **NEVER** register other Process Layer services/handlers
8. ❌ **NEVER** inject other Process Layer services directly
9. ❌ **NEVER** manually add TestRunId/RequestId headers (extension method does this)

### 6.6 Process Abstractions - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Process Abstraction, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Process Abstraction calls Process Layer Function URL from AppConfigs
2. ✅ **STOP AND VERIFY:** Process Abstraction uses `SendProcessHTTPReqAsync()` extension method (NOT direct HTTP calls)
3. ✅ **STOP AND VERIFY:** Process Abstraction uses `await` (NOT `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
4. ✅ **STOP AND VERIFY:** Process Abstraction implements interface and is registered with interface in Program.cs
5. ✅ **STOP AND VERIFY:** Business headers are only added if required by target Process Layer
6. ✅ **STOP AND VERIFY:** Technical headers (TestRunId/RequestId) are NOT manually added (extension method handles this)

**🛑 IF ANY VIOLATION → STOP → Fix Process Abstraction → Then proceed**
...
**❌ Creating Process Abstractions When Not Needed**
...
- **Why:** Process Abstractions are for Process→Process Layer communication - only create if this is required.

**❌ Not Using SendProcessHTTPReqAsync Extension Method**
...
**🚨 CRITICAL:** DTOs handle request/response data with validation and population logic. DTOs are the contract between Boomi and Process Layer Functions.

### 7.1 DTO Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE DTOs UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I read the Phase 1 document completely?
2. ✅ **STOP AND VERIFY:** Have I identified the Request Profile from Phase 1 (Step 1a: Input Structure Analysis)?
3. ✅ **STOP AND VERIFY:** Have I identified the Response Profile from Phase 1 (Step 1b: Response Structure Analysis)?
4. ✅ **STOP AND VERIFY:** Have I reviewed the Boomi JSON profile files for the exact structure?
5. ✅ **STOP AND VERIFY:** Do I understand the exact field names, data types, and mandatory/optional status from Phase 1?
6. ✅ **STOP AND VERIFY:** Have I checked date/datetime field formats from Boomi JSON profile files (especially System Layer output profile for SOR API format requirements)?
7. ✅ **STOP AND VERIFY:** Am I creating DTOs that match EXACTLY the Boomi INPUT profile structure (what Boomi sends to Process Layer), NOT the System Layer API structure?
8. ✅ **STOP AND VERIFY:** Have I checked the Boomi INPUT profile JSON file to see the exact nested structure (including "Object" wrappers if present)?
9. ✅ **STOP AND VERIFY:** Am I creating DTOs that match EXACTLY the Boomi contract (no additional classes, no assumptions, no unnecessary nesting)?
10. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document (Section 2: Process Properties Analysis) for HTTP headers and request parameters required by the Boomi process?
    - **🔴 CRITICAL CHECKLIST for each property in Phase 1 Section 2:**
      1. Read "Source Type" field for the property
      2. If "Source Type: definedparameter" → Add to AppConfigs (configuration), NOT headers/DTO, check component JSON for default value
      3. If "Source Type: trackparameter" with property name containing "inheader_" → Read from req.Headers in Function, design it to read it from headers, create it as a part of DTO and pass it to system layer in request DTO
      4. If "Source Type: trackparameter" with property name containing "inquery_" → Read from req.Query in Function, design it to read it from query, create it as a part of DTO and pass it to system layer in request DTO
      5. If "Source Type: trackparameter" with property name containing "indocument_" → Add to DTO request body
    - **❌ COMMON MISTAKE:** Assuming properties are headers when Phase 1 shows "Source Type: definedparameter"
    - **Example Error:** Adding `<ConfigProperty>` to headers when Phase 1 shows `Source Type: definedparameter` → Should be in AppConfigs
11. ✅ **STOP AND VERIFY:** Have I verified which fields the System Layer DTO requires by checking the System Layer DTO validation methods (e.g., `ValidateAPIRequestParameters()`, `ValidateDownStreamRequestParameters()`)?
12. ✅ **STOP AND VERIFY:** Have I identified ALL data sources needed (request body, HTTP headers, query parameters, config) to populate System Layer DTO fields?
13. ✅ **STOP AND VERIFY:** Have I read "🔴 CRITICAL: DTO DESIGN MISTAKES TO AVOID" section (Mistake #1-5)?
14. ✅ **STOP AND VERIFY:** Have I checked if Domain class exists (to determine if DTO needs `IRequestPopulatorDTO<DomainType>`)?
    - **🔴 CRITICAL:** If Domain exists → DTO MUST extend `IRequestPopulatorDTO<DomainType>` - applies to ALL operations (write AND read-only)
    - **🔴 CRITICAL:** This rule applies even if request DTO only contains query parameters and Domain represents response data
    - **🔴 CRITICAL:** This rule applies even if `Populate()` method will be empty (must still be implemented)

**🛑 IF ANY MISSING → STOP → Complete verification → Then create DTOs**
...
- **❌ NEVER confuse these two** - They are different structures!

**🔴 CRITICAL RULES FOR DTO CREATION:**
- **❌ NEVER create DTOs based on assumptions or existing code patterns**
- **❌ NEVER add fields/classes that don't exist in the Boomi contract**
- **❌ NEVER create helper classes (like `AttachmentInfo`) unless they exist in the Boomi profile**
- **✅ ALWAYS create DTOs based EXACTLY on Phase 1 document (Step 1a for Request, Step 1b for Response)**
- **✅ ALWAYS verify field names match Boomi JSON profile files EXACTLY**
- **✅ ALWAYS verify data types match Boomi profile (character = string, datetime = string with format, etc.)**
- **✅ ALWAYS verify mandatory/optional status from Phase 1 document**
- **✅ ALWAYS verify nested structure matches Boomi profile hierarchy EXACTLY**
- **🔴 CRITICAL: ALWAYS check date/datetime field formats from Boomi JSON profile files** - Check the System Layer output profile (the profile that maps to SOR API) for `ProfileDateFormat` with `dateFormat` attribute (e.g., `"yyyy-MM-dd"`, `"yyyy-MM-dd'T'HH:mm:ss"`). This format MUST be used when System Abstraction sends data to SOR.

**🔴 MANDATORY VERIFICATION WORKFLOW FOR DTO STRUCTURE:**
1. **✅ PRIMARY SOURCE: Match Boomi Request Payload Structure**
   ...
   - **Example:** If Boomi has `<operationName>` → `<entityName>` → fields, then DTO should have `<Operation>ReqDTO` → `<Entity>DTO` → properties

2. **✅ SECONDARY VERIFICATION: Check System Layer Function Request DTO**
...
**🚨 BEFORE creating ANY DTO, review these common mistakes:**

**Mistake #1: DTO Folder Naming with SOR Name**
- ❌ **WRONG:** `DTOs/<Operation><SOR>/` (includes SOR name, e.g., `DTOs/CreateEntityD365/`)
- ✅ **CORRECT:** `DTOs/<Operation>Request/` (operation-based, no SOR name, e.g., `DTOs/CreateEntityRequest/`)
...
- ❌ **WRONG:** `[JsonPropertyName("<fieldName>")] public string <FieldName> { get; set; }`
- ✅ **CORRECT:** `public string <FieldName> { get; set; } = string.Empty;` (property name matches Boomi field directly)
...
- ❌ **WRONG:** Adding helper methods like `Get<Field>ValueFromHeader()`, `Extract<Field>()`, field extraction methods
...
- ❌ **WRONG:** `<Operation>ReqDTO.cs` containing multiple nested classes (`<NestedClass1>`, `<NestedClass2>`, etc.) all in one file
...
- ❌ **WRONG:** Using Boomi's misleading field names directly in Process Layer DTOs (e.g., `{misleading-field-name}` for {actual-meaning}, `{ambiguous-field}` for {entity-details})
- ✅ **CORRECT:** Use meaningful property names internally, map from Boomi's field names in Function layer
...
### 7.2.1 🔴 CRITICAL: MEANINGFUL PROPERTY NAMES FOR BOOMI CONTRACTS

**🚨 MANDATORY RULE:** When Boomi process uses misleading, ambiguous, or unclear field names, Process Layer DTOs MUST use meaningful property names internally. Mapping from Boomi field names happens at the Function layer (when reading from request body, query parameters, or headers).
...
✅ **USE meaningful internal names when:**
...
❌ **DON'T use meaningful names when:**
...
**🔴 CRITICAL:** Mapping from Boomi field names to meaningful internal property names happens at the **Function layer**, NOT in DTOs:
...
**❌ WRONG - Using Boomi Misleading Name Directly:**
```csharp
public class <Operation>ReqDTO : IRequestBaseDTO
{
    // ❌ Misleading name - "{misleading-field-name}" suggests {wrong-meaning}, but it's actually {actual-meaning}
    public string {MisleadingFieldName} { get; set; } = string.Empty;
}
```

**✅ CORRECT - Meaningful Internal Name (Function Maps from Boomi):**
```csharp
...
**✅ CORRECT - System Abstraction Uses Meaningful Names:**
```csharp
...
{
    // ✅ Uses meaningful property name {MeaningfulPropertyName} (not "{misleading-field-name}")
    dynamic dynamicReq = new ExpandoObject();
...
**🔴 MANDATORY:** When Process Layer DTO property name differs from Boomi field name, XML documentation MUST explain:
...
### Summary
- ✅ **Process Layer DTOs:** Use meaningful property names internally
- ✅ **Function Layer:** Manually maps Boomi field names to meaningful DTO properties
- ✅ **System Abstraction:** Uses meaningful property names from Process Layer DTO
- ✅ **XML Documentation:** Explains property purpose, external name difference, and mapping approach
- ❌ **NEVER:** Use Boomi's misleading field names directly in Process Layer DTOs
- ❌ **NEVER:** Use `[JsonPropertyName]` in Process Layer DTOs for Boomi field name mapping (mapping happens in Function)
...
public class <Operation>ReqDTO : IRequestBaseDTO, IRequestPopulatorDTO<<Entity>Domain> {
    ...
    public void Validate() {
        ...
        // ✅ ONLY validate MANDATORY fields (from Phase 1 document)
        if (string.IsNullOrWhiteSpace(Code)) errors.Add(nameof(Code) + " is Required.");
        // ❌ Do NOT validate optional fields - they default to string.Empty
...
**🔴 MANDATORY RULE #0: DTO MUST EXTEND `IRequestPopulatorDTO<Domain>` FOR POPULATE FUNCTION**

**🚨 CRITICAL: This rule applies to ALL operations, including read-only query operations (Get, List, etc.)**

- **✅ MANDATORY:** If Domain class exists → Request DTO class declaration MUST extend `IRequestPopulatorDTO<DomainType>`
  - ✅ **CORRECT:** `public class <Operation>ReqDTO : IRequestDTO, IRequestPopulatorDTO<<Entity>Domain>`
  - ✅ **CORRECT:** `public class <Operation>ReqDTO : IRequestBaseDTO, IRequestPopulatorDTO<<Entity>Domain>`
  - ❌ **WRONG:** `public class <Operation>ReqDTO : IRequestDTO` (missing `IRequestPopulatorDTO<DomainType>`)
- **✅ MANDATORY:** DTO MUST implement `public void Populate(DomainType domain)` method
- **✅ MANDATORY:** Populate method MUST map ALL DTO properties to domain properties (or be empty if no mapping needed)
- **🔴 CRITICAL:** This rule has been missed 3+ times - DO NOT SKIP THIS MANDATORY RULE
...
- **Example 1: Write Operation (Create, Update) - MANDATORY Pattern:**
  ...
  public class <Operation>ReqDTO : IRequestDTO, IRequestPopulatorDTO<<Entity>Domain>
  {
      ...
      public void Populate(<Entity>Domain domain)
      {
          // ✅ CORRECT: Assign directly without null checks
      ...
      // public void Populate(<Entity>Domain domain)
      // {
      //     if (Property1 == null) return;  // ❌ Redundant - Validate() already ensures this
      //     if (Property2 != null && Property2.Count > 0)  // ❌ Redundant - Validate() already ensures this
      //     {
...
- **Example 2: Read-Only Query Operation (Get, List) - MANDATORY Pattern:**
  ...
  public class Get<Entity>ReqDTO : IRequestBaseDTO, IRequestPopulatorDTO<<Entity>Domain>
  {
      ...
      public void Populate(<Entity>Domain domain)
      {
          // ✅ CORRECT: Read-only query operation - Request DTO only contains query parameters,
...
- **Example 3: Function Implementation - Read-Only Operation:**
  ...
  public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "<entity>/details")] HttpRequest req)
  {
      ...
      // ✅ CORRECT: Domain creation and population required for ALL operations, including read-only
      <Entity>Domain domain = new <Entity>Domain();
...
1. **✅ ALL fields (optional and mandatory) MUST use default value `= string.Empty`** - Do NOT use nullable types (`string?`) for any fields
   - ✅ **CORRECT:** `public string <FieldName> { get; set; } = string.Empty;` (both optional and mandatory fields)
   - ❌ **WRONG:** `public string? <FieldName> { get; set; }` (fields should not be nullable)
   - **Why:** All fields default to `string.Empty`, eliminating the need for null checks throughout the codebase

2. **✅ Validate() method MUST ONLY validate MANDATORY fields** - Do NOT validate optional fields
   - ✅ **CORRECT:** Only validate fields marked as MANDATORY in Phase 1 document (check that they are not empty)
...
3. **🔴 CRITICAL: DTO Validation Error Messages MUST use `nameof()` with string interpolation - NEVER use ErrorConstants**
   - ✅ **CORRECT:** `errors.Add($"{nameof(PropertyName)} is required.");`
   - ✅ **CORRECT:** `errors.Add(nameof(PropertyName) + " is not a valid date in format yyyy-MM-dd.");`
   - ❌ **WRONG:** `errors.Add(ErrorConstants.PROPERTY_REQUIRED.Message);` - **PROHIBITED**
   - ❌ **WRONG:** `errors.Add(ErrorConstants.TYPE_REQUIRED.Message);` - **PROHIBITED**
   ...
   - **Example (WRONG Pattern - PROHIBITED):**
     ```csharp
     // ❌ WRONG - Using ErrorConstants for DTO validation
     if (string.IsNullOrWhiteSpace(LeaveRequest.OldLeaveId))
     {
         errors.Add(ErrorConstants.OLD_LEAVE_ID_REQUIRED.Message);  // ❌ PROHIBITED
     }
     ```

4. **✅ After Validate() passes, use fields directly** - No need for null coalescing (`??`) since all fields default to `string.Empty` and mandatory fields are validated
   - ✅ **CORRECT (in Function):** `dto.<FieldName>` (after Validate() passes, mandatory fields are guaranteed to be non-empty)
   - ❌ **WRONG (in Function):** `dto.<FieldName> ?? string.Empty` (unnecessary - fields already default to string.Empty)
...
5. **✅ Source of truth for mandatory/optional:** Phase 1 document (Step 1a) - Check `allowEmpty` attribute in Boomi JSON profile files
...
**🚨 CRITICAL: FUNCTION USAGE OF DTO PROPERTIES - NEVER GET CONFUSED**

**After `dto.Validate()` passes:**
- ✅ **CORRECT:** Use DTO properties directly - `dto.<FieldName>`
- ❌ **WRONG:** Using null coalescing - `dto.<FieldName> ?? string.Empty`
- ❌ **WRONG:** Using null checks - `if (dto.<FieldName> != null)`
- ❌ **WRONG:** Assigning default values - `string value = dto.<FieldName> ?? string.Empty;`
...
**Example Pattern:**
```csharp
// ✅ CORRECT - Trust the DTO contract after validation
...
// ❌ WRONG - Shows architectural misunderstanding
dto.Validate();
await service.Method(
    dto.FieldName ?? string.Empty,      // ❌ Unnecessary
    dto.AnotherField ?? string.Empty,   // ❌ Unnecessary
    dto.OptionalField ?? string.Empty   // ❌ Unnecessary
);
```
...
**Folder Structure:**
- ✅ **CORRECT:** `DTOs/<Operation>Request/` (operation-based, e.g., `DTOs/CreateEntityRequest/`)
- ❌ **WRONG:** `DTOs/<Operation><SOR>/` (includes SOR name, e.g., `DTOs/CreateEntityD365/`)
...
**Key Rules:**
- ✅ Folder names MUST be operation-based, NEVER include SOR name
- ✅ Each class MUST be in its own separate file (one class per file)
- ✅ Prefer flat DTOs with direct properties over nested classes unless Boomi contract explicitly requires nesting

### 7.6 DTO Rules - Key Rules

| Component | Pattern | Mandatory |
|-----------|---------|-----------|
| **Interface** | `IRequestBaseDTO` or `IRequestDTO` for requests | ✅ Request DTOs |
| **Interface** | `IRequestPopulatorDTO<<Entity>Domain>` - **🔴 MANDATORY: DTO MUST EXTEND THIS** | ✅ **MANDATORY** - Standard Process Layer (if Domain exists) - **🔴 CRITICAL: DTO class declaration MUST include this interface - Has been missed 3+ times - DO NOT SKIP** |
| **Validate()** | Throws `RequestValidationFailureException` | ✅ All Request DTOs |
| **Populate()** | Maps DTO → Domain - **DTO MUST IMPLEMENT THIS** | ✅ **MANDATORY** - Standard Process Layer (if Domain exists) - **🔴 CRITICAL: Has been missed 3+ times** - **🔴 CRITICAL: Applies to ALL operations (write AND read-only) - May be empty for read-only operations but must still be implemented** |
| **Headers** | `Dictionary<string, string> Headers` | ⚠️ **Need-based only** - Only if headers need propagation to System Layer/SOR |
| **Folder** | `DTOs/<OperationName>/` | ✅ Always |
| **Folder Naming** | Based on operation (e.g., `<Operation>Request`, `Get<Entity>`) | ✅ **Operation-based** - **❌ NEVER include SOR name** |
| **Folder Naming** | Include SOR name (e.g., `<Operation><SOR>`, `Get<Entity><SOR>`) | ❌ **WRONG** - SOR is determined by System Abstraction, not folder name |
| **Property Defaults** | `= string.Empty` for all string properties | ✅ **MANDATORY** - Never use nullable types |
| **Validation** | Only validate MANDATORY fields | ✅ **MANDATORY** - Optional fields default to `string.Empty` |
| **Structure** | Match Boomi INPUT profile EXACTLY | ✅ **MANDATORY** - Source of truth: Boomi JSON profile files |

**🔴 CRITICAL RULES:**

1. ✅ DTOs MUST match Boomi INPUT profile structure EXACTLY (what Boomi sends TO Process Layer)
2. ✅ DTOs MUST implement `IRequestPopulatorDTO<DomainType>` if Domain exists - **🔴 CRITICAL: Applies to ALL operations (write AND read-only)**
3. ✅ DTOs MUST have `Populate(DomainType domain)` method if Domain exists - **🔴 CRITICAL: Applies to ALL operations (write AND read-only) - May be empty for read-only operations but must still be implemented**
4. ✅ All properties MUST use default value `= string.Empty` (not nullable)
5. ✅ Validate() method MUST ONLY validate MANDATORY fields
6. ✅ Folder names MUST be operation-based (NEVER include SOR name)
7. ✅ Each class MUST be in its own separate file
8. ✅ **Response DTOs MUST use `[JsonPropertyName]` attributes** for camelCase serialization (matches Boomi contract)
9. ❌ **Request DTOs MUST NOT use `[JsonPropertyName]` attributes** (except `[JsonIgnore]` for non-serialized properties)
9. ❌ **NEVER create helper methods in DTOs**
10. ❌ **NEVER create unnecessary nested classes**

### 7.7 DTO Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION FOR REQUEST DTOs (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Request DTO, you MUST verify:**

1. ✅ **STOP AND VERIFY:** DTO implements `IRequestBaseDTO` 
2. ✅ **STOP AND VERIFY:** DTO has `Validate()` method that throws `RequestValidationFailureException`
   - [ ] **BLOCKING:** Run `grep -n "ErrorConstants\." <file>` in Validate() method → Document ALL matches found (line numbers)
   - [ ] **BLOCKING:** For EACH match found: Is it used in error message (e.g., `errors.Add(ErrorConstants.XXX.Message)`)? → **VIOLATION** → Must use `nameof()` with string interpolation → Fix → Re-run grep
   - [ ] **BLOCKING:** If ANY violations found → STOP → Replace with `nameof()` pattern → Re-run grep → Verify zero violations
   - [ ] **BLOCKING:** Verify ALL validation error messages use `nameof()` with string interpolation (e.g., `$"{nameof(PropertyName)} is required."`)
   - ✅ **CORRECT:** `errors.Add($"{nameof(PropertyName)} is required.");`
   - ✅ **CORRECT:** `errors.Add(nameof(PropertyName) + " is not a valid date in format yyyy-MM-dd.");`
   - ❌ **WRONG:** `errors.Add(ErrorConstants.PROPERTY_REQUIRED.Message);` - **PROHIBITED**
   - **🔴 CRITICAL:** DTO validation errors MUST use `nameof()` with string interpolation, NEVER use ErrorConstants
   - **🚨 CRITICAL:** You MUST run the grep command and verify no ErrorConstants are used in Validate() method. Do NOT skip this step.
3. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** DTO MUST extend `IRequestPopulatorDTO<DomainType>` AND implement `Populate(DomainType domain)` method
   - [ ] **STOP:** Check if Domain class exists in `Domains/<BusinessDomain>/` folder
   - [ ] **STOP:** If Domain exists → DTO class declaration MUST extend `IRequestPopulatorDTO<DomainType>` - **🔴 CRITICAL: Applies to ALL operations (write AND read-only)**
   - [ ] **STOP:** **🔴 CRITICAL:** This rule applies even if request DTO only contains query parameters and Domain represents response data
   - [ ] **STOP:** **🔴 CRITICAL:** This rule applies even if `Populate()` method will be empty (must still be implemented)
     - **✅ CORRECT:** `public class <Operation>ReqDTO : IRequestDTO, IRequestPopulatorDTO<<Entity>Domain>`
     - **✅ CORRECT:** `public class <Operation>ReqDTO : IRequestBaseDTO, IRequestPopulatorDTO<<Entity>Domain>`
     - **✅ CORRECT:** `public class Get<Entity>ReqDTO : IRequestBaseDTO, IRequestPopulatorDTO<<Entity>Domain>` (read-only operation - still required)
     - **❌ WRONG:** `public class <Operation>ReqDTO : IRequestDTO` (missing `IRequestPopulatorDTO<DomainType>`)
     - **❌ WRONG:** Assuming read-only operations are exempt from this rule
   - [ ] **STOP:** DTO MUST have `public void Populate(DomainType domain)` method implementation
   - [ ] **STOP:** **🔴 CRITICAL:** This applies to ALL operations, including read-only query operations (Get, List, etc.)
   ...
   - [ ] **BLOCKING:** Run `grep -n "\\?\\? string\\.Empty" <file>` in Populate() method → If found → **VIOLATION** → Populate() must NOT use null coalescing → Fix → Retry
   - [ ] **BLOCKING:** Run `grep -n "if.*null.*return\\|if.*!= null\\|if.*== null" <file>` in Populate() method → If found → **VIOLATION** → Populate() must NOT use null checks → Fix → Retry
   ...
   - [ ] **🔴 CRITICAL:** Populate() is called AFTER Validate() passes, so Validate() guarantees all mandatory properties are non-null/non-empty. Null checks are redundant and violate the architectural principle.
   - [ ] **🔴 CRITICAL:** This step has been missed 3+ times - DO NOT SKIP THIS CHECKPOINT
4. ✅ **STOP AND VERIFY:** All properties use default value `= string.Empty` (not nullable)
5. ✅ **STOP AND VERIFY:** Validate() method ONLY validates MANDATORY fields (from Phase 1)
6. ✅ **🔴 CRITICAL - BLOCKING RULE:** If a header/query parameter property in Process Layer DTO is required by System Layer request DTO validation (checked via `ValidateAPIRequestParameters()` or `ValidateDownStreamRequestParameters()`), then it MUST be validated in Process Layer DTO's `Validate()` method
   - [ ] **BLOCKING:** Open System Layer Function's request DTO file
   ...
   - [ ] **BLOCKING:** If System Layer DTO validation requires the header/query parameter property → MUST add validation in Process Layer DTO's `Validate()` method
   - ✅ **CORRECT:** Process Layer DTO validates `<HeaderProperty>` because System Layer DTO's `ValidateAPIRequestParameters()` requires it
   - ❌ **WRONG:** Process Layer DTO does NOT validate `<HeaderProperty>` even though System Layer DTO's `ValidateAPIRequestParameters()` requires it
...
7. ✅ **STOP AND VERIFY:** DTO structure matches Phase 1 document (Section 14: Input Structure Analysis)
8. ✅ **STOP AND VERIFY:** Folder name is operation-based (NOT including SOR name)
9. ✅ **STOP AND VERIFY:** Each class is in its own separate file
10. ✅ **STOP AND VERIFY:** Response DTOs MUST have `[JsonPropertyName]` attributes for camelCase serialization (matches Boomi contract)
11. ✅ **STOP AND VERIFY:** Request DTOs MUST NOT have `[JsonPropertyName]` attributes (except `[JsonIgnore]` if needed)
12. ✅ **STOP AND VERIFY:** No helper methods in DTO

**🛑 IF ANY VIOLATION → STOP → Fix DTO → Then proceed**

### 7.8 DTO Rules - Common Mistakes

**❌ Not Implementing IRequestPopulatorDTO<Domain>**
...
**❌ Using Nullable Types for DTO Properties**
...
**❌ Using Null Coalescing After Validation**
...
**❌ Using Null Checks in Populate() Method**

- **WRONG:** 
  ```csharp
  public void Populate(<Entity>Domain domain)
  {
      if (Property == null) return;  // ❌ VIOLATION
      if (Property != null && Property.Count > 0)  // ❌ VIOLATION
      {
...
- **CORRECT:**
  ...
  public void Populate(<Entity>Domain domain)
  {
      ...
      domain.Field = Property![0];  // ✅ Use null-forgiving operator if needed for compiler
...
- **Why:** 
  ...
  - Adding null checks shows misunderstanding of the Validate() → Populate() contract flow
  - **Architectural Principle:** "Functions trust DTOs - After Validate() passes, use properties directly"

**❌ Using ErrorConstants for DTO Validation Errors**
...
**❌ DTO Folder Naming with SOR Name**
...
**❌ Missing JsonPropertyName Attributes in Response DTOs**
...
**❌ Adding JsonPropertyName Attributes to Request DTOs**
...
**❌ Multiple Nested Classes in Single File**
...
**❌ Creating DTOs Based on Assumptions**
...
**🚨 CRITICAL:** Services orchestrate business logic and coordinate abstraction calls. Services are the orchestration layer between Functions and System/Process Abstractions.

**🔴 CRITICAL: Services DO NOT Orchestrate Cross-Cutting Concerns:**
- ✅ **Services orchestrate:** Business logic operations (multiple System Layer calls for domain operations)
- ❌ **Services DO NOT orchestrate:** Cross-cutting concerns (error notifications, logging, security) - These are Function responsibilities
...
**🔴 CRITICAL: Services DO NOT Extract/Validate Headers or Query Parameters:**
- ✅ **Function responsibility:** Extract headers/query parameters from `HttpRequest`, populate DTO.Headers, validate required headers
- ❌ **Service MUST NOT:** Extract headers from DTO.Headers, validate headers, extract query parameters
- **Why:** Header/query parameter extraction and validation are HTTP request concerns, not business logic concerns
- **Pattern:** Function reads headers → Populates DTO.Headers → Validates required headers → Passes DTO to Service (Service uses DTO.Headers if needed for System Layer calls, but does NOT validate)

**🔴 CRITICAL: Services DO NOT Orchestrate Error Notifications:**
- ✅ **Function responsibility:** Orchestrate error notifications (extract execution ID, serialize request payload, construct attachment filename, call NotificationService)
- ❌ **Service MUST NOT:** Call NotificationService, extract execution ID, serialize request payload, construct attachment filenames
- **Why:** Error notification orchestration is a cross-cutting concern, not business logic
- **Pattern:** Service throws exception on failure → Function catches failure in error path → Function orchestrates error notification → Function throws exception

### 8.1 Services Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE Services UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I read "🔴 CRITICAL: COMMON MISTAKES TO AVOID" section?
2. ✅ **STOP AND VERIFY:** Have I completed "🔍 MANDATORY FEATURE SEARCH" (search for "service")?
3. ✅ **STOP AND VERIFY:** Have I checked "COMMON MISTAKES" #20 (`internal` keyword), #27 (Extra Parameter)?
4. ✅ **STOP AND VERIFY:** Have I read Section 5 (System Abstractions Rules) - Services call System Abstractions?
5. ✅ **STOP AND VERIFY:** Have I read Section 6 (Process Abstractions Rules) - Services call Process Abstractions?
6. ✅ **STOP AND VERIFY:** Do I understand Services orchestrate, NOT implement business logic?
7. ✅ **STOP AND VERIFY:** Have I verified orchestration requirements (single call vs. multiple calls)?
8. ✅ **STOP AND VERIFY:** Do I understand that Services MUST accept DTOs (not Domain objects, not individual parameters)?
9. ✅ **STOP AND VERIFY:** Do I understand Services MUST NOT extract/validate headers or query parameters (Function responsibility)?
10. ✅ **STOP AND VERIFY:** Do I understand Services MUST NOT orchestrate error notifications (Function responsibility)?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Service**

**🔴 CRITICAL: DATA PASSING RULE (MANDATORY - NEVER VIOLATE)**

**🚨 BEFORE creating any Service method, you MUST verify:**
- ✅ **Service methods MUST accept DTOs** (not Domain objects, not individual parameters)
- ✅ **Domain objects are for internal purposes only** - Created and used within Function for business logic, validation, or other internal purposes
- ✅ **DTOs are the contract between layers** - Function passes DTO to Service, Service passes DTO to System Abstraction
- ❌ **NEVER accept Domain objects in Service methods** - This violates the layer separation principle
- ❌ **NEVER accept individual parameters** - Always accept the complete DTO object

**Example:**
```csharp
// ✅ CORRECT: Service accepts DTO
public async Task<HttpResponseSnapshot> CreateEntity(EntityReqDTO dto) { ... }

// ❌ WRONG: Service accepts Domain
public async Task<HttpResponseSnapshot> CreateEntity(EntityDomain domain) { ... }

// ❌ WRONG: Service accepts individual parameters
public async Task<HttpResponseSnapshot> CreateEntity(string id, string name, string code) { ... }
```

**🛑 IF YOU ARE ABOUT TO CREATE A SERVICE METHOD THAT ACCEPTS DOMAIN → STOP → Change it to accept DTO**

**🔴 CRITICAL: SERVICE RESPONSIBILITY RULE (MANDATORY - NEVER VIOLATE)**

**🚨 Services MUST NOT perform HTTP request concerns:**
- ❌ **NEVER extract headers from DTO.Headers** - Function already extracted headers before passing DTO to Service
- ❌ **NEVER validate headers** - Function validates required headers before calling Service
- ❌ **NEVER extract query parameters** - Function extracts query parameters and populates DTO before calling Service
- ❌ **NEVER orchestrate error notifications** - Function orchestrates error notifications in error path
- ❌ **NEVER extract execution ID** - Function extracts execution ID for error notifications
- ❌ **NEVER serialize request payload** - Function serializes request payload for error notification attachments
- ❌ **NEVER serialize request payload BEFORE service call** - Request payload serialization MUST be in error path ONLY (when attachment is needed), NOT before service call
- ❌ **NEVER construct attachment filenames** - Function constructs attachment filenames for error notifications
- ✅ **Services MAY use DTO.Headers** - If needed for System Layer calls (e.g., passing header values to System Abstraction), but Service does NOT extract or validate
- ✅ **Services orchestrate business logic** - Multiple System Layer calls, business rule validation, data transformation
- ✅ **Services throw exceptions** - On business logic failures or System Layer failures (Function handles error notification orchestration)

**Example - WRONG (Service extracting/validating headers):**
```csharp
// ❌ WRONG: Service extracting and validating headers
public class EntityService
{
    public async Task<HttpResponseMessage> CreateEntity(EntityReqDTO dto)
    {
        // ❌ WRONG: Service extracting header from DTO
        ...
        // ❌ WRONG: Service validating header
...
```csharp
// ✅ CORRECT: Function extracts and validates headers
...
public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "entity")] HttpRequest req)
{
    ...
    // ✅ CORRECT: Function extracts headers
    dto.Headers = req.Headers.ToDictionary(h => h.Key.ToLower(), h => h.Value.FirstOrDefault() ?? string.Empty);
    
    // ✅ CORRECT: Function validates required headers
    ...
    // ✅ CORRECT: Function passes DTO to Service (headers already extracted and validated)
    HttpResponseMessage response = await _service.CreateEntity(dto);
    // ... rest of Function code
}

// ✅ CORRECT: Service uses DTO.Headers if needed (but does NOT extract or validate)
public class EntityService
{
    public async Task<HttpResponseMessage> CreateEntity(EntityReqDTO dto)
    {
        // ✅ CORRECT: Service may use DTO.Headers for System Layer calls (already extracted by Function)
...
**🔴 CRITICAL: SERVICE SEPARATION RULE (MANDATORY)**

**🚨 ALWAYS create separate Services for different domain operations:**

1. ✅ **Domain-specific Services** → One Service per business domain (e.g., `EntityService`, `AnotherEntityService`, `ThirdEntityService`)
2. ✅ **Cross-cutting concern Services** → Separate Service for shared functionality (e.g., `NotificationService`, `SecurityService`, `ValidationService`)
3. ✅ **Different System Abstractions** → If a Service needs to call different System Abstractions for different purposes, consider separating them

**❌ WRONG Patterns:**
```csharp
// ❌ WRONG: Mixing domain operations with cross-cutting concerns
public class EntityService
{
    public async Task<HttpResponseSnapshot> CreateEntity(...) { ... }
    public async Task SendErrorNotification(...) { ... }  // ❌ Cross-cutting concern
}

// ❌ WRONG: Service orchestrating error handling (cross-cutting concern)
public class EntityService
{
    public async Task<HttpResponseMessage> CreateEntity(EntityReqDTO dto)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            // ❌ WRONG: Service orchestrating error notification
...
// ❌ WRONG: Service extracting/validating headers (HTTP request concern, not business logic)
public class EntityService
{
    public async Task<HttpResponseMessage> CreateEntity(EntityReqDTO dto)
    {
...
// ❌ WRONG: Mixing different domain operations
public class EntityService
{
    public async Task<HttpResponseSnapshot> CreateEntity(...) { ... }
    public async Task<HttpResponseSnapshot> CreateAnotherEntity(...) { ... }  // ❌ Different domain
}
```

**✅ CORRECT Patterns:**
```csharp
// ✅ CORRECT: Domain-specific Service (orchestrates business logic only)
public class EntityService
{
    public async Task<HttpResponseMessage> CreateEntity(EntityReqDTO dto)
    {
        // ✅ CORRECT: Service uses DTO.Headers if needed for System Layer calls (but does NOT extract/validate)
        // Function already extracted and validated headers before passing DTO to Service
        HttpResponseMessage response = await _entityMgmt.CreateEntity(dto);
        if (!response.IsSuccessStatusCode)
        {
            // ✅ CORRECT: Service throws exception, does NOT orchestrate error handling
...
// ✅ CORRECT: Function extracts/validates headers, then passes DTO to Service
[Function("CreateEntity")]
public async Task<BaseResponseDTO> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "entity")] HttpRequest req)
{
    EntityReqDTO? dto = await req.ReadBodyAsync<EntityReqDTO>();
    ...
    {
        // ✅ CORRECT: Function orchestrates error notification (cross-cutting concern)
        string executionId = RequestHeaders.RequestId.Item2.Value?.ToString() ?? string.Empty;
        string requestPayload = JsonSerializer.Serialize(dto);
...
// ✅ CORRECT: Separate Service for cross-cutting concerns
...
// ✅ CORRECT: Function orchestrates error handling (cross-cutting concern)
[Function("CreateEntity")]
public async Task<BaseResponseDTO> Run(...)
{
    HttpResponseMessage response = await _service.CreateEntity(dto);
...
public class <Entity>Service {
    ...
    private readonly I<Entity>Mgmt _entityMgmt;  // ✅ System Abstraction injected via interface
...
```csharp
public class <Entity>Service : I<Entity>Service {
    private readonly I<Entity>Proc _entityProc;  // ✅ Process Abstraction injected via interface
    private readonly ILogger<<Entity>Service> _logger;
...
**🚨 CRITICAL:** When multiple operations fail (e.g., first operation fails AND second operation fails), merge error details from both responses into a single BaseResponseDTO to ensure all failures are reported.
...
**🔴 CRITICAL ASYNC PATTERN - NEVER VIOLATE:**

**❌ PROHIBITED PATTERNS:**
- ❌ **NEVER use `_ = Task.Run(...)`** - Fire-and-forget pattern is prohibited
- ❌ **NEVER use `_ = await ...`** - Discarding task results is prohibited (always store return values)
- ❌ **NEVER use `Task.Run(async () => await ...)`** - Background task pattern is prohibited
- ❌ **NEVER use `async void`** - Exception handling issues
- ❌ **NEVER use `.Result`, `.Wait()`, `.GetAwaiter().GetResult()`** - Blocking calls cause deadlocks
- ❌ **NEVER use `try-catch`** - Let exceptions propagate to middleware for proper error handling
- ❌ **🔴 NEVER EVER use `var` keyword** - Always use explicit types for clarity and consistency (MANDATORY - NO EXCEPTIONS)

**✅ CORRECT PATTERNS:**
- ✅ **Always use `await`** for all asynchronous operations
- ✅ **Always store return values** - System/Process Abstraction calls return HttpResponseSnapshot, always store in variable
- ✅ **Always check non-200 responses** - Throw PassThroughHttpException for non-success status codes
- ✅ **Let exceptions propagate** to middleware for proper error handling
- ✅ **Middleware handles all exceptions** - ExceptionHandlerMiddleware normalizes all exceptions to BaseResponseDTO
- ✅ **🔴 ALWAYS use explicit types** - Never use `var`, always specify type explicitly (MANDATORY - NO EXCEPTIONS)

**Example - Error Notification (CORRECT):**
```csharp
if (!primaryResponse.IsSuccessStatusCode)
{
    // ✅ CORRECT - Store return value (HttpResponseSnapshot) for consistency
    HttpResponseSnapshot notificationResponse = await SendErrorNotification(dto);
    // ✅ CORRECT - Check notification response status and throw exception if not successful
    if (!notificationResponse.IsSuccessStatusCode)
...
```csharp
// ✅ CORRECT - Explicit type, store return value
...
```csharp
// ❌ WRONG - Task.Run is prohibited
_ = Task.Run(async () => await SendErrorNotification(dto));
// ❌ WRONG - try-catch is prohibited (let middleware handle exceptions)
...
|------|---------|-------------|
| **Interfaces** | `public class <Entity>Service : I<Entity>Service` | ⚠️ **OPTIONAL** - Services can have interfaces if needed |
| **No Interface** | `public class <Entity>Service` | ✅ **ALLOWED** - Services without interfaces are valid |
...
| **System Abstraction Injection** | `private readonly I<Entity>Mgmt _entityMgmt;` | ✅ **MANDATORY** - MUST inject System Abstractions via interface |
| **Process Abstraction Injection** | `private readonly I<Entity>Proc _entityProc;` | ✅ **MANDATORY** - MUST inject Process Abstractions via interface |
| **Function Injection** | `private readonly <Entity>Service _service;` OR `private readonly I<Entity>Service _service;` | ✅ Both patterns allowed - direct injection or via interface |
| **Logging Start** | `_logger.Info("[Process Layer]-Initiating MethodName")` | ALWAYS at method start - **🔴 REQUIRES `using Core.Extensions;`** |
| **Logging End** | `_logger.Info("[Process Layer]-Completed MethodName")` | ALWAYS before return - **🔴 REQUIRES `using Core.Extensions;`** |
| **Logging Error** | `_logger.Error("Description")` or `_logger.Warn("Warning")` | Before throwing exceptions or recoverable issues - **🔴 REQUIRES `using Core.Extensions;`** |
| **❌ PROHIBITED** | `_logger.LogInformation()` or `_logger.LogError()` | **NEVER use** - Must use `Info()`/`Error()` extension methods from `Core.Extensions` |
...
1. ✅ Services MUST inject System/Process Abstractions via interface
2. ✅ Services can be registered with or without interfaces (interface is optional)
3. ✅ Services MUST accept DTOs (not Domain objects, not individual parameters)
4. ✅ Services MUST use `await` and store return values (NEVER discard task results)
5. ✅ Services MUST check response status and throw exceptions for non-success
6. ✅ Services MUST have logging (entry and exit) using `_logger.Info()`
7. ❌ **NEVER use `var` keyword** - Always use explicit types
8. ❌ **NEVER use `internal` keyword** - Methods MUST be public
9. ❌ **NEVER use `try-catch` blocks** - Let exceptions propagate to middleware
10. ❌ **NEVER discard task results** - Always store return values

### 8.7 Services Rules - DI Registration Patterns

**🚨 CRITICAL:** Services can be registered with or without interfaces. System/Process Abstractions MUST be registered with interfaces.

```csharp
// ✅ CORRECT - Service without interface (direct injection)
builder.Services.AddScoped<<Entity>Service>();
builder.Services.AddScoped<DataService>();

// ✅ CORRECT - Service with interface (if interface exists)
builder.Services.AddScoped<I<Entity>Service, <Entity>Service>();
builder.Services.AddScoped<IDataService, DataService>();

// ✅ CORRECT - Central Data Layer (typically with interface for public APIs)
builder.Services.AddScoped<ISecurityService, SecurityService>();
builder.Services.AddScoped<IReferenceService, ReferenceService>();

// ✅ CORRECT - Transient for stateless utilities
...
**🛑 POST-CREATION VALIDATION FOR SERVICES (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Service, you MUST verify (BLOCKING - CANNOT SAVE FILE UNTIL ALL PASS):**

**🔴 MANDATORY VIOLATION CHECKS (RUN BEFORE SAVING FILE - SYSTEMATIC FILE-BY-FILE VALIDATION):**
...
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for `LogInformation` or `LogError` → If found → **VIOLATION** → Must use `_logger.Info()` or `_logger.Error()` → Document line number → Fix → Re-read file
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for `internal async Task` or `internal Task` → If found → **VIOLATION** → Service methods MUST be public → Document line number → Fix → Re-read file
- [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for ALL Service method signatures → For EACH: Verify it accepts ONLY DTO (not individual parameters) → If violation → **VIOLATION** → Document line number → Fix → Re-read file
- [ ] **BLOCKING:** Verify file has `using Core.Extensions;` (if using `_logger.Info()` extension method)

1. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Service separation rule is followed
   ...
   - [ ] **BLOCKING:** If YES → **VIOLATION** - Must separate into two Services:
   ...
   - ✅ **CORRECT:** `<Entity>Service` contains ONLY domain operations (e.g., `Create<Entity>Documents`)
   - ✅ **CORRECT:** `NotificationService` contains ONLY cross-cutting concerns (e.g., `SendErrorNotificationEmail`)
   - ❌ **WRONG:** `<Entity>Service` contains both `Create<Entity>Documents` AND `SendErrorNotificationEmail` - **PROHIBITED**
   - ❌ **WRONG:** Mixing domain operations with cross-cutting concerns in same Service - **PROHIBITED**
   - **🔴 CRITICAL:** This rule has been missed - DO NOT SKIP THIS CHECKPOINT
   - **OUTPUT REQUIRED:** Document Service method classification in validation report. If separation needed → Create separate Service classes → Re-verify

2. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Service methods accept DTOs, NOT Domain objects
   ...
   - ✅ **CORRECT:** `public async Task<HttpResponseSnapshot> Create<Entity>Documents(<Entity>ReqDTO request)`
   - ❌ **WRONG:** `public async Task<HttpResponseSnapshot> Create<Entity>Documents(<Entity> domain)` - **PROHIBITED**
   - **🔴 CRITICAL:** Service methods MUST accept DTOs, Domain objects stay in Function for internal use only

3. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Service methods do NOT have extra parameters
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for ALL Service method signatures
   - [ ] **BLOCKING:** For EACH Service method signature found:
     - [ ] Does it accept ONLY a DTO parameter (e.g., `(<Operation>ReqDTO request)`)? → ✅ PASS
     - [ ] Does it accept multiple individual parameters (e.g., `(string param1, string param2, ...)`)? → **VIOLATION** → Document line number
     - [ ] Does it accept DTO + extra parameters (e.g., `(<Operation>ReqDTO request, string param1)`)? → **VIOLATION** → Document line number
   - [ ] **BLOCKING:** If ANY violations found → STOP → Create DTO for all parameters → Update Service method signature → Re-read file → Verify only DTO parameters
   - [ ] **BLOCKING:** Re-read file → Verify ALL Service methods accept ONLY DTO parameters (no individual parameters)
   - ✅ **CORRECT:** `public async Task<HttpResponseSnapshot> Create<Entity>Documents(<Entity>ReqDTO request)` - all data in DTO
   - ✅ **CORRECT:** `public async Task<HttpResponseSnapshot> SendErrorNotificationEmail(<ErrorNotificationEmail>ReqDTO request)` - all data in DTO
   - ❌ **WRONG:** `public async Task<HttpResponseSnapshot> Create<Entity>Documents(<Entity>ReqDTO request, string <parameter>)` - **PROHIBITED**
   - ❌ **WRONG:** `public async Task<HttpResponseSnapshot> SendErrorNotificationEmail(string processName, string environment, string executionId, ...)` - **PROHIBITED**
   - **🔴 CRITICAL:** All data MUST flow through DTOs, never add extra parameters
   - **🚨 CRITICAL:** You MUST read the entire file systematically and verify ALL Service methods accept ONLY DTO parameters. Do NOT skip this step.

4. ✅ **STOP AND VERIFY:** Service injects System/Process Abstractions via interface
   - ✅ **CORRECT:** `private readonly I<Entity>Mgmt _entityMgmt;` (injected via interface)
   - ❌ **WRONG:** `private readonly <Entity>MgmtSys _entityMgmt;` (injected as concrete class)

5. ✅ **STOP AND VERIFY:** Service methods use `await` and store return values
   - ✅ **CORRECT:** `HttpResponseSnapshot response = await _entityMgmt.Create<Entity>(request);`
   - ❌ **WRONG:** `_ = await _entityMgmt.Create<Entity>(request);` (discarding return value - **PROHIBITED**)

6. ✅ **STOP AND VERIFY:** Service methods check response status and throw exceptions for non-success
   - ✅ **CORRECT:** `if (!response.IsSuccessStatusCode) throw new PassThroughHttpException(...);`
   - ❌ **WRONG:** Ignoring response status codes

7. ✅ **STOP AND VERIFY:** Service has logging (entry and exit)
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Verify logging uses `_logger.Info()` (NOT `LogInformation()`) → If found → **VIOLATION** → Document line number → Fix → Re-read file
   - [ ] **BLOCKING:** Verify file has `using Core.Extensions;` (required for `Info()` extension method)
   - ✅ **CORRECT:** `_logger.Info($"[Process Layer]-{InfoConstants.CREATE_<ENTITY>_STARTED}");` at start
   - ✅ **CORRECT:** `_logger.Info($"[Process Layer]-{InfoConstants.CREATE_<ENTITY>_COMPLETED}");` at end
   - ❌ **WRONG:** `_logger.LogInformation(...)` - **VIOLATION** - Must use `_logger.Info()`

8. ✅ **STOP AND VERIFY:** Service is registered in `Program.cs` DI container
   - ✅ **CORRECT:** `builder.Services.AddScoped<<Entity>Service>();`
   - ✅ **CORRECT:** `builder.Services.AddScoped<NotificationService>();` (if cross-cutting service exists)

9. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Email template validation (if Service contains email notification methods)
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Check if this Service contains email notification methods (e.g., `SendErrorNotificationEmail`, `SendEmail`, `SendNotification`)
   - [ ] **BLOCKING:** If YES → Did I verify email templates are embedded HTML resources (NOT hardcoded strings)?
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Look for email body construction patterns (`emailBody`, `BuildErrorEmailBody`, HTML strings, etc.)
   - [ ] **BLOCKING:** For EACH email body construction found:
     - [ ] Is it calling a helper method that reads embedded template (e.g., `ResponseDTOHelper.ReadEmailTemplate(...)`)? → ✅ PASS
   ...
   - [ ] **BLOCKING:** Did I verify placeholders in email template are populated from correct sources (check Phase 1 Section 9 - Properties Read table and subprocess JSON msgParameters)?
   - [ ] **BLOCKING:** Did I verify placeholder population uses `string.Format()` with correct parameter order matching Boomi subprocess msgParameters key order?
   - [ ] **BLOCKING:** Read the ENTIRE file line by line → Verify NO hardcoded email HTML strings in Service code (e.g., `string.Format(InfoConstants.EMAIL_BODY_TEMPLATE, ...)`)
   - [ ] **BLOCKING:** Read InfoConstants.cs and ErrorConstants.cs files → Verify NO email HTML constants exist
   - ✅ **CORRECT:** Email notification uses embedded HTML template: `string template = ResponseDTOHelper.ReadEmailTemplate("<ProjectName>.EmailTemplate.<TemplateName>.html"); string emailBody = string.Format(template, processName, environment, executionId, errorMessage);`
   - ✅ **CORRECT:** Email template file exists: `EmailTemplate/<TemplateName>.html` with exact Boomi HTML structure extracted from subprocess JSON msgTxt section
   - ✅ **CORRECT:** Email template marked as embedded resource in .csproj: `<EmbeddedResource Include="EmailTemplate\<TemplateName>.html" />`
   - ❌ **WRONG:** Hardcoded email body: `string emailBody = string.Format(InfoConstants.EMAIL_BODY_TEMPLATE, ...);` - **PROHIBITED**
   - ❌ **WRONG:** Email HTML hardcoded in constants: `EMAIL_BODY_TEMPLATE = "Process: {0}<br>..."` in `InfoConstants.cs` - **PROHIBITED**
   - ❌ **WRONG:** Email template doesn't match Boomi structure from subprocess JSON - **PROHIBITED**
   - ❌ **WRONG:** Email template placeholders don't match Boomi subprocess msgParameters numbering - **PROHIBITED**
   - ❌ **WRONG:** Placeholder population parameter order doesn't match Boomi subprocess msgParameters key order - **PROHIBITED**
   ...
   - **🔴 CRITICAL:** Placeholder population parameter order MUST match Boomi subprocess msgParameters key order (verify parameter order in string.Format matches msgParameters)
   - **🔴 CRITICAL:** Each process has different email templates - verify template name and structure from Phase 1 Section 9 or subprocess JSON
   - **🔴 CRITICAL:** This rule has been missed - DO NOT SKIP THIS CHECKPOINT if Service contains email methods
   - **🚨 CRITICAL:** You MUST read the entire file systematically and verify embedded templates are used. Do NOT skip this step.

**🛑 IF ANY CHECKPOINT FAILS → STOP → Fix violation → Re-verify → Then proceed**

### 8.10 Services Rules - Common Mistakes

**❌ Not Separating Domain and Cross-Cutting Concerns**
...
**❌ Accepting Domain Objects in Service Methods**
...
**❌ Adding Extra Parameters to Service Methods**
...
**❌ Using LogInformation/LogError Instead of Extension Methods**
...
**❌ Not Injecting Abstractions via Interface**
...
**When to Use:**
- ✅ Need LOVs or configuration values from internal database
- ✅ Need authentication/authorization services (OTP, Token generation/validation)
- ✅ Need to read/write application data (Applications, Settings)
- ✅ Need to connect to 3rd party system database
- ✅ Need cached data with DB fallback
- ✅ Need cache management operations

**How to Use:**
1. Check if Azure Function already exists in Shared Services (`Functions/` directory)
2. If exists → Call it via Process Abstraction (Section 6)
3. If not exists → Create new Azure Function in Shared Services, then call it
...
public class DataServicesProc {
    public async Task<HttpResponseMessage> GetData(DataReqDTO req) {
        ...
        // ✅ Build business headers if needed (technical headers added automatically by extension method)
        List<Tuple<string, string>> businessHeaders = new List<Tuple<string, string>>();
        if (req.Headers != null) {
...
- Maps EF Core entities → DTOs (one direction only, never reverse for database operations)
...
|-----------|------------------------|-------------------------------------|
| **Repositories/** | ❌ NEVER exists | ✅ Generic Repository<T> + specific repositories |
| **Models/** | ❌ NEVER exists | ✅ EF Core entities (Application, Lov, Settings) |
| **AutoMapping/** | ❌ NEVER exists | ✅ AutoMapper profiles (Entity→DTO) |
| **DbContext** | ❌ NEVER used | ✅ AppDbContext with full entity configurations |
| **Internal DB Access** | ❌ MUST call Shared Services Azure Functions | ✅ Direct via Repository<T> + EF Core |
| **3rd Party DB Access** | ❌ MUST call Shared Services Azure Functions | ✅ Create Azure Function to connect |
| **Domains** | ✅ IDomain<T> with business logic | ✅ Simple POCOs (e.g., OTP with IsLockout()) |
| **KeyVault** | ❌ Not used | ✅ KeyVaultReader for secrets (connection strings) |
| **Flyway** | ❌ Not used | ✅ CI/CD pipeline migrations for internal DB |
...
1. ✅ **Standard Process Layers MUST call Shared Services Azure Functions** for ANY database operations
2. ✅ **Shared Services is the ONLY Process Layer with database access**
3. ✅ **NEVER create repositories, DbContext, or database connections in standard Process Layers**
4. ✅ **Shared Services uses Repository<T> pattern + EF Core for internal database access**
5. ✅ **Shared Services creates Azure Functions for 3rd party database access**
6. ✅ **Shared Services uses KeyVault for connection string management**
7. ✅ **Shared Services uses AutoMapper for Entity→DTO mapping**

### 9.10 Central Data Process Layer - Common Mistakes

**❌ Creating Database Access in Standard Process Layers**
...
**❌ Direct Database Connections in Standard Process Layers**
...
**❌ Missing KeyVault Configuration**
...
- **CORRECT:** Use AutoMapper profiles for Entity→DTO mapping
...
**Key Points:**
- ✅ Use `SendProcessHTTPReqAsync()` extension method (same as Process Abstractions)
- ✅ Technical headers (TestRunId, RequestId) are automatically added
- ✅ Business headers are optional - only add if required by Shared Services Function
- ✅ Build dynamic request using `ExpandoObject`
- ✅ Call Shared Services Function URL from AppConfigs
- ✅ MUST implement interface and be registered with interface in Program.cs
...
| **Purpose** | ALL database operations (internal + 3rd party), shared business logic |
| **When to Create New Function** | Need new DB operation not covered by existing Azure Functions |
| **Interface** | ✅ **MANDATORY** - Must implement interface (e.g., `IDataServicesProc`) |
| **Registration** | ✅ **MANDATORY** - Must be registered with interface in Program.cs |
...
1. ✅ **Standard Process Layers MUST call Shared Services Azure Functions** for ANY database operations
2. ✅ **NEVER create repositories, DbContext, or database connections** in standard Process Layers
3. ✅ **Use Process Abstraction pattern** to call Shared Services (same as calling other Process Layers)
4. ✅ **MUST implement interface** and be registered with interface in Program.cs
5. ✅ **Use `SendProcessHTTPReqAsync()` extension method** for HTTP calls
6. ✅ **Technical headers are automatically added** - no need to add TestRunId/RequestId manually
7. ✅ **Business headers are optional** - only add if required by Shared Services Function

### 10.4 Calling Central Data Process Layer - When to Call Shared Services

**✅ Call Shared Services When You Need:**
...
**❌ Do NOT Call Shared Services For:**
...
**🚨 BEFORE creating Process Abstraction to call Shared Services, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Do I actually need database access or Shared Services functionality?
2. ✅ **STOP AND VERIFY:** Have I checked if Shared Services Azure Function already exists?
3. ✅ **STOP AND VERIFY:** Do I have the Shared Services Function URL in AppConfigs?
4. ✅ **STOP AND VERIFY:** Have I read Section 6 (Process Abstractions Rules)?
5. ✅ **STOP AND VERIFY:** Do I understand that Shared Services is called like any other Process Layer?
6. ✅ **STOP AND VERIFY:** Do I know to use `SendProcessHTTPReqAsync()` extension method?
7. ✅ **STOP AND VERIFY:** Do I understand the Shared Services Function's request/response structure?
...
**🚨 AFTER creating Process Abstraction to call Shared Services, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Process Abstraction calls Shared Services Function URL from AppConfigs
2. ✅ **STOP AND VERIFY:** Process Abstraction uses `SendProcessHTTPReqAsync()` extension method (NOT direct HTTP calls)
3. ✅ **STOP AND VERIFY:** Process Abstraction uses `await` (NOT `.Result`, `.Wait()`, or `.GetAwaiter().GetResult()`)
4. ✅ **STOP AND VERIFY:** Process Abstraction implements interface and is registered with interface in Program.cs
5. ✅ **STOP AND VERIFY:** Business headers are only added if required by Shared Services Function
6. ✅ **STOP AND VERIFY:** Technical headers (TestRunId/RequestId) are NOT manually added (extension method handles this)
...
**❌ Not Using Process Abstraction Pattern**
...
**❌ Manually Adding Technical Headers**

- **WRONG:** Manually adding `RequestHeaders.TestRunId.Simplify()` and `RequestHeaders.RequestId.Simplify()` to headers
- **CORRECT:** `SendProcessHTTPReqAsync()` extension method automatically adds these headers
- **Why:** Extension method handles technical headers automatically - no need to add them manually.

**❌ Not Checking if Function Exists**
...
**🚨 CRITICAL:** Process Layer uses ONLY Framework middlewares. NO custom middlewares are allowed.

### 11.1 Middleware Rules - Middleware Order (Non-Negotiable)

**🔴 CRITICAL:** Middleware order is STRICT and NON-NEGOTIABLE. The order MUST be exactly as specified.
...
builder.ConfigureFunctionsWebApplication(app => {
    ...
    // ❌ NO CustomAuthenticationMiddleware - System Layer concept
});
```
...
**❌ PROHIBITED:**
- ❌ **NO CustomAuthenticationMiddleware** - This is a System Layer concept, NOT Process Layer
- ❌ **NO custom middlewares** - Use Framework middlewares only
- ❌ **NO middleware order changes** - Order is non-negotiable
...
| **CustomAuthenticationMiddleware** | ❌ NOT IN PROCESS LAYER | System Layer only | N/A |

**🔴 CRITICAL RULES:**

1. ✅ **ONLY Framework middlewares** - Use ExecutionTimingMiddleware and ExceptionHandlerMiddleware from Framework
2. ✅ **Strict order** - ExecutionTimingMiddleware FIRST, ExceptionHandlerMiddleware SECOND
3. ✅ **NO custom middlewares** - Do NOT create custom middleware classes
4. ✅ **NO CustomAuthenticationMiddleware** - This is System Layer only, NOT Process Layer
5. ✅ **Let exceptions propagate** - Functions/Services should NOT use try-catch blocks - let ExceptionHandlerMiddleware handle all exceptions
...
// ❌ WRONG - try-catch prevents proper exception handling
public async Task<BaseResponseDTO> Run([HttpTrigger(...)] HttpRequest req) {
    try {
        // ... function code
    } catch (Exception ex) {
        // ❌ WRONG - ExceptionHandlerMiddleware should handle this
...
**🚨 BEFORE registering middleware in Program.cs, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Do I understand that ONLY Framework middlewares are allowed?
3. ✅ **STOP AND VERIFY:** Do I understand the strict middleware order requirement?
4. ✅ **STOP AND VERIFY:** Do I know that CustomAuthenticationMiddleware is NOT allowed in Process Layer?
5. ✅ **STOP AND VERIFY:** Do I understand that Functions/Services should NOT use try-catch blocks?

**🛑 IF ANY MISSING → STOP → Complete verification → Then register middleware**

### 11.6 Middleware Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER registering middleware in Program.cs, you MUST verify:**

1. ✅ **STOP AND VERIFY:** ExecutionTimingMiddleware is registered FIRST
2. ✅ **STOP AND VERIFY:** ExceptionHandlerMiddleware is registered SECOND
3. ✅ **STOP AND VERIFY:** NO CustomAuthenticationMiddleware is registered
4. ✅ **STOP AND VERIFY:** NO custom middlewares are registered
5. ✅ **STOP AND VERIFY:** Middleware order is exactly: ExecutionTiming → ExceptionHandler

**🛑 IF ANY VIOLATION → STOP → Fix Program.cs → Then proceed**

### 11.7 Middleware Rules - Common Mistakes

**❌ Using CustomAuthenticationMiddleware**
...
**❌ Using try-catch Blocks in Functions/Services**
...
**🚨 CRITICAL:** ResponseDTOHelper provides standardized JSON→DTO mapping. ResponseDTOHelper is MANDATORY for ALL Process Layers.

### 12.1 Response DTO Helper Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating ResponseDTOHelper methods, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I reviewed the System Layer Function's Handler to understand what `BaseResponseDTO` structure it returns?
2. ✅ **STOP AND VERIFY:** Have I reviewed the System Layer Function's response DTOs to understand the `data` property structure?
3. ✅ **STOP AND VERIFY:** Do I understand how the System Layer maps downstream API responses to its response DTOs?
4. ✅ **STOP AND VERIFY:** Have I read this section completely?
5. ✅ **STOP AND VERIFY:** Do I understand that ALL response mapping logic MUST be in `Helper/ResponseDTOHelper.cs`?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create ResponseDTOHelper methods**
...
**Key Requirements:**
- ✅ **MANDATORY:** ResponseDTOHelper MUST exist in ALL Process Layers
- ✅ **Location:** `Helper/ResponseDTOHelper.cs` (exact location)
- ✅ **Class:** `public static class ResponseDTOHelper`
- ✅ **Methods:** All methods MUST be `public static`
- ✅ **Purpose:** Standardized JSON→DTO mapping for all System Layer and Process Layer responses
...
1. ✅ **MANDATORY:** ALWAYS use `Dictionary<string, object>` deserialization pattern
2. ✅ **MANDATORY:** ALWAYS use Framework extension methods (`ToStringValue()`, `ToIntValue()`, `ToDateTimeValue()`, `ToBoolValue()`, `ToDecimalValue()`, `ToLongValue()`)
3. ✅ **MANDATORY:** Use property names from System Layer DTO (PascalCase: `"<SystemLayerProperty>"`, `"<PropertyName>"`, etc.) - NOT from Process Layer DTO
4. ❌ **PROHIBITED:** NEVER create private classes inside ResponseDTOHelper for deserialization
5. ❌ **PROHIBITED:** NEVER use `JsonSerializer.Deserialize<T>()` with custom classes in ResponseDTOHelper
6. ❌ **PROHIBITED:** NEVER create temporary mapping classes (e.g., `private class <SystemLayerResDTO>`)
7. ❌ **PROHIBITED:** NEVER use `JsonSerializerOptions` with `PropertyNameCaseInsensitive` for deserializing to custom classes
...
**When to Use:**
- ✅ **ALWAYS** - This is the ONLY allowed pattern for ResponseDTOHelper methods
...
1. ✅ **ALL response mapping logic MUST be in `Helper/ResponseDTOHelper.cs`** - Functions/Services MUST NOT contain mapping logic
2. ✅ **All methods MUST be `public static`** - ResponseDTOHelper is a static utility class
3. ✅ **Use appropriate pattern** - Choose pattern based on JSON structure complexity
4. ✅ **Use DictionaryExtensions** - For flexible mapping, use Framework DictionaryExtensions (ToIntValue, ToStringValue, etc.)
5. ✅ **Use `nameof()` for property names** - Ensures type safety and refactoring support
6. ✅ **Handle null cases** - Always check for null before accessing dictionary values
7. ❌ **NEVER put mapping logic in Functions** - All mapping must be in ResponseDTOHelper
8. ❌ **NEVER use manual JSON parsing in Functions** - Use ResponseDTOHelper methods

### 12.8 Response DTO Helper Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating ResponseDTOHelper methods, you MUST verify:**

1. ✅ **STOP AND VERIFY:** ResponseDTOHelper is located in `Helper/ResponseDTOHelper.cs`
2. ✅ **STOP AND VERIFY:** All methods are `public static`
3. ✅ **STOP AND VERIFY:** Methods use appropriate pattern (Direct Deserialization, Dictionary Extraction, Complex Nested, or Response Enrichment)
4. ✅ **STOP AND VERIFY:** Methods handle null cases properly
5. ✅ **STOP AND VERIFY:** Methods use `nameof()` for property names (if using Dictionary pattern)
6. ✅ **STOP AND VERIFY:** Methods use DictionaryExtensions from Framework (if using Dictionary pattern)
7. ✅ **STOP AND VERIFY:** Functions/Services use ResponseDTOHelper methods (NOT manual JSON parsing)

**🛑 IF ANY VIOLATION → STOP → Fix ResponseDTOHelper → Then proceed**

### 12.9 Response DTO Helper Rules - Common Mistakes

**❌ Putting Mapping Logic in Functions**
...
**❌ Manual JSON Parsing in Functions**
...
**❌ Not Using DictionaryExtensions**
...
**❌ Not Using nameof() for Property Names**
...
**❌ Not Understanding System Layer Response Structure**
...
**🚨 CRITICAL:** AppConfigs centralize all configuration values. ALL configuration values MUST be in AppConfigs, NEVER hardcoded.

**🔴 MANDATORY CODE GENERATION CHECKLIST - APPsettings UPDATES (BLOCKING):**

**🚨 THIS IS STEP 8 OF THE MANDATORY CODE GENERATION WORKFLOW (SEE TOP OF DOCUMENT)**

**When you reach this step in code generation, you MUST:**

1. **MANDATORY:** Read AppConfigs.cs → List ALL new properties added
...
4. **🔴 CRITICAL - BLOCKING:** Verify ALL appsettings VALUES against Boomi JSON files (source of truth):
   - [ ] **BLOCKING:** For EACH property in appsettings that has a known value (not TODO placeholder):
     - Identify which Boomi process/operation this property belongs to
     - Read Phase 1 document Section 2 (Process Properties Analysis) OR Section 22 (Process-Level Configuration) to find source
     - Read Boomi component JSON file (component_*.json) → Locate `DefinedProcessProperties/definedProcessProperty[]/defaultValue/_` field
...
**🚨 CRITICAL:** This step is BLOCKING - you CANNOT complete code generation without updating ALL appsettings files. This is NOT optional.

**🔴 CROSS-REFERENCE:** This is part of the complete workflow. See "MANDATORY CODE GENERATION WORKFLOW" at the top of this document for the full sequence.
...
- **For fixed/hardcoded values found in contracts or Boomi JSON files:**
  ...
  2. **IF value exists in Phase 1 Section 2:** Use that ACTUAL value (NOT placeholder) in ALL AppConfig files - ❌ **NEVER use placeholder if value is known**
  3. **IF value does NOT exist in Phase 1 Section 2:** Check Boomi component JSON file (`component_*.json`) for default value
  4. **IF value exists in component JSON:** Use that ACTUAL value (NOT placeholder) in ALL AppConfig files - ❌ **NEVER use placeholder if value is known**
  5. **IF value does NOT exist in Phase 1 OR component JSON:** Add placeholder in ALL AppConfig files (e.g., `"TODO_REPLACE_WITH_<PROPERTY_NAME>"` or `"TODO_REPLACE_WITH_EMAIL"` for email properties) - ✅ **Placeholders are ONLY for unknown values**

**🔴 CRITICAL: Value vs Placeholder Decision:**
- **Value exists in Phase 1 OR component JSON?** → Use ACTUAL value in all files (e.g., `"BoomiIntegrationTeam@al-ghurair.com"`)
- **Value does NOT exist in Phase 1 AND component JSON?** → Use placeholder in all files (e.g., `"TODO_REPLACE_WITH_EMAIL"`)
- **Example:** Phase 1 shows `To_Email: "BoomiIntegrationTeam@al-ghurair.com"` → Use `"BoomiIntegrationTeam@al-ghurair.com"` in all files, NOT `"TODO_REPLACE_WITH_EMAIL"`
...
**🚨 BEFORE creating AppConfigs, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Have I checked Phase 1 document (Section 2: Process Properties Analysis) for all configuration properties?
3. ✅ **STOP AND VERIFY:** Have I checked Boomi component JSON files for default values?
4. ✅ **STOP AND VERIFY:** Have I verified which layer needs each configuration (Process Layer vs System Layer)?
5. ✅ **STOP AND VERIFY:** Do I understand the Decision Framework (AppConfigs vs Request DTO)?
6. ✅ **STOP AND VERIFY:** Have I verified I'm NOT adding SOR base URLs or resource paths to Process Layer AppConfigs?
7. ✅ **🔴 CRITICAL - STOP AND VERIFY:** Do I understand that `appsettings.json` must be kept EMPTY because the pipeline fills it during deployment?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create AppConfigs**

### 13.2 Config Models Rules - Decision Framework: AppConfigs vs Request DTO

**🔴 CRITICAL:** Use this decision framework to determine if a value should be in AppConfigs (static configuration) or Request DTO (user-provided data).

**🚨 PRIMARY RULE - BOOMI PARITY (MANDATORY):**

**The decision MUST be based on what Boomi does to maintain 1:1 behavioral parity:**

1. **✅ AppConfigs:** If Boomi stores the value in:
...
2. **✅ Request DTO:** If Boomi has the value in:
...
**✅ AppConfigs (Static Configuration):**
...
**✅ Request DTO (User-Provided Data):**
...
5. **🔴 CRITICAL: Defined Parameters Used for Runtime Decisions (Decision Shapes):**
   ...
   - **Example:** `<DecisionProperty>` used in decision shape `if (<DecisionProperty> == "<Value>")` → use "<OperationA>", else → use "<OperationB>"
   ...
   - **❌ WRONG:** Adding `<DecisionProperty>` to AppConfigs when it's used in a decision shape
   - **✅ CORRECT:** Determine `<decisionProperty>` via code logic: `bool <decisionProperty> = !string.IsNullOrWhiteSpace(<dataField1>) && !string.IsNullOrWhiteSpace(<dataField2>);`
...
**Key Requirements:**
- ✅ **SectionName:** MUST be `public static string SectionName = "AppVariables";`
- ✅ **Properties:** Use appropriate types (string, int, Dictionary, nested classes)
- ✅ **Nested Configs:** Use separate classes for complex configurations
- ✅ **Default Values:** Use default values for optional properties (e.g., `= string.Empty`, `= new()`)
...
- ❌ **SOR base URLs** - Base URLs for external systems
- ❌ **SOR resource paths** - Resource paths for external systems
- ❌ **Any SOR URL construction components** - Any values used to construct SOR URLs
...
**🔴 CRITICAL: Operation-Specific Error Email Configuration Rule**

**🚨 MANDATORY RULE:** When multiple operations in the same Process Layer have different error email configuration values (file name prefix, has attachment flag) from Phase 1, these MUST be operation-specific properties in AppConfigs, NOT generic properties.
...
2. **Compare values across operations:**
   - If values are different → Operation-specific properties REQUIRED
   - If values are identical → Generic properties allowed (but operation-specific is still preferred for clarity)
...
**Implementation Pattern:**
```csharp
// ✅ CORRECT: Operation-specific properties (when values differ)
public class AppConfigs {
    public static string SectionName = "AppVariables";
...
```csharp
// ✅ CORRECT: Use operation-specific config in Function
...
**Example Violations:**
- ❌ **WRONG:** Generic `ErrorEmailFileNamePrefix` and `ErrorEmailHasAttachment` when operations have different values from Phase 1
- ❌ **WRONG:** Using same generic property for multiple operations with different Phase 1 values
- ❌ **WRONG:** Not checking Phase 1 Section 2 for operation-specific values
- ✅ **CORRECT:** Operation-specific properties when Phase 1 shows different values per operation
...
**Verification Steps:**
1. ✅ Check Phase 1 Section 2 for each operation's `process.DPP_File_Name` and `process.DPP_HasAttachment` values
2. ✅ Compare values across operations - if different, operation-specific properties REQUIRED
3. ✅ Verify AppConfigs has operation-specific properties (not generic) when values differ
4. ✅ Verify Functions use operation-specific properties (not generic) when values differ
5. ✅ Verify all appsettings files have operation-specific values populated from Phase 1
...
**Key Points:**
- ✅ Load `appsettings.json` first (base/fallback values)
- ✅ Load environment-specific file second (`appsettings.{environment}.json`)
- ✅ Add environment variables last (override config files)
- ✅ Use `Configure<T>()` to bind configuration sections
- ✅ Use `SectionName` static property for section name
...
**🔴 CRITICAL: Deployment Pipeline Behavior:**
- **`appsettings.json`** - **🔴 KEEP EMPTY** - Runtime file used by Azure Function App after deployment. The deployment pipeline automatically copies environment-specific appsettings into this file during deployment. Do NOT add values or placeholders to this file - the pipeline will overwrite them.
...
**Key Principles:**
- **`appsettings.json`** - **🔴 CRITICAL: Keep this file EMPTY or with minimal structure only** - The deployment pipeline automatically fills this file from environment-specific files during deployment. Do NOT add values or placeholders to `appsettings.json` - the pipeline will overwrite them.
...
| Component | Pattern | Mandatory |
|-----------|---------|-----------|
| **SectionName** | `public static string SectionName = "AppVariables";` | ✅ Always |
| **Class Name** | `AppConfigs` | ✅ Standard Process Layer |
| **Nested Configs** | Separate classes (e.g., `JwtOptions`, `KeyVaultConfigs`) | ⚠️ Central Data Layer only |
| **Injection** | `IOptions<AppConfigs>` | ✅ Always |
| **Registration** | `builder.Services.Configure<AppConfigs>(...)` | ✅ Always |
| **Environment Files** | `appsettings.json` (keep empty), `appsettings.dev.json`, `appsettings.qa.json`, `appsettings.stg.json`, `appsettings.prod.json`, `appsettings.dr.json` | ✅ All must exist |
| **URL Rule** | ONLY System Layer Function URLs, NEVER SOR URLs | ✅ **CRITICAL** |
| **Value vs Placeholder** | Use actual value if exists in Phase 1/component JSON, placeholder if unknown | ✅ **CRITICAL** |

**🔴 CRITICAL RULES:**

1. ✅ **ALL configuration values MUST be in AppConfigs** - NEVER hardcode values
2. ✅ **Use actual values if they exist in Phase 1 or component JSON** - NEVER use placeholder if value is known
3. ✅ **Process Layer AppConfigs URLs MUST ONLY contain System Layer Function URLs** - NEVER SOR URLs
4. ✅ **🔴 CRITICAL: Keep `appsettings.json` EMPTY** - Pipeline fills it during deployment. Add all values to environment-specific files (`appsettings.dev.json`, `appsettings.qa.json`, `appsettings.stg.json`, `appsettings.prod.json`, `appsettings.dr.json`)
5. ✅ **All properties must exist in ALL environment-specific files** - Same structure in all environment-specific files, different values per environment
6. ✅ **Inject via `IOptions<AppConfigs>`** - Always use dependency injection
7. ✅ **Use Decision Framework** - AppConfigs vs Request DTO based on Boomi parity
8. ✅ **Verify which layer needs configuration** - Process Layer vs System Layer

### 13.10 Config Models Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating AppConfigs, you MUST verify:**

1. ✅ **STOP AND VERIFY:** AppConfigs class has `public static string SectionName = "AppVariables";`
2. ✅ **STOP AND VERIFY:** AppConfigs properties match Phase 1 Section 2 (defined parameters)
3. ✅ **🔴 CRITICAL - STOP AND VERIFY:** `appsettings.json` is kept EMPTY (or minimal structure only) - Pipeline fills it during deployment
4. ✅ **STOP AND VERIFY:** All properties are added to ALL environment-specific files (`appsettings.dev.json`, `appsettings.qa.json`, `appsettings.stg.json`, `appsettings.prod.json`, `appsettings.dr.json`)
5. ✅ **🔴 CRITICAL - BLOCKING:** Verify ALL appsettings VALUES against Boomi JSON files (source of truth):
   - [ ] **BLOCKING:** For EACH property in appsettings that has a known value (not TODO placeholder):
     - Identify which Boomi process/operation this property belongs to
     - Read Phase 1 document Section 2 (Process Properties Analysis) OR Section 22 (Process-Level Configuration) to find source
     - Read Boomi component JSON file (component_*.json) → Locate `DefinedProcessProperties/definedProcessProperty[]/defaultValue/_` field
   ...
   - [ ] **BLOCKING:** Document verification results: For EACH property, document:
     ...
     - Actual value in appsettings
     - Match status (✅ MATCH / ❌ MISMATCH)
   - ✅ **CORRECT:** All appsettings values match Boomi component JSON defaultValue fields exactly (verified character-by-character)
   - ❌ **WRONG:** Appsettings values don't match Boomi component JSON values (even if close or similar)
6. ✅ **🔴 CRITICAL - MANDATORY CHECKPOINT:** Have I verified I'm NOT adding SOR base URLs, SOR resource paths, or any SOR URL construction components to Process Layer AppConfigs?
   - ✅ **CORRECT:** Process Layer AppConfigs contains ONLY System Layer Function URLs
   - ❌ **WRONG:** Process Layer AppConfigs contains SOR URLs, SOR base URLs, or SOR resource paths
7. ✅ **STOP AND VERIFY:** Values use actual values (if exists in Phase 1/component JSON) or placeholders (if unknown)
8. ✅ **🔴 CRITICAL - STOP AND VERIFY:** If value is unknown, placeholder string MUST be added (NOT empty string) - Empty values are NOT allowed for unknown configuration
9. ✅ **STOP AND VERIFY:** AppConfigs is registered in Program.cs: `builder.Services.Configure<AppConfigs>(...)`
10. ✅ **STOP AND VERIFY:** AppConfigs is injected via `IOptions<AppConfigs>` in Functions/Services/Abstractions
11. ✅ **STOP AND VERIFY:** All configuration values are read from `_appConfigs` (NOT hardcoded)
12. ✅ **STOP AND VERIFY:** `HttpClientPolicy.RetryCount` is `0` (not `1`) in all environment files unless explicitly required

**🛑 IF ANY VIOLATION → STOP → Fix AppConfigs → Then proceed**
...
**❌ Using Placeholders When Actual Value Exists**
...
**❌ Using Empty String for Unknown Values**
...
**❌ Not Adding Properties to All Environment Files**
...
**❌ Not Verifying Which Layer Needs Configuration**
...
**❌ Using Generic Error Email Configuration When Operations Have Different Values**
...
**🚨 CRITICAL:** Constants use standardized formats for error codes and string literals. Business logic string literals MUST use constants, NEVER hardcode strings. Exceptions: Function attribute names, stepName values, and logging messages are allowed as literal strings.

### 14.1 Constants Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating Constants, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Have I identified all operations that need error constants?
3. ✅ **STOP AND VERIFY:** Do I know the BusinessDomain abbreviation (3 uppercase chars, e.g., PRP for Properties, FCL for Facilities, AUT for Automotive, HRM for HumanResource)?
4. ✅ **STOP AND VERIFY:** Have I identified all string literals that need constants (process names, messages, email subjects, default values)?
5. ✅ **STOP AND VERIFY:** Do I understand the difference between ErrorConstants (error codes) and InfoConstants (string literals)?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Constants**

### 14.2 Constants Rules - Error Constants Format

**🔴 CRITICAL: Error Constants Format**
...
**🔴 CRITICAL: Process Layer vs System Layer Format:**
...
```csharp
// ❌ WRONG: Missing operation part
("PRP_001", ...)

// ❌ WRONG: Lowercase
("prp_crtent_0001", ...)

// ❌ WRONG: Operation too long (CREATEENTITY = 12 chars, max 6)
("PRP_CREATEENTITY_0001", ...)

// ❌ WRONG: BusinessDomain too long (PRPC = 4 chars, max 3)
("PRPC_CRTENT_0001", ...)

// ❌ WRONG: BusinessDomain too short (PR = 2 chars, must be 3)
("PR_CRTENT_0001", ...)
...
**🔴 CRITICAL: Info Constants Format**
...
**🔴 CRITICAL: Logging Message Pattern (MANDATORY)**

**🚨 MANDATORY RULE:** Logging messages MUST use literal strings, NEVER constants.

**Rule:**
- ✅ **CORRECT:** Use direct string literals in logging calls (e.g., `_logger.Error($"Operation failed: {errorMessage}")`)
- ❌ **WRONG:** Use constants in logging calls (e.g., `_logger.Error($"{InfoConstants.OPERATION_FAILED}: {errorMessage}")`)
...
- Logging messages are allowed as literal strings per validation rules (Step 4: "Is it logging message? → **ALLOWED** (literal string)")

**Examples:**
```csharp
// ✅ CORRECT: Literal strings in logging
_logger.Info("HTTP Request received for CreateLeave.");
_logger.Error($"D365 call failed: {errorFromD365.Message}");
_logger.Error($"Failed to send error notification email. Status: {emailResponse.StatusCode}. Error: {emailErrorMessage}");
...
- ✅ **Constants (InfoConstants.cs):** Business logic strings (process names, success messages, email subjects, default values)
- ✅ **Literal Strings (Logging):** All logging messages (`_logger.Info()`, `_logger.Error()`)

**🔴 MANDATORY: String Literals That MUST Be Constants:**
- ✅ **Process names** - Default process names used in email templates or logging
- ✅ **Default environment values** - Fallback environment names (e.g., "dev", "qa", "prod")
- ✅ **Default execution IDs** - Fallback values for execution IDs (e.g., "N/A")
- ✅ **Success messages** - All success messages returned in BaseResponseDTO
- ✅ **Email subjects** - Email subject lines for notifications
- ✅ **Email body placeholders** - Placeholder values used in email templates
- ✅ **Default values** - Any hardcoded default/fallback values
- ✅ **Step names** - Standardized step names for exceptions (if reused)
- ❌ **Log messages** - **NEVER create constants for logging string literals** - Use direct string literals in logging calls (e.g., `_logger.Info("Error notification email sent successfully.")`)

### 14.4 Constants Rules - Usage Patterns

**🔴 CRITICAL: Use Constants Instead of Hardcoded Strings (MANDATORY)**

**🚨 ALWAYS use constants files (InfoConstants.cs, ErrorConstants.cs) for string literals, NEVER hardcode strings directly in code.**

**❌ WRONG Patterns:**
```csharp
// ❌ WRONG: Hardcoded process name
string processName = Environment.GetEnvironmentVariable("PROCESS_NAME") ?? "<Operation>Function";

// ❌ WRONG: Hardcoded default environment
...
// ✅ CORRECT: Use constants for success messages
return new BaseResponseDTO(InfoConstants.<OPERATION>_SUCCESS, string.Empty, <Operation>Res);

// ✅ CORRECT: Use constants for email subjects
Subject = InfoConstants.ERROR_EMAIL_SUBJECT;

// ✅ CORRECT: Use RequestHeaders class for header names (NOT hardcoded)
RequestHeaders.RequestId.Item2.Value?.ToString()

// ✅ CORRECT: Use direct string literals for log messages (NEVER create constants for logging)
_logger.Info("HTTP Request received for CreateEntity.");
...
| Component | Pattern | Mandatory |
|-----------|---------|-----------|
| **Error Constants Format** | `(string ErrorCode, string Message)` tuple | ✅ Always |
| **Error Code Format** | `AAA_AAAAAA_DDDD` (BusinessDomain_Operation_Number) | ✅ Always |
| **BusinessDomain** | 3 uppercase chars (PRP, FCL, AUT, HRM, KAB) | ✅ Always |
| **Operation** | 6 uppercase chars (GETENT, CRTENT, UPDENT) | ✅ Always |
| **Number** | 4 digits (0001, 0002, 0003, ...) | ✅ Always |
| **Info Constants Format** | `public const string` | ✅ Always |
| **String Literals** | MUST use constants, NEVER hardcode | ✅ **CRITICAL** |
| **Header Names** | Use `RequestHeaders` class or constants, NEVER hardcoded | ✅ **CRITICAL** |

**🔴 CRITICAL RULES:**

1. ✅ **Business logic string literals MUST use constants** - NEVER hardcode strings in Functions, Services, or any code. Exceptions: Function attribute names, stepName values, and logging messages are allowed as literal strings.
2. ✅ **Error constants MUST use tuple format** - `(string ErrorCode, string Message)` NOT `(string Code, string Message)`
3. ✅ **Error code format MUST be `AAA_AAAAAA_DDDD`** - BusinessDomain (3 chars) + Operation (6 chars) + Number (4 digits)
4. ✅ **BusinessDomain abbreviation MUST be 3 uppercase characters** - Exactly 3 chars, no more, no less
5. ✅ **Operation abbreviation MUST be 6 uppercase characters** - Exactly 6 chars, no more, no less
6. ✅ **Error number MUST be 4 digits** - Sequential numbering (0001, 0002, 0003, ...)
7. ✅ **Header names MUST use `RequestHeaders` class or constants** - NEVER hardcoded header names
8. ✅ **Process names, messages, email subjects MUST use InfoConstants** - NEVER hardcode these values

### 14.7 Constants Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Constants, you MUST verify:**

**ErrorConstants.cs Validation:**
1. ✅ **STOP AND VERIFY:** Tuple format is `(string ErrorCode, string Message)` NOT `(string Code, string Message)`
2. ✅ **STOP AND VERIFY:** Error code format is `AAA_AAAAAA_DDDD` (exactly this pattern)
3. ✅ **STOP AND VERIFY:** AAA is exactly 3 chars - BusinessDomain abbreviation (e.g., PRP, FCL, AUT, HRM, KAB)
4. ✅ **STOP AND VERIFY:** AAAAAA is exactly 6 chars - Operation name abbreviation (e.g., GETENT, CRTENT, UPDENT)
5. ✅ **STOP AND VERIFY:** DDDD is exactly 4 digits (0001, 0002, etc.)
6. ✅ **STOP AND VERIFY:** All error codes are uppercase
7. ✅ **STOP AND VERIFY:** All constants use `readonly (string ErrorCode, string Message)`

**InfoConstants.cs Validation:**
8. ✅ **STOP AND VERIFY:** Business logic string literals use constants from InfoConstants.cs (exceptions: Function attribute names, stepName, logging messages)
9. ✅ **STOP AND VERIFY:** Process names use constants (NOT hardcoded)
10. ✅ **STOP AND VERIFY:** Default values use constants (NOT hardcoded)
11. ✅ **STOP AND VERIFY:** Success messages use constants (NOT hardcoded)
12. ✅ **STOP AND VERIFY:** Email subjects use constants (NOT hardcoded)
    13. ✅ **STOP AND VERIFY:** Log messages use direct string literals (NOT constants) - Logging string literals should NOT be in InfoConstants.cs

**Code Validation:**
14. ✅ **STOP AND VERIFY:** Run `grep -n "\"[A-Za-z]" <file>` → For EACH string literal found, verify it uses a constant
15. ✅ **STOP AND VERIFY:** Verify NO hardcoded header names (use `RequestHeaders` class or constants)
16. ✅ **STOP AND VERIFY:** Verify NO hardcoded process names, messages, or default values

**🛑 IF ANY VIOLATION → STOP → Fix Constants → Then proceed**

### 14.8 Constants Rules - Common Mistakes

**❌ Using Wrong Tuple Format**
...
**❌ Confusing ErrorConstants with InfoConstants**
...
**❌ Not Using Constants for Step Names**
...
**🚨 CRITICAL:** Enums use `[StringValue]` attribute for external values. Enums are used when values need to map to external string representations (e.g., SOR API values, Boomi contract values).

### 15.1 Enums Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating Enums, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Do I need an enum (does the value need to map to an external string representation)?
3. ✅ **STOP AND VERIFY:** Have I checked if Framework already provides the enum I need?
4. ✅ **STOP AND VERIFY:** Do I know the exact external string values that need to be mapped?
5. ✅ **STOP AND VERIFY:** Have I verified the external string values from Phase 1 document or Boomi JSON profile?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Enum**

### 15.2 Enums Rules - Structure & Pattern

**🔴 CRITICAL: Enum Pattern with StringValue Attribute**
...
**Key Rules:**
- ✅ **ALWAYS use `[StringValue]` attribute** for enum values that map to external strings
- ✅ **Enum name** uses PascalCase (e.g., `Primary`, `Secondary`)
- ✅ **StringValue** matches external string representation exactly (e.g., `"PRIMARY"`, `"SECONDARY"`)
- ✅ **StringValue values** are case-sensitive and must match external system values exactly

**When to Use Enums:**
- ✅ When values need to map to external string representations (SOR API values, Boomi contract values)
- ✅ When values are fixed/known and need type safety
- ✅ When values are reused across multiple operations

**When NOT to Use Enums:**
- ❌ For user-provided values (use string properties in DTOs)
- ❌ For configuration values (use AppConfigs)
- ❌ For values that change frequently (use string properties)

### 15.3 Enums Rules - Enum Extension Method

**🔴 CRITICAL: GetStringValue() Extension Method**
...
**Key Rules:**
- ✅ **ALWAYS use `GetStringValue()` extension method** to get string value from enum
- ✅ **ALWAYS add `using Core.Extensions;`** to access `GetStringValue()` method
- ✅ **If StringValue attribute is missing**, method returns `value.ToString()` as fallback
...
| Component | Pattern | Mandatory |
|-----------|---------|-----------|
| **StringValue Attribute** | `[StringValue("EXTERNAL_VALUE")]` | ✅ Always |
| **Enum Extension** | `GetStringValue()` from `Core.Extensions` | ✅ Always |
| **File Location** | `Enums/<EnumName>.cs` | ✅ Always |
| **Namespace** | `<ProjectName>.Enums` | ✅ Always |
| **StringValue Matching** | Must match external string exactly (case-sensitive) | ✅ **CRITICAL** |
| **Framework Extension** | Use Framework `EnumExtensions.GetStringValue()` | ✅ Always |

**🔴 CRITICAL RULES:**

1. ✅ **ALL enums MUST use `[StringValue]` attribute** - For mapping to external string representations
2. ✅ **ALWAYS use `GetStringValue()` extension method** - From `Core.Extensions.EnumExtensions`
3. ✅ **StringValue values MUST match external strings exactly** - Case-sensitive, must match SOR API or Boomi contract values
4. ✅ **One enum per file** - Standard C# convention, one enum class per file
5. ✅ **Use Framework extensions** - Do NOT create custom enum extension methods, use Framework `GetStringValue()`
6. ✅ **Verify external string values** - Check Phase 1 document or Boomi JSON profile for exact string values

### 15.6 Enums Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Enum, you MUST verify:**

1. ✅ **STOP AND VERIFY:** All enum values have `[StringValue]` attribute
2. ✅ **STOP AND VERIFY:** StringValue values match external string representations exactly (case-sensitive)
3. ✅ **STOP AND VERIFY:** Enum file is in `Enums/` folder with correct naming (`<EnumName>.cs`)
4. ✅ **STOP AND VERIFY:** Enum uses correct namespace (`<ProjectName>.Enums`)
5. ✅ **STOP AND VERIFY:** Code uses `GetStringValue()` extension method (NOT manual string conversion)
6. ✅ **STOP AND VERIFY:** Code includes `using Core.Extensions;` to access `GetStringValue()` method
7. ✅ **STOP AND VERIFY:** StringValue values are verified against Phase 1 document or Boomi JSON profile

**🛑 IF ANY VIOLATION → STOP → Fix Enum → Then proceed**

### 15.7 Enums Rules - Common Mistakes

**❌ Missing StringValue Attribute**

- **WRONG:** Enum without `[StringValue]` attribute
```csharp
public enum DataType {
    Primary,  // ❌ Missing [StringValue] attribute
    Secondary
}
```
...
**❌ Wrong StringValue Case or Value**
...
**❌ Not Using GetStringValue() Extension Method**
...
**❌ Creating Custom Enum Extension Methods**
...
**❌ Using Enums for User-Provided Values**
...
**❌ Using Enums for Configuration Values**
...
**🚨 CRITICAL:** Use Framework extensions; create project-specific extensions only if needed. Framework extensions are mandatory and must be used instead of direct API calls.

### 16.1 Extensions Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating Project-Specific Extensions, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Have I checked if Framework already provides this extension functionality?
3. ✅ **STOP AND VERIFY:** Is this extension truly domain-specific and not available in Framework?
4. ✅ **STOP AND VERIFY:** Do I understand the difference between Framework extensions (mandatory) and project-specific extensions (optional)?
5. ✅ **STOP AND VERIFY:** Am I using all required Framework extensions (LoggerExtensions, HttpRequestExtensions, HttpResponseExtensions)?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Extension**
...
**🔴 CRITICAL: Framework Extensions are MANDATORY**
...
// ❌ WRONG: Never use ILogger directly
...
- **Mandatory:** ✅ YES - Never use `ILogger` directly
...
- **Mandatory:** ✅ YES - Must use for all request body reading
...
// ❌ WRONG: Manual response extraction
...
- **Mandatory:** ✅ YES - Must use for all response data extraction

**4. StringExtensions (Use Framework Version)**
```csharp
// ✅ CORRECT: Use Framework extension methods
string clean = value.RemoveAllWhitespace();
bool valid = email.IsValidEmail();

// ❌ WRONG: Custom string manipulation
...
- **Mandatory:** ✅ YES - Use Framework version, don't create custom string extensions

**5. DictionaryExtensions (Use Framework Version)**
```csharp
// ✅ CORRECT: Use Framework extension methods (used in ResponseDTOHelper)
...
- **Mandatory:** ✅ YES - Use Framework version for dictionary operations

**6. EnumExtensions (Use Framework Version)**
```csharp
// ✅ CORRECT: Use Framework extension method
string type = DataType.Primary.GetStringValue();  // Returns "PRIMARY"
```
- **Location:** `Core/Extensions/EnumExtensions.cs`
- **Usage:** Use `GetStringValue()` for enum string value extraction
- **Mandatory:** ✅ YES - Use Framework version (see Section 15: Enums Rules)
...
```csharp
// ✅ CORRECT: Use extension method for HTTP calls
HttpResponseMessage response = await _customHttpClient.SendProcessHTTPReqAsync(
    method: HttpMethod.Post,
...
```
- **Location:** `Framework/Core/Core/ProcessLayer/Extensions/CustomHttpClientExtensions.cs`
- **Usage:** ALWAYS use `SendProcessHTTPReqAsync()` for Process→System and Process→Process HTTP calls
- **Mandatory:** ✅ YES - Must use for all HTTP calls (see Sections 5 and 6)
...
**⚠️ OPTIONAL: Project-Specific Extensions**
...
- ✅ Domain-specific business logic that cannot be generalized
- ✅ Project-specific data transformations not available in Framework
- ✅ Custom validation logic specific to the project

**When NOT to Create Project-Specific Extensions:**
- ❌ If Framework already provides similar functionality
- ❌ For general-purpose utilities (use Framework extensions)
- ❌ For operations that could be in Services or Helpers
...
|-----------|--------|-------|-----------|
| **LoggerExtensions** | Framework Core | `_logger.Info()`, `_logger.Error()`, `_logger.Warn()`, `_logger.Debug()` | ✅ **MANDATORY** - Never use ILogger directly |
| **HttpRequestExtensions** | Framework Core | `await req.ReadBodyAsync<T>()` | ✅ **MANDATORY** - Must use for request body reading |
| **HttpResponseExtensions** | Framework Core | `await response.ExtractDataAsync()`, `response.ExtractBaseResponse()` | ✅ **MANDATORY** - Must use for response extraction |
| **StringExtensions** | Framework Core | `RemoveAllWhitespace()`, `IsValidEmail()` | ✅ **MANDATORY** - Use Framework version |
| **DictionaryExtensions** | Framework Core | `ToIntValue()`, `ToStringValue()`, `ToDateTimeValue()` | ✅ **MANDATORY** - Use Framework version |
| **EnumExtensions** | Framework Core | `GetStringValue()` | ✅ **MANDATORY** - Use Framework version |
| **CustomHttpClientExtensions** | Framework Core | `SendProcessHTTPReqAsync()` | ✅ **MANDATORY** - Must use for HTTP calls |
| **Project Extensions** | Project/Extensions/ | Domain-specific extensions | ⚠️ **OPTIONAL** - Only if domain-specific need |

**🔴 CRITICAL RULES:**

1. ✅ **ALWAYS use Framework extensions** - Never use direct API calls when Framework extensions are available
2. ✅ **ALWAYS add `using Core.Extensions;`** - Required for all Framework extensions
3. ✅ **NEVER use ILogger directly** - Always use `Info()`, `Error()`, `Warn()`, `Debug()` extension methods
4. ✅ **NEVER manually read request body** - Always use `ReadBodyAsync<T>()` extension method
5. ✅ **NEVER manually extract response data** - Always use `ExtractDataAsync()` and `ExtractBaseResponse()` extension methods
6. ✅ **NEVER create custom extensions for Framework functionality** - Use Framework extensions instead
7. ✅ **Project-specific extensions are OPTIONAL** - Only create if Framework doesn't provide the functionality and it's domain-specific
8. ✅ **ALWAYS use `SendProcessHTTPReqAsync()`** - For all Process→System and Process→Process HTTP calls

### 16.6 Extensions Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER using Extensions, you MUST verify:**

**Framework Extensions Validation:**
1. ✅ **STOP AND VERIFY:** File has `using Core.Extensions;` statement
2. ✅ **STOP AND VERIFY:** Using `_logger.Info()`, `_logger.Error()`, etc. (NOT `LogInformation()`, `LogError()`)
3. ✅ **STOP AND VERIFY:** Using `await req.ReadBodyAsync<T>()` for request body reading
4. ✅ **STOP AND VERIFY:** Using `await response.ExtractDataAsync()` and `response.ExtractBaseResponse()` for response handling
5. ✅ **STOP AND VERIFY:** Using `SendProcessHTTPReqAsync()` for HTTP calls (NOT direct HttpClient calls)
6. ✅ **STOP AND VERIFY:** Using Framework StringExtensions, DictionaryExtensions, EnumExtensions (NOT custom versions)

**Project-Specific Extensions Validation (if created):**
7. ✅ **STOP AND VERIFY:** Extension is truly domain-specific and not available in Framework
8. ✅ **STOP AND VERIFY:** Extension file is in `Extensions/` folder with correct naming
9. ✅ **STOP AND VERIFY:** Extension uses correct namespace (`<ProjectName>.Extensions`)
10. ✅ **STOP AND VERIFY:** Extension follows Framework extension patterns (static class, extension methods)

**🛑 IF ANY VIOLATION → STOP → Fix Extensions → Then proceed**

### 16.7 Extensions Rules - Common Mistakes

**❌ Not Using Framework Extensions**
...
**❌ Creating Custom Extensions for Framework Functionality**
...
**❌ Creating Project Extensions for General-Purpose Operations**
...
**❌ Not Using SendProcessHTTPReqAsync() for HTTP Calls**
...
**🚨 CRITICAL:** ResponseDTOHelper is mandatory for ALL Process Layers. Additional helpers are optional and project-specific. Helpers provide reusable utility methods for common operations.

### 17.1 Helper Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating any Helper (except ResponseDTOHelper), you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Have I verified this helper is actually needed (not just convenient)?
3. ✅ **STOP AND VERIFY:** Have I checked if Framework/Core already provides this functionality?
4. ✅ **STOP AND VERIFY:** Have I checked if Extensions already provide this functionality?
5. ✅ **STOP AND VERIFY:** Is this truly a reusable utility that belongs in a helper (not business logic that belongs in a Service)?
6. ✅ **STOP AND VERIFY:** If email-related → Have I read Section 17.4 (Email Template Helpers)?
7. ✅ **STOP AND VERIFY:** Do I understand that ResponseDTOHelper is mandatory (see Section 12), but other helpers are optional?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create Helper**
...
**🔴 CRITICAL: ResponseDTOHelper is MANDATORY**
...
**🔴 CROSS-REFERENCE:** See Section 12 (Response DTO Helper Rules) for complete details on ResponseDTOHelper patterns, usage, and implementation.

### 17.3 Helper Rules - Additional Helper Classes (Optional)

**⚠️ OPTIONAL: Project-Specific Helpers**
...
- ✅ Reusable utility methods that don't belong in Services (no business logic)
- ✅ Common operations used across multiple Functions/Services
- ✅ File/PDF processing, document manipulation
- ✅ Type conversion utilities not available in Framework
- ✅ Format transformers
- ✅ Error message formatting, localization helpers
- ✅ Email template reading (see Section 17.4)

**When NOT to Create Additional Helpers:**
- ❌ If Framework Extensions already provide the functionality
- ❌ If the logic belongs in a Service (business logic)
- ❌ If it's a one-time operation (not reusable)
- ❌ If it's just convenient (not truly needed)
...
**Key Rules:**
- ✅ **All methods MUST be `public static`** - Helpers are stateless utility classes
- ✅ **No business logic** - Business logic belongs in Services, not Helpers
- ✅ **Reusable operations only** - Helpers should contain operations used multiple times
- ✅ **One helper class per file** - Standard C# convention

**File Structure:**
```
<ProjectName>/
├── Helper/
│   ├── ResponseDTOHelper.cs        # MANDATORY
│   ├── EmailTemplateHelper.cs      # Optional (if email templates needed)
│   ├── PDFHelper.cs                # Optional (if PDF generation needed)
│   └── <CustomHelper>.cs           # Optional (project-specific)
```

### 17.4 Helper Rules - Email Template Helpers

**🔴 MANDATORY: If you are implementing email functionality, you MUST read this section BEFORE writing any email code.**

**🛑 BEFORE implementing email body generation (MANDATORY - STOP IF NOT COMPLETE):**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Have I read Phase 1 Section 9 (Subprocess Analysis) to identify email subprocess?
3. ✅ **STOP AND VERIFY:** Have I located the Boomi subprocess JSON file (subprocess_*.json) for the email subprocess?
4. ✅ **STOP AND VERIFY:** Have I searched the subprocess JSON for email template (look for "msgTxt" in message shapes or "MailBody" in documentproperties shapes)?
5. ✅ **STOP AND VERIFY:** Have I extracted the EXACT email template HTML from Boomi subprocess JSON (including DOCTYPE, html, head, body tags, all whitespace)?
6. ✅ **STOP AND VERIFY:** Have I verified placeholder numbering from msgParameters section (e.g., {1}, {2}, {3}, {4})?
7. ✅ **STOP AND VERIFY:** Have I verified placeholder order matches msgParameters key order (check key values and their corresponding process properties)?
8. ✅ **STOP AND VERIFY:** Have I verified I'm NOT hardcoding email HTML in Function code?
9. ✅ **STOP AND VERIFY:** Do I understand that email templates MUST be embedded resources?
10. ✅ **STOP AND VERIFY:** Have I checked "COMMON MISTAKES" #31 (Hardcoded Configuration Values)?
11. ✅ **STOP AND VERIFY:** Do I understand that email addresses come from AppConfigs (see Section 13)?

**🛑 IF ANY CHECKPOINT FAILS → STOP → Complete missing step → Then implement email templates**

**🔴 CRITICAL: Email Template Extraction from Boomi Subprocess JSON (MANDATORY)**
...
- Example: If Boomi msgParameters shows {1}=ProcessName, {2}=Environment, {3}=ExecutionID, {4}=ErrorMessage
  - ✅ CORRECT: `string.Format(template, processName, environment, executionId, errorMessage)`
  - ❌ WRONG: `string.Format(template, environment, executionId, errorMessage)` (missing processName, wrong order)
...
- ✅ **Helper methods should handle missing template errors gracefully** - Throw `FileNotFoundException` if template not found
- ✅ **Email HTML MUST NOT be hardcoded** - Always use embedded templates
- ✅ **Email addresses come from AppConfigs** - Never hardcode email addresses (see Section 13)
- ✅ **🔴 CRITICAL:** Email template HTML MUST be extracted EXACTLY from Boomi subprocess JSON file (subprocess_*.json) - locate in msgTxt section of message shapes
- ✅ **🔴 CRITICAL:** Email template placeholders MUST match Boomi subprocess placeholders EXACTLY (verify placeholder numbering and order from msgParameters section in subprocess JSON)
- ✅ **🔴 CRITICAL:** Placeholder population parameter order MUST match Boomi subprocess msgParameters key order (verify parameter order in string.Format matches msgParameters)

**🔴 CRITICAL: Execution ID for Email Templates:**
...
<ProjectName>/
├── Helper/
│   ├── ResponseDTOHelper.cs        # MANDATORY - Response mapping
│   ├── EmailTemplateHelper.cs      # Optional - Email template reading
│   └── <CustomHelper>.cs           # Optional - Project-specific helpers
```
...
| Component | Pattern | Mandatory |
|-----------|---------|-----------|
| **ResponseDTOHelper** | `Helper/ResponseDTOHelper.cs` | ✅ **MANDATORY** - ALL Process Layers |
| **Additional Helpers** | `Helper/<HelperName>Helper.cs` | ⚠️ **OPTIONAL** - Only if needed |
| **Class Structure** | `public static class <HelperName>` | ✅ Always |
| **Methods** | `public static` methods | ✅ Always |
| **Business Logic** | ❌ NEVER in Helpers | ✅ **CRITICAL** - Business logic belongs in Services |
| **Framework Extensions** | Use Framework extensions first | ✅ Always - Check Framework before creating helpers |

**🔴 CRITICAL RULES:**

1. ✅ **ResponseDTOHelper is MANDATORY** - ALL Process Layers MUST have `Helper/ResponseDTOHelper.cs`
2. ✅ **All helper methods MUST be `public static`** - Helpers are stateless utility classes
3. ✅ **NO business logic in Helpers** - Business logic belongs in Services, not Helpers
4. ✅ **Check Framework Extensions first** - Don't create helpers for functionality available in Framework
5. ✅ **Helpers are for reusable utilities** - Only create if operation is used multiple times
6. ✅ **Email templates MUST be embedded resources** - Never hardcode email HTML
7. ✅ **Email addresses come from AppConfigs** - Never hardcode email addresses
8. ✅ **Execution ID from RequestHeaders** - Use `RequestHeaders.RequestId.Item2.Value?.ToString()` for email templates

### 17.7 Helper Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Helper, you MUST verify:**

**ResponseDTOHelper Validation:**
1. ✅ **STOP AND VERIFY:** ResponseDTOHelper exists at `Helper/ResponseDTOHelper.cs`
2. ✅ **STOP AND VERIFY:** ResponseDTOHelper class is `public static class ResponseDTOHelper`
3. ✅ **STOP AND VERIFY:** All methods are `public static`
4. ✅ **STOP AND VERIFY:** ResponseDTOHelper methods handle JSON→DTO mapping correctly

**Additional Helpers Validation (if created):**
5. ✅ **STOP AND VERIFY:** Helper is truly needed (not just convenient)
6. ✅ **STOP AND VERIFY:** Framework doesn't already provide this functionality
7. ✅ **STOP AND VERIFY:** Helper contains NO business logic (only utility methods)
8. ✅ **STOP AND VERIFY:** All methods are `public static`
9. ✅ **STOP AND VERIFY:** Helper file is in `Helper/` folder with correct naming

**Email Template Helpers Validation (if created):**
10. ✅ **STOP AND VERIFY:** Email templates are marked as `EmbeddedResource` in .csproj
11. ✅ **STOP AND VERIFY:** Resource name format is correct: `<Namespace>.EmailTemplate.<FileName>.html`
12. ✅ **STOP AND VERIFY:** Helper method handles missing template errors gracefully
13. ✅ **STOP AND VERIFY:** Execution ID uses `RequestHeaders.RequestId.Item2.Value?.ToString()`
14. ✅ **STOP AND VERIFY:** Email addresses come from AppConfigs (NOT hardcoded)

**🛑 IF ANY VIOLATION → STOP → Fix Helper → Then proceed**

### 17.8 Helper Rules - Common Mistakes

**❌ Creating Helpers for Business Logic**
...
**❌ Creating Helpers When Framework Extensions Exist**
...
**❌ Using Wrong Execution ID Source**
...
**❌ Not Making Helper Methods Static**
...
- **Why:** ResponseDTOHelper provides standardized JSON→DTO mapping. All response mapping logic MUST be in ResponseDTOHelper, not in Functions.

**❌ Creating Unnecessary Helpers**
...
**🚨 CRITICAL:** Program.cs handles dependency injection (DI) registration, configuration loading, and middleware setup. DI registration order is mandatory. System/Process Abstractions MUST be registered with interfaces.

### 18.1 Program.cs Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 YOU CANNOT CREATE/MODIFY Program.cs UNTIL:**

1. ✅ **STOP AND VERIFY:** Have I created all System Abstractions?
2. ✅ **STOP AND VERIFY:** Have I created all Process Abstractions (if needed)?
3. ✅ **STOP AND VERIFY:** Have I created all Services (if needed)?
4. ✅ **STOP AND VERIFY:** Have I created AppConfigs?
5. ✅ **STOP AND VERIFY:** Have I read this section completely?
6. ✅ **STOP AND VERIFY:** Do I understand the DI registration order?
7. ✅ **STOP AND VERIFY:** Do I understand that System/Process Abstractions MUST be registered with interfaces?

**🛑 IF ANY MISSING → STOP → Complete missing components → Then create/modify Program.cs**
...
2. **System/Process Abstractions: ALWAYS register and inject via interface**
   - ✅ `builder.Services.AddScoped<IEntityMgmt, EntityMgmtSys>()`
   - ❌ `builder.Services.AddScoped<EntityMgmtSys>()` - WRONG
   - ✅ `private readonly IEntityMgmt _entityMgmt` - CORRECT
   - ❌ `private readonly EntityMgmtSys _entityMgmt` - WRONG
3. **Services: Can be registered with or without interface**
   - ✅ `builder.Services.AddScoped<EntityService>()` (direct injection)
   - ✅ `builder.Services.AddScoped<IEntityService, EntityService>()` (with interface)
   - ✅ `private readonly EntityService _service` (direct injection)
   - ✅ `private readonly IEntityService _service` (interface injection)
4. **CALL FLOW MANDATORY PATTERN:**
   - Functions → Service (inject directly OR via interface)
   - Service → System Abstraction (via interface: `IEntityMgmt`) OR Process Abstraction (via interface: `IEntityProc`)
   - System/Process Abstraction → System Layer OR Process Layer (HTTP calls)
5. **🔴 CRITICAL: Process Layer Program.cs MUST NOT register System Layer services/handlers** - ONLY System Abstractions

### 18.3 Program.cs Rules - Registration Order (Standard Process Layer)

**🔴 CRITICAL:** Process Layer Program.cs MUST NOT register System Layer services/handlers. ONLY System Abstractions.
...
// ✅ ALWAYS register with interface
// 🔴 CRITICAL: ONLY System Abstractions here. NEVER System Layer services/handlers.
...
// ❌ NEVER DO THIS (System Layer components):
// builder.Services.AddScoped<ISystemLayerService, SystemLayerService>();  // ❌ WRONG
// builder.Services.AddScoped<SystemLayerHandler>();  // ❌ WRONG
...
// ✅ Can register with or without interface
...
**🔴 CRITICAL: Central Data Layer (Shared Services) Registration Pattern**
...
**🚨 IMPORTANT:** These patterns are documented with precise implementation details. Follow these patterns EXACTLY as shown.
...
|-------|-----------|----------|------------------------|-------------------|
| 1 | HTTP Client | N/A | ✅ AddHttpClient | ✅ AddHttpClient |
| 2 | Environment & Config Loading | N/A | ✅ | ✅ |
| 3 | Application Insights | N/A | ✅ | ✅ |
| 4 | Configuration Binding | N/A | ✅ Configure<AppConfigs> | ✅ Multiple Configure |
| 5 | Domains | N/A | ❌ NOT registered | ❌ NOT registered |
| 6 | Redis Caching | N/A | ✅ AddRedisCacheLibrary | ✅ (after KeyVault) |
| 7 | System Abstractions | Scoped/Cached | ✅ AddScoped/AddCachedService | ✅ AddScoped |
| 8 | Process Abstractions | Scoped | ⚠️ Optional | ⚠️ Optional |
| 9 | Services | Scoped/Transient | ⚠️ Optional | ✅ |
| 10 | CustomHTTPClient | Scoped | ✅ | ✅ |
| 11 | Polly Policies | Singleton | ✅ Optional | ✅ Optional |
| 12 | Repositories | Scoped | ❌ | ✅ |
| 13 | AutoMapper | Singleton | ❌ | ✅ |
| 14 | KeyVault | Singleton | ❌ | ✅ |
| 15 | DbContext | Scoped | ❌ | ✅ |
| 16 | ConfigureFunctionsWebApplication | N/A | ✅ | ✅ |
| 17 | Middleware Registration | N/A | ✅ (2 only) | ✅ (2 only) |
| 18 | ServiceLocator | N/A | ✅ | ✅ |

**🔴 CRITICAL RULES:**

1. ✅ **DI registration order is mandatory** - Components must be registered in the exact order shown
2. ✅ **System/Process Abstractions MUST be registered with interface** - Always use `AddScoped<IAbstraction, AbstractionImpl>()`
3. ✅ **Services can be registered with or without interface** - Both patterns are valid
4. ✅ **Domains are NOT registered in DI** - Instantiate directly: `EntityDomain domain = new EntityDomain();`
5. ✅ **Process Layer Program.cs MUST NOT register System Layer services/handlers** - ONLY System Abstractions
6. ✅ **Middleware order is non-negotiable** - ExecutionTimingMiddleware FIRST, ExceptionHandlerMiddleware SECOND
7. ✅ **ServiceLocator MUST be set** - Required for nested domain object resolution in DTO.Populate()
8. ✅ **Use constants for default environment** - Use `InfoConstants.DEFAULT_ENVIRONMENT` instead of hardcoded "dev"

### 18.7 Program.cs Rules - Update Checklist for New Operations

**🔴 MANDATORY: When adding a new operation, you MUST update Program.cs:**

**Registration Checklist:**
1. ✅ **Domain:** `builder.Services.AddTransient<<Entity>>();`
...
2. ✅ **System Abstraction:** `builder.Services.AddScoped<<Entity>MgmtSys>();`
   ...
   - **🔴 CRITICAL:** If caching is used, use `AddCachedService` instead of `AddScoped`

3. ✅ **Service:** `builder.Services.AddScoped<<Entity>Service>();`
   - **When:** Always register Service
   - **Lifetime:** `AddScoped` (one instance per request scope)
   - **Example:** `builder.Services.AddScoped<<Entity>Service>();`

4. ✅ **Verify AppConfigs Registration:** `builder.Services.Configure<AppConfigs>(builder.Configuration.GetSection(AppConfigs.SectionName));`
...
**🔴 CROSS-REFERENCE:** This is STEP 9 of the "MANDATORY CODE GENERATION WORKFLOW" at the top of this document.

### 18.8 Program.cs Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating/modifying Program.cs, you MUST verify:**

1. ✅ **STOP AND VERIFY:** NO System Layer project references in .csproj
2. ✅ **STOP AND VERIFY:** NO System Layer services registered (only System Abstractions)
3. ✅ **STOP AND VERIFY:** All System Abstractions registered with interface: `AddScoped<ISystemMgmt, SystemMgmtSys>()`
4. ✅ **STOP AND VERIFY:** All Process Abstractions registered with interface: `AddScoped<IProcessProc, ProcessProc>()`
5. ✅ **STOP AND VERIFY:** All Services registered (with or without interface)
6. ✅ **STOP AND VERIFY:** CustomHTTPClient registered: `AddScoped<CustomHTTPClient>()`
7. ✅ **STOP AND VERIFY:** AppConfigs configured: `Configure<AppConfigs>(...)`
8. ✅ **STOP AND VERIFY:** Middleware order: ExecutionTiming → ExceptionHandler (NO CustomAuthenticationMiddleware)
9. ✅ **STOP AND VERIFY:** ServiceLocator set: `ServiceLocator.ServiceProvider = builder.Services.BuildServiceProvider()`
10. ✅ **STOP AND VERIFY:** Domains are NOT registered in DI (instantiated directly)
11. ✅ **STOP AND VERIFY:** Environment variable uses constant: `InfoConstants.DEFAULT_ENVIRONMENT` (NOT hardcoded "dev")
12. ✅ **STOP AND VERIFY:** Registration order matches the template exactly

**🛑 IF ANY VIOLATION → STOP → Fix Program.cs → Then proceed**

### 18.8 Program.cs Rules - Common Mistakes

**❌ Registering System Layer Services in Process Layer Program.cs**
...
**❌ Registering System/Process Abstractions Without Interface**
...
**❌ Hardcoding Environment Default**
...
**🚨 CRITICAL:** Azure Functions host runtime configuration. Use EXACTLY the mandatory template format. NO deviations. NO additional properties.

### 19.1 host.json Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating/modifying host.json, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Do I understand that host.json format is EXACT and MANDATORY?
3. ✅ **STOP AND VERIFY:** Do I understand that host.json is the SAME for ALL environments (dev/qa/prod)?
4. ✅ **STOP AND VERIFY:** Do I understand that app configs go in appsettings.json, NOT host.json?
5. ✅ **STOP AND VERIFY:** Do I understand that retry policies go in Program.cs (Polly), NOT host.json?

**🛑 IF ANY MISSING → STOP → Complete verification → Then create/modify host.json**

### 19.2 host.json Rules - Standard Template (Mandatory - Exact Format)

**🔴 CRITICAL: Use EXACTLY this format. NO deviations. NO additional properties.**
...
**🔴 CRITICAL:** This is the EXACT format. Use it EXACTLY as shown. Do NOT add properties. Do NOT remove properties. Do NOT modify values.
...
| **fileLoggingMode** | `"always"` | Log to file system (debugging) | ✅ **MANDATORY** |
| **isEnabled** | `true` | Enable sampling for Application Insights | ✅ **MANDATORY** |
| **enableLiveMetricsFilters** | `true` | Real-time metrics in Azure Portal | ✅ **MANDATORY** |
...
**Key Rules:**
- ✅ **Same level as Program.cs** - host.json must be at project root level
- ✅ **Same level as appsettings.json** - All configuration files at same level
- ✅ **Content + Copy to Output Directory** - File must be included in build output

### 19.5 host.json Rules - Key Rules

| Aspect | Rule | Mandatory |
|--------|------|-----------|
| **Format** | EXACT template format (no deviations) | ✅ **CRITICAL** |
| **Version** | `"2.0"` (for .NET 8 Isolated Worker Model) | ✅ **MANDATORY** |
| **Environments** | Same host.json for ALL environments (dev/qa/stg/prod/dr) | ✅ **MANDATORY** |
| **File Location** | Project root (same level as Program.cs) | ✅ **MANDATORY** |
| **Build Action** | Content + Copy to Output Directory | ✅ **MANDATORY** |
| **Additional Properties** | ❌ NEVER add additional properties | ✅ **CRITICAL** |
| **Environment-Specific Files** | ❌ NEVER create host.dev.json or host.prod.json | ✅ **CRITICAL** |

**🔴 CRITICAL RULES:**

1. ✅ **Use EXACT template format** - NO deviations, NO additional properties, NO modifications
2. ✅ **Same host.json for ALL environments** - dev, qa, stg, prod, dr all use the same file
3. ✅ **File must be Content + Copy to Output Directory** - Required for deployment
4. ✅ **Version MUST be "2.0"** - Required for .NET 8 Isolated Worker Model
5. ❌ **NEVER add app configs to host.json** - Use appsettings.json for application configuration
6. ❌ **NEVER create environment-specific host.json files** - host.dev.json, host.prod.json are PROHIBITED
7. ❌ **NEVER add retry policies to host.json** - Use Polly policies in Program.cs for retry logic
8. ❌ **NEVER modify the template** - Use it EXACTLY as shown
...
- ❌ **DO NOT add app configs** - Use `appsettings.json` for application configuration
- ❌ **DO NOT add connection strings** - Use `appsettings.json` or AppConfigs
- ❌ **DO NOT add URLs or endpoints** - Use `appsettings.json` or AppConfigs

**2. Environment-Specific Files:**
- ❌ **DO NOT create `host.dev.json`** - Same host.json for all environments
- ❌ **DO NOT create `host.prod.json`** - Same host.json for all environments
- ❌ **DO NOT create `host.qa.json`** - Same host.json for all environments

**3. Retry Policies:**
- ❌ **DO NOT add retry policies** - Use Polly policies in Program.cs (see Section 18)
- ❌ **DO NOT add timeout configurations** - Use Polly timeout policies in Program.cs

**4. Custom Settings:**
- ❌ **DO NOT add custom properties** - Use the EXACT template only
- ❌ **DO NOT modify existing properties** - Use the EXACT template values
...
**🚨 AFTER creating/modifying host.json, you MUST verify:**

1. ✅ **STOP AND VERIFY:** host.json uses EXACT template format (no deviations)
2. ✅ **STOP AND VERIFY:** version is `"2.0"` (for .NET 8 Isolated Worker Model)
3. ✅ **STOP AND VERIFY:** fileLoggingMode is `"always"`
4. ✅ **STOP AND VERIFY:** isEnabled is `true` (Application Insights sampling)
5. ✅ **STOP AND VERIFY:** enableLiveMetricsFilters is `true`
6. ✅ **STOP AND VERIFY:** NO additional properties added
7. ✅ **STOP AND VERIFY:** File is at project root (same level as Program.cs)
8. ✅ **STOP AND VERIFY:** File properties: Content + Copy to Output Directory
9. ✅ **STOP AND VERIFY:** NO environment-specific host.json files created (host.dev.json, host.prod.json, etc.)
10. ✅ **STOP AND VERIFY:** NO app configs added to host.json (use appsettings.json)
11. ✅ **STOP AND VERIFY:** NO retry policies added to host.json (use Polly in Program.cs)

**🛑 IF ANY VIOLATION → STOP → Fix host.json → Then proceed**

### 19.8 host.json Rules - Common Mistakes

**❌ Modifying the Template Format**
...
**❌ Creating Environment-Specific host.json Files**
...
**❌ Adding Application Configuration to host.json**
...
**❌ Adding Retry Policies to host.json**
...
**🚨 CRITICAL:** Use layer-appropriate exceptions. NEVER use try-catch blocks. Let all exceptions propagate to middleware for proper error handling.

### 20.1 Exception Handling Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE throwing exceptions, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Do I understand that try-catch blocks are PROHIBITED?
3. ✅ **STOP AND VERIFY:** Do I understand which exception type to use for my scenario?
4. ✅ **STOP AND VERIFY:** Do I have the correct stepName format?
5. ✅ **STOP AND VERIFY:** Do I have errorDetails (if required by exception type)?
6. ✅ **STOP AND VERIFY:** Do I understand that ExceptionHandlerMiddleware handles all exceptions?

**🛑 IF ANY MISSING → STOP → Complete verification → Then throw exception**

### 20.2 Exception Handling Rules - Critical Rule: Never Use Try-Catch

**🔴 CRITICAL RULE - NEVER USE TRY-CATCH:**

**❌ PROHIBITED:**
- ❌ **NEVER use `try-catch` blocks** in Services, Functions, or any Process Layer code
- ❌ **NEVER catch exceptions** - Let all exceptions propagate to middleware
- ❌ **NEVER suppress exceptions** - ExceptionHandlerMiddleware handles all exception normalization
- ❌ **NEVER handle exceptions manually** - ExceptionHandlerMiddleware normalizes all exceptions to BaseResponseDTO

**✅ CORRECT PATTERN:**
- ✅ **Always throw exceptions** - Let exceptions propagate naturally
- ✅ **Middleware handles exceptions** - ExceptionHandlerMiddleware catches all exceptions and normalizes to BaseResponseDTO
- ✅ **Log before throwing** - Use `_logger.Error()` before throwing exceptions for debugging
- ✅ **Check response status** - Check `IsSuccessStatusCode` and throw `PassThroughHttpException` for non-success
...
├── PassThroughHttpException          // Propagate downstream errors
└── RequestValidationFailureException // DTO validation failures

Core Exceptions
├── NoRequestBodyException            // Missing request body
├── HttpBaseServerException           // Server errors (central data layer)
└── NotFoundException                 // Not found cases
```
...
**Key Rules:**
- ✅ **400 status codes** - Client errors (validation, business rules, missing body)
- ✅ **500 status codes** - Server errors (Central Data Layer only)
- ✅ **Preserved status codes** - PassThroughHttpException preserves the original HTTP status from downstream

### 20.6 Exception Handling Rules - StepName Format

**🔴 CRITICAL: StepName Format**
...
**Key Rules:**
- ✅ **Format:** `"<FileName>.cs / Executing <MethodName> / <OptionalContext>"`
- ✅ **FileName** - Actual file name (e.g., `CreateEntityFunction.cs`)
- ✅ **MethodName** - Actual method name (e.g., `Run`, `GetEntity`, `ProcessBatch`)
- ✅ **OptionalContext** - Additional context (e.g., exception type, validation step, operation name)
- ✅ **Always include stepName** - All exceptions MUST have stepName parameter
...
**Key Rules:**
- ✅ **errorDetails** - List of error messages (for validation exceptions)
- ✅ **ErrorConstants** - Use error constants for error codes and messages (see Section 14)
- ✅ **BaseResponseDTO** - Use `ExtractBaseResponse()` for PassThroughHttpException

### 20.8 Exception Handling Rules - Key Rules

| Aspect | Rule | Mandatory |
|--------|------|-----------|
| **Try-Catch Blocks** | ❌ NEVER use try-catch | ✅ **CRITICAL** |
| **Exception Propagation** | ✅ Let exceptions propagate to middleware | ✅ **MANDATORY** |
| **ExceptionHandlerMiddleware** | ✅ Handles all exceptions | ✅ Always |
| **StepName Format** | `"<FileName>.cs / Executing <MethodName> / <OptionalContext>"` | ✅ Always |
| **ErrorDetails** | List of error messages or ErrorConstants | ✅ When required |
| **Logging Before Exception** | Use `_logger.Error()` before throwing | ✅ Recommended |
| **Response Status Check** | Check `IsSuccessStatusCode` and throw PassThroughHttpException | ✅ Always for HTTP responses |

**🔴 CRITICAL RULES:**

1. ✅ **NEVER use try-catch blocks** - Let exceptions propagate to ExceptionHandlerMiddleware
2. ✅ **Always include stepName** - All exceptions MUST have stepName parameter with standardized format
3. ✅ **Use layer-appropriate exceptions** - Function layer uses NoRequestBodyException, DTO layer uses RequestValidationFailureException, Service layer uses BusinessCaseFailureException
4. ✅ **Check response status** - Always check `IsSuccessStatusCode` and throw PassThroughHttpException for non-success
5. ✅ **Use ErrorConstants** - Use error constants for error codes and messages (see Section 14)
6. ✅ **Log before throwing** - Use `_logger.Error()` before throwing exceptions for debugging
7. ✅ **ExceptionHandlerMiddleware handles all** - All exceptions are normalized to BaseResponseDTO by middleware
8. ✅ **HttpBaseServerException for Central Data Layer only** - Use for DB/config failures in Central Data Layer

### 20.9 Exception Handling Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER throwing exceptions, you MUST verify:**

1. ✅ **STOP AND VERIFY:** NO try-catch blocks in code (exceptions propagate to middleware)
2. ✅ **STOP AND VERIFY:** Correct exception type used (layer-appropriate)
3. ✅ **STOP AND VERIFY:** stepName follows standard format: `"<FileName>.cs / Executing <MethodName> / <OptionalContext>"`
4. ✅ **STOP AND VERIFY:** errorDetails provided (if required by exception type)
5. ✅ **STOP AND VERIFY:** ErrorConstants used (not hardcoded error messages)
6. ✅ **STOP AND VERIFY:** Response status checked before throwing PassThroughHttpException
7. ✅ **STOP AND VERIFY:** Logging added before throwing exceptions (for debugging)

**🛑 IF ANY VIOLATION → STOP → Fix exception handling → Then proceed**
...
**🚨 CRITICAL:** These rules NEVER change. They define the fundamental architecture boundaries and constraints that must be followed across all Process Layer implementations.

### 21.1 Architecture Invariants Rules - Pre-Creation Validation

**🛑 PRE-CREATION VALIDATION (MANDATORY - STOP IF NOT COMPLETE)**

**🚨 BEFORE creating any Process Layer code, you MUST verify:**

1. ✅ **STOP AND VERIFY:** Have I read this section completely?
2. ✅ **STOP AND VERIFY:** Do I understand the layer boundaries (what is ALLOWED vs FORBIDDEN)?
3. ✅ **STOP AND VERIFY:** Do I understand the Central Data Layer as database gateway pattern?
4. ✅ **STOP AND VERIFY:** Do I understand the header standardization rules?
5. ✅ **STOP AND VERIFY:** Do I understand the middleware restrictions?
6. ✅ **STOP AND VERIFY:** Do I understand the folder naming conventions?
7. ✅ **STOP AND VERIFY:** Do I understand the repository management principles?

**🛑 IF ANY MISSING → STOP → Complete verification → Then proceed**

### 21.2 Architecture Invariants Rules - Layer Boundaries

**🔴 CRITICAL: Layer Boundaries (NEVER VIOLATE)**

**Allowed Communication Patterns:**
1. ✅ **Process Layer → System Layer** - ALLOWED (via System Abstractions)
2. ✅ **Process Layer → Process Layer** - ALLOWED (via Process Abstractions)

**Forbidden Communication Patterns:**
3. ❌ **System Layer → Process Layer** - FORBIDDEN
4. ❌ **System Layer → System Layer** - FORBIDDEN
5. ❌ **Process Layer → Downstream API** - FORBIDDEN (must go through System Layer)

**Key Rules:**
- ✅ **Process Layer calls System Layer** - Use System Abstractions with `SendProcessHTTPReqAsync()` (see Section 5)
- ✅ **Process Layer calls Process Layer** - Use Process Abstractions with `SendProcessHTTPReqAsync()` (see Section 6)
- ❌ **System Layer MUST NOT call Process Layer** - This violates architecture boundaries
- ❌ **System Layer MUST NOT call System Layer** - System Layers are independent
- ❌ **Process Layer MUST NOT call Downstream APIs directly** - All external API calls must go through System Layer
...
**🔴 CRITICAL: Central Data Layer Database Access Pattern**

**Central Data Layer (Shared Services):**
1. ✅ **Central data layer** - ONLY Process Layer with direct DB access
2. ✅ **Direct database access** - Via EF Core, Repository<T>, and AppDbContext
3. ✅ **Flyway migrations** - Integrated in central data layer CI/CD pipeline

**All Other Process Layers:**
1. ✅ **Database access via central data layer** - MUST call Central Data Layer Azure Functions
2. ❌ **NO direct database access** - Cannot use EF Core, Repository<T>, or AppDbContext
3. ❌ **NO database migrations** - Flyway is only in Central Data Layer

**Key Rules:**
- ✅ **Central Data Layer** - Has `Repositories/`, `Models/`, `AutoMapping/`, `AppDbContext`, `KeyVaultReader`, `DatabaseConnectionStringProvider`
- ✅ **Standard Process Layer** - Has `Domains/`, `DTOs/`, `SystemAbstractions/`, `ProcessAbstractions/`, `Services/`, `Functions/`
- ❌ **Standard Process Layer MUST NOT have** - `Repositories/`, `Models/`, `AutoMapping/`, `AppDbContext`
- ✅ **Standard Process Layer MUST call** - Central Data Layer Azure Functions for ANY database operations
...
**🔴 CRITICAL: Header Standardization (MANDATORY FORMAT)**

**Process→System Layer Headers:**
```csharp
List<Tuple<string, string>> headers = new List<Tuple<string, string>> 
{ 
...
**Key Rules:**
- ✅ **ALWAYS include TestRunId and RequestId** - Use `RequestHeaders.TestRunId.Simplify()` and `RequestHeaders.RequestId.Simplify()`
- ✅ **Process→System** - Only TestRunId and RequestId (no additional headers)
- ✅ **Process→Process** - TestRunId, RequestId, plus all headers from incoming request (if `req.Headers != null`)
- ✅ **Use `SendProcessHTTPReqAsync()`** - This extension method automatically adds TestRunId/RequestId headers (see Section 5.5 and Section 6.4)
...
**🔴 CRITICAL: Middleware Restrictions (MANDATORY)**

**Process Layer Middleware (ONLY):**
1. ✅ **ExecutionTimingMiddleware** - Request timing tracking (MANDATORY)
2. ✅ **ExceptionHandlerMiddleware** - Centralized exception handling (MANDATORY)

**Forbidden Middleware:**
3. ❌ **NO CustomAuthenticationMiddleware** - System Layer concept (NOT for Process Layer)
4. ❌ **NO custom middlewares** - Use framework middlewares only
5. ❌ **NO additional middlewares** - Only ExecutionTiming and ExceptionHandler

**Key Rules:**
- ✅ **ExecutionTimingMiddleware** - MUST be registered first (see Section 11.3)
- ✅ **ExceptionHandlerMiddleware** - MUST be registered second (see Section 11.3)
- ❌ **NO custom authentication** - Process Layer does not handle authentication
- ❌ **NO custom middleware** - Only use framework-provided middlewares
...
**🔴 CRITICAL: Folder Naming Conventions (MANDATORY)**

**Standard Process Layer Folders:**
1. ✅ **DTOs/** - Request/response data transfer objects
2. ✅ **SystemAbstractions/** - System Layer calls (Process→System communication)
3. ✅ **ProcessAbstractions/** - Process Layer calls (Process→Process communication)
4. ✅ **Domains/** - Business objects with `IDomain<T>` interface
5. ✅ **Services/** - Orchestration logic
6. ✅ **Functions/** - Azure Functions (HTTP orchestrators)

**Central Data Layer Additional Folders:**
7. ✅ **Repositories/** - Generic Repository<T> + specific repositories (Central Data Layer ONLY)
8. ✅ **Models/** - EF Core entities (Central Data Layer ONLY)
9. ✅ **AutoMapping/** - AutoMapper profiles (Central Data Layer ONLY)

**Key Rules:**
- ✅ **DTOs/** - For ALL request/response DTOs (see Section 7)
- ✅ **SystemAbstractions/** - For ALL System Layer calls (see Section 5)
- ✅ **ProcessAbstractions/** - For ALL Process Layer calls (see Section 6)
- ✅ **Domains/** - For ALL business objects with `IDomain<T>` (see Section 3) - EXCEPT Central Data Layer
- ❌ **Standard Process Layer MUST NOT have** - `Repositories/`, `Models/`, `AutoMapping/`
- ✅ **Central Data Layer ONLY** - Has `Repositories/`, `Models/`, `AutoMapping/`
...
**🔴 CRITICAL: Repository Management Principles (MANDATORY)**
...
1. ✅ **One repository per business process**
   - Each Process Layer must be in a dedicated code repository
   - ❌ Multiple processes in one repository (unless closely related business domain)

2. ✅ **Scan repositories before creating code**
...
3. ✅ **Use existing repository if available**
...
5. ✅ **Maintain repository awareness**
...
1. ✅ **Check if Function already exists** in target Process Layer repository
...
// New: public string EntityDate { get; set; } → BREAKING

// ✅ NON-BREAKING: Added optional field
// Original: public string EntityName { get; set; }
// New: public string EntityName { get; set; }
//      public string? EntityEmail { get; set; }  // Optional → NON-BREAKING
```
...
**Key Rules:**
- ✅ **One repository per business process** - Maintain clear separation
- ✅ **Scan before creating** - Check if repository/Function already exists
- ✅ **Reuse existing** - Update existing Function for non-breaking changes
- ✅ **Version for breaking changes** - Create V2, V3, etc. for breaking changes
- ❌ **NO multiple processes in one repository** - Unless closely related business domain
...
| Aspect | Rule | Mandatory |
|--------|------|-----------|
| **Layer Boundaries** | Process→System ✅, Process→Process ✅, System→Process ❌, System→System ❌, Process→Downstream API ❌ | ✅ **CRITICAL** |
| **Central Data Layer** | ONLY Process Layer with direct DB access | ✅ **CRITICAL** |
| **Standard Process Layer** | Database access via Central Data Layer Azure Functions | ✅ **CRITICAL** |
| **Header Standardization** | TestRunId + RequestId (Process→System), TestRunId + RequestId + req.Headers (Process→Process) | ✅ **MANDATORY** |
| **Middleware** | ONLY ExecutionTiming + ExceptionHandler | ✅ **MANDATORY** |
| **Folder Naming** | DTOs/, SystemAbstractions/, ProcessAbstractions/, Domains/, Services/, Functions/ | ✅ **MANDATORY** |
| **Repository Naming** | `proc-<reponame>` format | ✅ **MANDATORY** |
| **Function Reuse** | Check if Function exists, update for non-breaking, version for breaking | ✅ **MANDATORY** |

**🔴 CRITICAL RULES:**

1. ✅ **NEVER violate layer boundaries** - Process→System ✅, Process→Process ✅, System→Process ❌, System→System ❌, Process→Downstream API ❌
2. ✅ **NEVER add direct database access to Standard Process Layer** - Use Central Data Layer Azure Functions
3. ✅ **ALWAYS use standardized headers** - TestRunId + RequestId for all Process Layer communications
4. ✅ **NEVER add custom middleware** - Only ExecutionTiming + ExceptionHandler
5. ✅ **ALWAYS follow folder naming conventions** - DTOs/, SystemAbstractions/, ProcessAbstractions/, Domains/, Services/, Functions/
6. ✅ **ALWAYS use repository naming convention** - `proc-<reponame>` format
7. ✅ **ALWAYS check for existing Functions** - Reuse or version appropriately

### 21.9 Architecture Invariants Rules - Post-Creation Validation

**🛑 POST-CREATION VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING)**

**🚨 AFTER creating Process Layer code, you MUST verify:**

1. ✅ **STOP AND VERIFY:** NO layer boundary violations (System→Process, System→System, Process→Downstream API)
2. ✅ **STOP AND VERIFY:** NO direct database access in Standard Process Layer (use Central Data Layer)
3. ✅ **STOP AND VERIFY:** Headers follow standardization (TestRunId + RequestId)
4. ✅ **STOP AND VERIFY:** ONLY ExecutionTiming + ExceptionHandler middleware (no custom middleware)
5. ✅ **STOP AND VERIFY:** Folder names follow conventions (DTOs/, SystemAbstractions/, ProcessAbstractions/, Domains/, Services/, Functions/)
6. ✅ **STOP AND VERIFY:** Repository follows naming convention (`proc-<reponame>`)
7. ✅ **STOP AND VERIFY:** Function reuse/versioning rules followed (check existing, update or version appropriately)

**🛑 IF ANY VIOLATION → STOP → Fix architecture violations → Then proceed**
...
- **CORRECT:** Process→System ✅, Process→Process ✅, System→Process ❌, System→System ❌, Process→Downstream API ❌
...
**❌ Direct Database Access in Standard Process Layer**
...
**❌ Not Checking for Existing Functions**
...
**🚨 CRITICAL:** Constants centralize all business logic string literals. When adding a new operation, you MUST update Constants files.

### 22.1 Constants Update Rules - Checklist

**🔴 MANDATORY: When adding a new operation, you MUST update Constants:**
...
**🔴 CRITICAL: Constants MUST be used in Functions (NOT hardcoded strings):**
...
- ✅ Function attribute names: `[Function("<Operation>")]` (literal string)
- ✅ stepName values: `"<Operation>Function.cs / Executing Run"` (literal string)
- ✅ Logging messages: `_logger.Info("<Operation> completed successfully.");` (literal string)

**❌ WRONG (Hardcoded in Business Logic):**
```csharp
return new BaseResponseDTO("<Entity> retrieved successfully", string.Empty, resDto);  // ❌ Hardcoded
```

**✅ CORRECT (Using Constants):**
```csharp
return new BaseResponseDTO(InfoConstants.<OPERATION>_SUCCESS, string.Empty, resDto);  // ✅ Constant
```
...
**🛑 POST-UPDATE VALIDATION (MANDATORY - VERIFY BEFORE PROCEEDING):**

**🚨 AFTER updating Constants, you MUST verify:**

1. ✅ **STOP AND VERIFY:** ErrorConstants.cs has error constant (if needed): `public static readonly (string ErrorCode, string Message) <OPERATION>_FAILURE = ...`
2. ✅ **STOP AND VERIFY:** InfoConstants.cs has success message: `public const string <OPERATION>_SUCCESS = "...";`
3. ✅ **STOP AND VERIFY:** InfoConstants.cs has process name: `public const string PROCESS_NAME_<OPERATION> = "...";`
4. ✅ **STOP AND VERIFY:** Function uses constants (not hardcoded strings) in business logic
5. ✅ **STOP AND VERIFY:** Constants follow naming conventions (uppercase with underscores)
6. ✅ **STOP AND VERIFY:** ErrorCode follows format: `<SOR>_<ABBREVIATION>_<NUMBER>`

**🛑 IF ANY VIOLATION → STOP → Fix Constants → Then proceed**

**🔴 CROSS-REFERENCE:** This is STEP 10 of the "MANDATORY CODE GENERATION WORKFLOW" at the top of this document.
