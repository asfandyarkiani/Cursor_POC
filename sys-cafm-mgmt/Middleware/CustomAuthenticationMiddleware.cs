...
{
    /// <summary>
    /// Middleware for CAFM session-based authentication.
    ...
    public class CustomAuthenticationMiddleware : IFunctionsWorkerMiddleware
    {
        ...
        public async Task Invoke(FunctionContext context, FunctionExecutionDelegate next)
        {
            ...
            try
            {
                _logger.Info("Starting CAFM authentication");
                ...
                if (!authResponse.IsSuccessStatusCode)
                {
                    ...
                    throw new DownStreamApiFailureException(
                        ...
                        errorDetails: new List<string> { $"Authentication failed. Status: {authResponse.StatusCode}. Response: {authResponse.Content}" },
                        stepName: "CustomAuthenticationMiddleware.cs / Invoke");
                }

                AuthenticationResponseDTO? authAPIRes = Helpers.SOAPHelper.DeserializeSoapResponse<AuthenticationResponseDTO>(authResponse.Content!);
                ...
                if (string.IsNullOrEmpty(sessionId))
                {
                    ...
                    throw new DownStreamApiFailureException(
                        ...
                        errorDetails: new List<string> { "Authentication succeeded but no SessionId in response" },
                        stepName: "CustomAuthenticationMiddleware.cs / Invoke");
                }
            ...
            finally
            {
                if (!string.IsNullOrEmpty(sessionId))
                {
                    _logger.Info("Executing CAFM logout");
                    try
                    {
                        await _logoutAtomicHandler.Handle(new LogoutRequestDTO { SessionId = sessionId });
                        _logger.Info("CAFM logout completed");
                    }
                    catch (Exception ex)
                    {
                        _logger.Warn($"CAFM logout failed but continuing: {ex.Message}");
                    }
                    finally
                    {
